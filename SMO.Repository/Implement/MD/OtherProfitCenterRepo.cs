...
{
    public class OtherProfitCenterRepo : GenericCenterRepository<T_MD_OTHER_PROFIT_CENTER>, IOtherProfitCenterRepo
    {
        public OtherProfitCenterRepo(NHUnitOfWork unitOfWork) : base(unitOfWork)
        {
        }

        public override IList<T_MD_OTHER_PROFIT_CENTER> Search(T_MD_OTHER_PROFIT_CENTER objFilter, int pageSize, int pageIndex, out int total)
        {
            var query = Queryable();
