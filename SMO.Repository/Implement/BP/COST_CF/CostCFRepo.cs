...
{
    public class CostCFRepo : GenericBPRepository<T_BP_COST_CF>, ICostCFRepo
    {
        public CostCFRepo(NHUnitOfWork unitOfWork) : base(unitOfWork)
        {

        }

        public override IList<T_BP_COST_CF> Search(T_BP_COST_CF objFilter, int pageSize, int pageIndex, out int total)
        {
            var query = NHibernateSession.QueryOver<T_BP_COST_CF>();
