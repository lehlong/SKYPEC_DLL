...
{
    public class CostPLRepo : GenericBPRepository<T_BP_COST_PL>, ICostPLRepo
    {
        public CostPLRepo(NHUnitOfWork unitOfWork) : base(unitOfWork)
        {

        }

        public override IList<T_BP_COST_PL> Search(T_BP_COST_PL objFilter, int pageSize, int pageIndex, out int total)
        {
            var query = NHibernateSession.QueryOver<T_BP_COST_PL>();
