...
namespace SMO.Areas.BP.Controllers
{
    ...
    public class ContructCostCFController : 
        BPControllerBase<ContructCostCFService, T_BP_CONTRUCT_COST_CF, ContructCostCFRepo, T_MD_COST_CF_ELEMENT, T_BP_CONTRUCT_COST_CF_VERSION, T_BP_CONTRUCT_COST_CF_HISTORY, ContructCostCFHistoryRepo>, 
        IBPController<ContructCostCFService, T_BP_CONTRUCT_COST_CF, ContructCostCFRepo, T_MD_COST_CF_ELEMENT, T_BP_CONTRUCT_COST_CF_VERSION, T_BP_CONTRUCT_COST_CF_HISTORY, ContructCostCFHistoryRepo>
    {
        public override FileContentResult DownloadTemplate(string templateId, int year)
        {
            ...
            if (template.IS_BASE)
            {
                path = Server.MapPath("~/TemplateExcel/" + "Template_Base_Contruct_CostCF.xlsx");
                _service.GenerateTemplateBase(ref outFileStream, path, templateId, year);
            }
            else
            {
                path = Server.MapPath("~/TemplateExcel/" + "Template_Contruct_CostCF.xlsx");
        ...
        {
            // cost
            var dataCost = _service.SummaryCenterOut(out IList<T_BP_CONTRUCT_COST_CF_DATA> detailCostElements, centerCode ?? ProfileUtilities.User.ORGANIZE_CODE, year.Value, version);
        ...
        {
            var dataCost = _service.GetDataCFCost(out IList<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_CF> detailCostElements,
                out IList<T_BP_CONTRUCT_COST_CF_DATA> detailCostData, out bool isDrillDownApply, model);
            if (dataCost == null)
            {
        ...
        public override ActionResult ViewTemplate(string templateId, int? version, int year, string centerCode = "")
        {
            ...
            {
                var dataCost = _service.PreparePureListForTemplate(out IList<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_CF> detailContructCostElements, templateId, year);
                ViewBag.detailContructCostElements = detailContructCostElements;
                var isBaseTemplate = _service.GetTemplate(templateId).IS_BASE;
                if (isBaseTemplate)
                {
                    return PartialView("ViewTemplateBaseContructCostCF", dataCost);
                }
                else
                {
                    return PartialView("ViewTemplateContructCostCF", dataCost);
                }
            }
            else
            {
