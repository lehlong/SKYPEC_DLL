...
namespace SMO.Areas.BP.Controllers
{
    ...
    [Authorize]
    public class CostPLController : 
        BPControllerBase<CostPLService, T_BP_COST_PL, CostPLRepo, T_MD_COST_PL_ELEMENT, T_BP_COST_PL_VERSION, T_BP_COST_PL_HISTORY, CostPLHistoryRepo>, 
        IBPController<CostPLService, T_BP_COST_PL, CostPLRepo, T_MD_COST_PL_ELEMENT, T_BP_COST_PL_VERSION, T_BP_COST_PL_HISTORY, CostPLHistoryRepo>
    {
        public override FileContentResult DownloadTemplate(string templateId, int year)
        {
            ...
            if (template.IS_BASE)
            {
                path = Server.MapPath("~/TemplateExcel/" + "Template_Base_CostPL.xlsx");
                _service.GenerateTemplateBase(ref outFileStream, path, templateId, year);
            }
            else
            {
                path = Server.MapPath("~/TemplateExcel/" + "Template_CostPL.xlsx");
        ...
        {
            var dataCost = _service.GetDataCost(out IList<T_MD_TEMPLATE_DETAIL_COST_PL> detailCostElements,
                out IList<T_BP_COST_PL_DATA> detailCostData, out bool isDrillDownApply, model);
            if (dataCost == null)
            {
        ...
        {
            // cost
            var dataCost = _service.SummaryCenterOut(out IList<T_BP_COST_PL_DATA> detailCostElements, centerCode ?? ProfileUtilities.User.ORGANIZE_CODE, year.Value, version);
            ViewBag.detailCostElements = detailCostElements;
        ...
        public override ActionResult ViewTemplate(string templateId, int? version, int year, string centerCode = "")
        {
            ...
            {
                var dataCost = _service.PreparePureListForTemplate(out IList<T_MD_TEMPLATE_DETAIL_COST_PL> detailCostElements, templateId, year);
                ViewBag.detailCostElements = detailCostElements;
                var isBaseTemplate = _service.GetTemplate(templateId).IS_BASE;
                if (isBaseTemplate)
                {
                    return PartialView("ViewTemplateBaseCostPL", dataCost);
                }
                else
                {
                    return PartialView("ViewTemplateCostPL", dataCost);
                }
            }
            else
            {
