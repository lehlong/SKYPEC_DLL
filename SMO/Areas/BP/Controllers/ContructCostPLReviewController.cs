...
{
    public class ContructCostPLReviewController : Controller
    {
        private readonly ContructCostPLReviewService _service;

        public ContructCostPLReviewController()
        {
            _service = new ContructCostPLReviewService();
        }
        // GET: BP/ContructCostPLReview
        ...
        public ActionResult Index(int year)
        {
            ...
            var dataCost = _service.SummaryCenterVersion(out IList<T_BP_CONTRUCT_COST_PL_DATA> detailCostElements);
            ...
            var model = new ContructCostPLReviewViewModel
            {
        ...
        public ActionResult ResultReviewsIndex()
        {
        ...
        public ActionResult ReviewIndex(ContructCostPLReviewService service)
        {
            service.SearchReview();
            ViewBag.header = service.GetHeader();
            ViewBag.showReviewBtn = new ContructCostPLService()
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ReviewHistoryIndex(ContructCostPLReviewService service)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateReview(ContructCostPLReviewViewModel model)
        {
            ...
            if (_service.State)
            {
                if (model.IsEnd)
                {
                    if (model.IsSummary)
                    {
                        return RedirectToAction("TrinhDuyetTongKiemSoat", "ContructCostPL",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                    else
                    {
                        return RedirectToAction("KetThucThamDinh", "ContructCostPL",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                }
                else
