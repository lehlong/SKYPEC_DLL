...
{
    public class OtherCostCFReviewController : Controller
    {
        private readonly OtherCostCFReviewService _service;

        public OtherCostCFReviewController()
        {
            _service = new OtherCostCFReviewService();
        }
        // GET: BP/OtherCostCFReview
        ...
        public ActionResult Index(int year)
        {
            ...
            var dataCost = _service.SummaryCenterVersion(out IList<T_BP_OTHER_COST_CF_DATA> detailCostElements);
            ...
            var model = new OtherCostCFReviewViewModel
            {
        ...
        public ActionResult ReviewIndex(OtherCostCFReviewService service)
        {
            service.SearchReview();
            ViewBag.header = service.GetHeader();
            ViewBag.showReviewBtn = new OtherCostCFService()
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ReviewHistoryIndex(OtherCostCFReviewService service)
        {
        ...
        [ValidateAntiForgeryToken]
        // 210003
        public ActionResult Details(OtherCostCFReviewService service)
        {
            service.ObjDetail.ORG_CODE = service.GetCorp().CODE;
            var dataCost = service.SummaryCenterVersion(out IList<T_BP_OTHER_COST_CF_DATA> detailCostElements);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateReview(OtherCostCFReviewViewModel model)
        {
            ...
            if (_service.State)
            {
                if (model.IsEnd)
                {
                    if (model.IsSummary)
                    {
                        return RedirectToAction("TrinhDuyetTongKiemSoat", "OtherCostCF",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                    else
                    {
                        return RedirectToAction("KetThucThamDinh", "OtherCostCF",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                }
                else
