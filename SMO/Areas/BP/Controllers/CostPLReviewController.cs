...
{
    public class CostPLReviewController : Controller
    {
        private readonly CostPLReviewService _service;

        public CostPLReviewController()
        {
            _service = new CostPLReviewService();
        }
        // GET: BP/CostPLReview
        ...
        public ActionResult Index(int year)
        {
            ...
            var dataCost = _service.SummaryCenterVersion(out IList<T_BP_COST_PL_DATA> detailCostElements);
            ...
            var model = new CostPLReviewViewModel
            {
        ...
        public ActionResult ReviewIndex(CostPLReviewService service)
        {
            service.SearchReview();
            ViewBag.header = service.GetHeader();
            ViewBag.showReviewBtn = new CostPLService()
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ReviewHistoryIndex(CostPLReviewService service)
        {
        ...
        [ValidateAntiForgeryToken]
        // 210003
        public ActionResult Details(CostPLReviewService service)
        {
            service.ObjDetail.ORG_CODE = service.GetCorp().CODE;
            var dataCost = service.SummaryCenterVersion(out IList<T_BP_COST_PL_DATA> detailCostElements);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateReview(CostPLReviewViewModel model)
        {
            ...
            if (_service.State)
            {
                if (model.IsEnd)
                {
                    if (model.IsSummary)
                    {
                        return RedirectToAction("TrinhDuyetTongKiemSoat", "CostPL",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                    else
                    {
                        return RedirectToAction("KetThucThamDinh", "CostPL",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                }
                else
