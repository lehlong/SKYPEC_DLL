...
namespace SMO.Areas.BP.Controllers
{
    ...
    public class OtherCostPLController : 
        BPControllerBase<OtherCostPLService, T_BP_OTHER_COST_PL, OtherCostPLRepo, T_MD_COST_PL_ELEMENT, T_BP_OTHER_COST_PL_VERSION, T_BP_OTHER_COST_PL_HISTORY, OtherCostPLHistoryRepo>, 
        IBPController<OtherCostPLService, T_BP_OTHER_COST_PL, OtherCostPLRepo, T_MD_COST_PL_ELEMENT, T_BP_OTHER_COST_PL_VERSION, T_BP_OTHER_COST_PL_HISTORY, OtherCostPLHistoryRepo>
    {
        public override FileContentResult DownloadTemplate(string templateId, int year)
        {
            ...
            if (template.IS_BASE)
            {
                path = Server.MapPath("~/TemplateExcel/" + "Template_Base_Other_CostPL.xlsx");
                _service.GenerateTemplateBase(ref outFileStream, path, templateId, year);
            }
            else
            {
                path = Server.MapPath("~/TemplateExcel/" + "Template_Other_CostPL.xlsx");
        ...
        {
            // cost
            var dataCost = _service.SummaryCenterOut(out IList<T_BP_OTHER_COST_PL_DATA> detailCostElements, centerCode ?? ProfileUtilities.User.ORGANIZE_CODE, year.Value, version);
        ...
        {
            var dataCost = _service.GetDataCost(out IList<T_MD_TEMPLATE_DETAIL_OTHER_COST_PL> detailCostElements,
                out IList<T_BP_OTHER_COST_PL_DATA> detailCostData, out bool isDrillDownApply, model);
            if (dataCost == null)
            {
            ...
            ViewBag.costPLHeader = _service.GetHeader(model);
        ...
        public override ActionResult ViewTemplate(string templateId, int? version, int year, string centerCode = "")
        {
            ...
            {
                var dataCost = _service.PreparePureListForTemplate(out IList<T_MD_TEMPLATE_DETAIL_OTHER_COST_PL> detailOtherCostElements, templateId, year);
                ViewBag.detailOtherCostElements = detailOtherCostElements;
                var isBaseTemplate = _service.GetTemplate(templateId).IS_BASE;
                if (isBaseTemplate)
                {
                    return PartialView("ViewTemplateBaseOtherCostPL", dataCost);
                }
                else
                {
                    return PartialView("ViewTemplateOtherCostPL", dataCost);
                }
            }
            else
            {
