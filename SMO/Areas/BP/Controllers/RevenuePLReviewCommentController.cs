...
{
    public class RevenuePLReviewCommentController : Controller
    {
        private readonly RevenuePLReviewCommentService _service;

        public RevenuePLReviewCommentController()
        {
            _service = new RevenuePLReviewCommentService();
        }
        ...
        public ActionResult List(RevenuePLReviewCommentService service)
        {
            service.GetComments();
            return PartialView("../Comment/ListRevenuePL", service.ObjList);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult Comment(RevenuePLReviewCommentService service)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult CommentDataCenter(RevenuePLReviewCommentService service)
        {
            var result = new TransferObject
            {
                Type = TransferType.AlertSuccessAndJsCommand
            ...
            if (service.State)
            {
                ...
                result.ExtData = $"Forms.SubmitForm('{service.ObjDetail.PKID}'); " +
                    $"RefreshComment('{service.ObjDetail.REVENUE_PL_ELEMENT_CODE}', '{service.ObjDetail.ON_ORG_CODE}'); " +
                    $"$('#txtContent').val('')";
            }
            else
            {
