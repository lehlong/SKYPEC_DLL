...
{
    public class ContructCostCFReviewController : Controller
    {
        private readonly ContructCostCFReviewService _service;

        public ContructCostCFReviewController()
        {
            _service = new ContructCostCFReviewService();
        }
        // GET: BP/ContructCostCFReview
        // thẩm định
        ...
        public ActionResult Index(int year)
        {
            ...
            var dataCost = _service.SummaryCenterVersion(out IList<T_BP_CONTRUCT_COST_CF_DATA> detailCostElements);
            ...
            var model = new ContructCostCFReviewViewModel
            {
        ...
        public ActionResult ReviewIndex(ContructCostCFReviewService service)
        {
            service.SearchReview();
            ViewBag.header = service.GetHeader();
            ViewBag.showReviewBtn = new ContructCostCFService()
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ReviewHistoryIndex(ContructCostCFReviewService service)
        {
        ...
        //[ValidateAntiForgeryToken]
        //// 210003
        //public ActionResult Details(ContructCostCFReviewService service)
        //{
        //    service.ObjDetail.ORG_CODE = service.GetCorp().CODE;
        //    var dataCost = service.SummaryCenterVersion(out IList<T_BP_CONTRUCT_COST_CF_DATA> detailCostElements);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateReview(ContructCostCFReviewViewModel model)
        {
            ...
            if (_service.State)
            {
                if (model.IsEnd)
                {
                    if (model.IsSummary)
                    {
                        return RedirectToAction("TrinhDuyetTongKiemSoat", "ContructCostCF",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                    else
                    {
                        return RedirectToAction("KetThucThamDinh", "ContructCostCF",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                }
                else
