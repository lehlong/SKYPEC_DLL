...
{
    public class CostCFReviewController : Controller
    {
        private readonly CostCFReviewService _service;

        public CostCFReviewController()
        {
            _service = new CostCFReviewService();
        }
        // GET: BP/CostCFReview
        ...
        public ActionResult Index(int year)
        {
            ...
            var dataCost = _service.SummaryCenterVersion(out IList<T_BP_COST_CF_DATA> detailCostElements);
            ...
            var model = new CostCFReviewViewModel
            {
        ...
        public ActionResult ReviewIndex(CostCFReviewService service)
        {
            service.SearchReview();
            ViewBag.header = service.GetHeader();
            ViewBag.showReviewBtn = new CostCFService()
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ReviewHistoryIndex(CostCFReviewService service)
        {
        ...
        [ValidateAntiForgeryToken]
        // 210003
        public ActionResult Details(CostCFReviewService service)
        {
            service.ObjDetail.ORG_CODE = service.GetCorp().CODE;
            var dataCost = service.SummaryCenterVersion(out IList<T_BP_COST_CF_DATA> detailCostElements);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateReview(CostCFReviewViewModel model)
        {
            ...
            if (_service.State)
            {
                if (model.IsEnd)
                {
                    if (model.IsSummary)
                    {
                        return RedirectToAction("TrinhDuyetTongKiemSoat", "CostCF",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                    else
                    {
                        return RedirectToAction("KetThucThamDinh", "CostCF",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                }
                else
