...
namespace SMO.Areas.BP.Controllers
{
    ...
    public class RevenueCFController : 
        BPControllerBase<RevenueCFService, T_BP_REVENUE_CF, RevenueCFRepo, T_MD_REVENUE_CF_ELEMENT, T_BP_REVENUE_CF_VERSION, T_BP_REVENUE_CF_HISTORY, RevenueCFHistoryRepo>, 
        IBPController<RevenueCFService, T_BP_REVENUE_CF, RevenueCFRepo, T_MD_REVENUE_CF_ELEMENT, T_BP_REVENUE_CF_VERSION, T_BP_REVENUE_CF_HISTORY, RevenueCFHistoryRepo>
    {
        public override FileContentResult DownloadTemplate(string templateId, int year)
        {
            ...
            if (template.IS_BASE)
            {
                path = Server.MapPath("~/TemplateExcel/" + "Template_Base_RevenueCF.xlsx");
                _service.GenerateTemplateBase(ref outFileStream, path, templateId, year);
            }
            else
            {
                path = Server.MapPath("~/TemplateExcel/" + "Template_RevenueCF.xlsx");
        ...
        {
            // revenue
            var dataRevenue = _service.SummaryCenterOut(out IList<T_BP_REVENUE_CF_DATA> detailRevenueElements, centerCode ?? ProfileUtilities.User.ORGANIZE_CODE, year.Value, version);
            ...
            if (!isRenderPartial)
            {
                return PartialView("ViewSummaryCenterRevenueCF", dataRevenue);
            }
            else
            {
                return PartialView("_PartialViewSummaryCenterRevenueCF", dataRevenue);
            }
        }
        ...
        {
            var dataRevenue = _service.GetDataRevenue(out IList<T_MD_TEMPLATE_DETAIL_REVENUE_CF> detailRevenueElements,
                out IList<T_BP_REVENUE_CF_DATA> detailRevenueData, out bool isDrillDownApply, model);
            if (dataRevenue == null)
            {
            ...
            ViewBag.revenueCFHeader = _service.GetHeader(model);
        ...
        public override ActionResult ViewTemplate(string templateId, int? version, int year, string centerCode = "")
        {
            ...
            {
                var dataRevenue = _service.PreparePureListForTemplate(out IList<T_MD_TEMPLATE_DETAIL_REVENUE_CF> detailRevenueElements, templateId, year);
                ViewBag.detailRevenueElements = detailRevenueElements;
                var isBaseTemplate = _service.GetTemplate(templateId).IS_BASE;
                if (isBaseTemplate)
                {
                    return PartialView("ViewTemplateBaseRevenueCF", dataRevenue);
                }
                else
                {
                    return PartialView("ViewTemplateRevenueCF", dataRevenue);
                }
            }
            else
            {
