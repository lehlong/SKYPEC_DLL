...
{
    public class RevenuePLReviewController : Controller
    {
        private readonly RevenuePLReviewService _service;

        public RevenuePLReviewController()
        {
            _service = new RevenuePLReviewService();
        }
        // GET: BP/RevenuePLReview
        ...
        public ActionResult Index(int year)
        {
            ...
            var dataCost = _service.SummaryCenterVersion(out IList<T_BP_REVENUE_PL_DATA> detailCostElements);
            ...
            var model = new RevenuePLReviewViewModel
            {
        ...
        public ActionResult ReviewIndex(RevenuePLReviewService service)
        {
            service.SearchReview();
            ViewBag.header = service.GetHeader();
            ViewBag.showReviewBtn = new RevenuePLService()
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ReviewHistoryIndex(RevenuePLReviewService service)
        {
        ...
        [ValidateAntiForgeryToken]
        // 210003
        public ActionResult Details(RevenuePLReviewService service)
        {
            service.ObjDetail.ORG_CODE = service.GetCorp().CODE;
            var dataCost = service.SummaryCenterVersion(out IList<T_BP_REVENUE_PL_DATA> detailCostElements);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateReview(RevenuePLReviewViewModel model)
        {
            ...
            if (_service.State)
            {
                if (model.IsEnd)
                {
                    if (model.IsSummary)
                    {
                        return RedirectToAction("TrinhDuyetTongKiemSoat", "RevenuePL",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                    else
                    {
                        return RedirectToAction("KetThucThamDinh", "RevenuePL",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                }
                else
