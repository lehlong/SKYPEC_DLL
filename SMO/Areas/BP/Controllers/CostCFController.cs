...
namespace SMO.Areas.BP.Controllers
{
    ...
    public class CostCFController : 
        BPControllerBase<CostCFService, T_BP_COST_CF, CostCFRepo, T_MD_COST_CF_ELEMENT, T_BP_COST_CF_VERSION, T_BP_COST_CF_HISTORY, CostCFHistoryRepo>, 
        IBPController<CostCFService, T_BP_COST_CF, CostCFRepo, T_MD_COST_CF_ELEMENT, T_BP_COST_CF_VERSION, T_BP_COST_CF_HISTORY, CostCFHistoryRepo>
    {
        public override FileContentResult DownloadTemplate(string templateId, int year)
        {
            ...
            if (template.IS_BASE)
            {
                path = Server.MapPath("~/TemplateExcel/" + "Template_Base_CostCF.xlsx");
                _service.GenerateTemplateBase(ref outFileStream, path, templateId, year);
            }
            else
            {
                path = Server.MapPath("~/TemplateExcel/" + "Template_CostCF.xlsx");
        ...
        public override ActionResult ViewTemplate(string templateId, int? version, int year, string centerCode = "")
        {
            ...
            {
                var dataCost = _service.PreparePureListForTemplate(out IList<T_MD_TEMPLATE_DETAIL_COST_CF> detailCostElements, templateId, year);
                ViewBag.detailCostElements = detailCostElements;
                var isBaseTemplate = _service.GetTemplate(templateId).IS_BASE;
                if (isBaseTemplate)
                {
                    return PartialView("ViewTemplateBaseCostCF", dataCost);
                }
                else
                {
                    return PartialView("ViewTemplateCostCF", dataCost);
                }
            }
            else
            {
        ...
        {
            var dataCost = _service.GetDataCost(out IList<T_MD_TEMPLATE_DETAIL_COST_CF> detailCostElements,
                out IList<T_BP_COST_CF_DATA> detailCostData, out bool isDrillDownApply, model);
            if (dataCost == null)
            {
        ...
        {
            // cost
            var dataCost = _service.SummaryCenterOut(out IList<T_BP_COST_CF_DATA> detailCostElements, centerCode ?? ProfileUtilities.User.ORGANIZE_CODE, year.Value, version);
            ViewBag.detailCostElements = detailCostElements;
