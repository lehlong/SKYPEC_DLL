...
using SMO.Repository.Implement.BP.COST_CF.COST_CF_DATA_BASE;
using SMO.Repository.Implement.MD;
using SMO.Service.BP.COST_CF;
using SMO.Service.Class;
using SMO.Service.Common;
using SMO.ServiceInterface.BP.CostCF;
...
{
    public class CostCFService : BaseBPService<T_BP_COST_CF, CostCFRepo, T_MD_COST_CF_ELEMENT, T_BP_COST_CF_VERSION, T_BP_COST_CF_HISTORY, CostCFHistoryRepo>, ICostCFService
    {
        ...
        public List<T_BP_COST_CF_HISTORY> ObjListHistory { get; set; }
        public List<T_BP_COST_CF_VERSION> ObjListVersion { get; set; }
        public List<T_BP_COST_CF_SUM_UP_DETAIL> ObjListSumUpHistory { get; set; }
        public CostCFService()
        {
            this.StartRowData = 6;
            ...
            this.ObjListHistory = new List<T_BP_COST_CF_HISTORY>();
            this.ObjListVersion = new List<T_BP_COST_CF_VERSION>();
            this.ObjListSumUpHistory = new List<T_BP_COST_CF_SUM_UP_DETAIL>();
        }
        ...
        public override bool ShowReviewBtn(int year)
        {
            ...
            else
            {
                ...
                var historyReview = UnitOfWork.Repository<CostCFReviewRepo>()
        ...
        public override void TrinhDuyet(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<CostCFHistoryRepo>()
                    .Create(new T_BP_COST_CF_HISTORY()
                    {
                ...
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_COST_CF_DATA SET STATUS = '{Approve_Status.ChoPheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void HuyTrinhDuyet(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<CostCFHistoryRepo>().Create(new T_BP_COST_CF_HISTORY()
                {
                    PKID = Guid.NewGuid().ToString(),
                    ORG_CODE = ObjDetail.ORG_CODE,
                    TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_COST_CF_DATA SET STATUS = '{Approve_Status.ChuaTrinhDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void PheDuyet(string code)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_COST_CF_DATA SET STATUS = '{Approve_Status.DaPheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void HuyNop(string code)
        {
            ...
            try
            {
                ...
                UnitOfWork.Repository<CostCFVersionRepo>().Update(x => x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.VERSION == ObjDetail.VERSION, x => x.IS_DELETED = 1, x => x.UPDATE_BY = currentUser);
        ...
        public override void TuChoi(string code)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_COST_CF_DATA SET STATUS = '{Approve_Status.TuChoi}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void ChuyenTKS(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<CostCFReviewRepo>()
        ...
        public override void KetThucThamDinh(string orgCode, int year, int version)
        {
            try
            {
                ...
                UnitOfWork.BeginTransaction();
                var currentUser = ProfileUtilities.User?.USER_NAME;
                var reviewHeader = UnitOfWork.Repository<CostCFReviewRepo>()
                ...
                if (reviewHeader != null)
                {
                    ...
                    UnitOfWork.Repository<CostCFReviewRepo>().Update(reviewHeader);
                }

                var totalReviewUsers = UnitOfWork.Repository<UserReviewRepo>()
                    .GetManyWithFetch(x => x.TIME_YEAR == ObjDetail.TIME_YEAR).Count;

                var totalUsersEndReview = UnitOfWork.Repository<CostCFReviewRepo>()
                ...
                var review = UnitOfWork.Repository<CostCFReviewRepo>()
                ...
                if (review == null)
                {
                    ...
                    UnitOfWork.Repository<CostCFReviewRepo>().Create(
                        new T_BP_COST_CF_REVIEW
                        {
                            PKID = reviewId,
                ...
                {
                    review.IS_END = true;
                    UnitOfWork.Repository<CostCFReviewRepo>().Update(review);
                }
        ...
        public override void PheDuyetTongKiemSoat(string orgCode, int year, int version)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_COST_CF_DATA SET STATUS = '{Approve_Status.TKS_PheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
                UnitOfWork.Repository<CostCFReviewRepo>()
        ...
        internal override void TuChoiTongKiemSoat(string orgCode, int year, int version, string comment)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_COST_CF_DATA SET STATUS = '{Approve_Status.TKS_TuChoi}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
                UnitOfWork.Repository<CostCFReviewRepo>()
                    .Update(x => x.ORG_CODE == orgCode
                                && x.TIME_YEAR == year
                                && x.DATA_VERSION == version
        ...
        public override void TrinhDuyetTongKiemSoat(string orgCode, int year, int version)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_COST_CF_DATA SET STATUS = '{Approve_Status.TKS_TrinhDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
                UnitOfWork.Repository<CostCFReviewRepo>()
                    .Update(x => x.ORG_CODE == orgCode
                                && x.TIME_YEAR == year
                                && x.DATA_VERSION == version
        ...
        public override void ChuyenHDNS(string code)
        {
            try
            {
                ...
                {
                    UnitOfWork.Repository<CostCFReviewRepo>().Update(x => x.DATA_VERSION == ObjDetail.VERSION
        ...
        public override void GetListOfChild()
        {
            ...
            // Tìm mẫu nộp hộ
            var listTemplateCode = this.UnitOfWork.Repository<TemplateDetailCostCFRepo>().Queryable().Where(
        ...
        {
            var query = UnitOfWork.GetSession().QueryOver<T_BP_COST_CF_SUM_UP_DETAIL>();
        ...
        public IList<T_MD_COST_CF_ELEMENT> GetDataCost(out IList<T_MD_TEMPLATE_DETAIL_COST_CF> detailCostElements,
            out IList<T_BP_COST_CF_DATA> detailCostData,
        ...
        {
            var plVersionRepo = UnitOfWork.Repository<CostCFRepo>();
            var plDataRepo = UnitOfWork.Repository<CostCFDataRepo>();
            var plDataHistoryRepo = UnitOfWork.Repository<CostCFDataHistoryRepo>();

            var plDataCostElements = UnitOfWork.Repository<CostCFDataRepo>().GetCFDataByCenterCode(null, new List<string> { centerCode }, year, null, null);
        ...
        private IList<T_MD_COST_CF_ELEMENT> GetDataCostPreview(out IList<T_MD_TEMPLATE_DETAIL_COST_CF> detailCostElements, string templateId, List<string> lstOrgs, int year, int? version, bool? isHasValue)
        {
            detailCostElements = new List<T_MD_TEMPLATE_DETAIL_COST_CF>();
            var lstElements = new List<T_MD_COST_CF_ELEMENT>();
            foreach (var orgCode in lstOrgs)
            {
                var elements = GetDataCostPreview(out IList<T_MD_TEMPLATE_DETAIL_COST_CF> detail, templateId, orgCode, year, version, isHasValue);
        ...
        public override T_BP_COST_CF_VERSION GetHeader(string templateCode, string centerCode, int year, int? version)
        {
            templateCode = templateCode ?? string.Empty;
            if (version == null)
            {
                return UnitOfWork.Repository<CostCFVersionRepo>()
        ...
        {
            ObjListVersion = UnitOfWork.Repository<CostCFVersionRepo>()
        ...
        {
            var query = this.UnitOfWork.GetSession().QueryOver<T_BP_COST_CF_HISTORY>();
        ...
        public override void GetHistory(string orgCode, string templateId, int? year)
        {
            if (year.HasValue)
            {
                this.ObjListHistory = UnitOfWork.Repository<CostCFHistoryRepo>().GetManyByExpression(
        ...
        public override void ValidateData(DataTable dataTable, bool isDataBase)
        {
            ...
            var lstDetailTemplate = this.UnitOfWork.Repository<TemplateDetailCostCFRepo>().GetManyByExpression(x => x.TIME_YEAR == this.ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == this.ObjDetail.TEMPLATE_CODE);
        ...
        public override void ImportExcel(HttpRequestBase request)
        {
            ...
            var dataCurrent = new List<T_BP_COST_CF_DATA>();
            if (costCFCurrent != null)
            {
                dataCurrent = UnitOfWork.Repository<CostCFDataRepo>().Queryable().Where(x => x.ORG_CODE == orgCode
            ...
            try
            {
                ...
                else
                {
                    ...
                    CurrentRepository.Create(new T_BP_COST_CF()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                        TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                UnitOfWork.Repository<CostCFVersionRepo>().Create(new T_BP_COST_CF_VERSION()
                {
                    PKID = Guid.NewGuid().ToString(),
                    ORG_CODE = orgCode,
                    TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                {
                    var costDataHis = (T_BP_COST_CF_DATA_HISTORY)item;
                    UnitOfWork.Repository<CostCFDataHistoryRepo>().Create(costDataHis);
                    UnitOfWork.Repository<CostCFDataRepo>().Delete(item);
                }
                ...
                for (int i = this.StartRowData; i < actualRows; i++)
                {
                    ...
                    int j = 4;
                    var costData = new T_BP_COST_CF_DATA()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                        ...
                        STATUS = Approve_Status.ChuaTrinhDuyet,
                        VERSION = versionNext,
                        COST_CF_ELEMENT_CODE = tableData.Rows[i][2].ToString().Trim(),
                    ...
                    UnitOfWork.Repository<CostCFDataRepo>().Create(costData);
                }
                UnitOfWork.Commit();
                NotifyUtilities.CreateNotify(
                    new NotifyPara()
        ...
        public override void ImportExcelBase(HttpRequestBase request)
        {
            ...
            this.StartRowData = 6;
            var orgCode = ProfileUtilities.User?.ORGANIZE_CODE;
            var costCFDataRepo = UnitOfWork.Repository<CostCFDataRepo>();
            var costCFDataBaseRepo = UnitOfWork.Repository<CostCFDataBaseRepo>();
            var currentUser = ProfileUtilities.User?.USER_NAME;
            ...
            var costCFCurrent = UnitOfWork.Repository<CostCFRepo>().Queryable().Where(x => x.ORG_CODE == orgCode
            ...
            // Lấy dữ liệu của version hiện tại
            var dataCurrent = new List<T_BP_COST_CF_DATA>();
            var dataCurrentBase = new List<T_BP_COST_CF_DATA_BASE>();
            if (costCFCurrent != null)
            {
                dataCurrent = costCFDataRepo.GetManyWithFetch(x => x.ORG_CODE == orgCode
                    && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE).ToList();

                dataCurrentBase = costCFDataBaseRepo.GetManyWithFetch(x => x.ORG_CODE == orgCode
                    && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE).ToList();
            }
            ...
            try
            {
                ...
                foreach (var key in lookUp.Select(x => x.Key))
                {
                    ...
                    costData.VALUE_SUM_YEAR_PREVENTIVE = costData.VALUE_SUM_YEAR * percentagePreventive;
                    costCFDataRepo.Create(costData);
                }

                // Insert data vào base data history
                var lstBaseDataHistory = (from x in dataCurrentBase
                                          select (T_BP_COST_CF_DATA_BASE_HISTORY)x).ToList();
                UnitOfWork.Repository<CostCFDataBaseHistoryRepo>().Create(lstObj: lstBaseDataHistory);
                costCFDataBaseRepo.Delete(dataCurrentBase);
                ...
                for (int i = this.StartRowData; i < actualRows; i++)
                {
                    ...
                    int j = 6;
                    var costData = new T_BP_COST_CF_DATA_BASE()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                        COST_CENTER_CODE = centerCode,
                    ...
                    costCFDataBaseRepo.Create(costData);
                }
                UnitOfWork.Commit();
        ...
        public override IList<T_BP_COST_CF_VERSION> GetVersions(string orgCode, string templateId, int year)
        {
        ...
        {
            return UnitOfWork.Repository<CostCFDataRepo>()
        ...
        {
            return UnitOfWork.Repository<CostCFDataHistoryRepo>()
                .GetManyByExpression(x => lstOrgCodes.Contains(x.COST_CENTER_CODE) && !lstOrgCodes.Contains(x.ORG_CODE) && x.TEMPLATE_CODE != string.Empty && (!year.HasValue || x.TIME_YEAR == year.Value))
                .Select(x => x.TEMPLATE_CODE)
                .Distinct()
                .ToList();
        ...
        public override IList<int> GetTemplateVersion(string templateId, string centerCode, int year)
        {
            if (string.IsNullOrEmpty(templateId))
            {
                return UnitOfWork.Repository<CostCFSumUpDetailRepo>()
        ...
        {
            var dataCost = PreparePureList(out IList<T_MD_TEMPLATE_DETAIL_COST_CF> detailCostElements, templateId, year, ignoreAuth: true);
            ...
            try
            {
                ...
                templateWorkbook.SetSheetName(0, ModulType.GetTextSheetName(ModulType.CostCF));
        ...
        public override IList<NodeDataFlow> BuildDataFlowTree(string orgCode, int year, int? version, int? sumUpVersion)
        {
            ...
            if (costCenterRepo.GetManyByExpression(x => x.PARENT_CODE == orgCode).Count > 0 && version == null)
            {
                ...
                {
                    var costCFData = UnitOfWork.Repository<CostCFSumUpDetailRepo>()
            ...
            else
            {
                // level 2: department
                // get templates
                var sumupDetails = UnitOfWork.Repository<CostCFSumUpDetailRepo>()
        ...
        /// <returns></returns>
        private (IList<T_BP_COST_CF_DATA>, bool) GetDataSumUp(int year, string centerCode, string elementCode, int sumUpVersion)
        {
            var plDataRepo = UnitOfWork.Repository<CostCFDataRepo>();
            var costPlReviewCommentRepo = UnitOfWork.Repository<CostCFReviewCommentRepo>();
            var plVersionRepo = UnitOfWork.Repository<CostCFRepo>();
            var plDataHistoryRepo = UnitOfWork.Repository<CostCFDataHistoryRepo>();

            var lstDetails = UnitOfWork.Repository<CostCFSumUpDetailRepo>()
            ...
            var data = new List<T_BP_COST_CF_DATA>();
            foreach (var key in lookup.Select(x => x.Key))
            {
                var costPL = new List<T_BP_COST_CF_DATA>();
        ...
        public override IEnumerable<T_MD_COST_CF_ELEMENT> GetDetailSumUp(string centerCode, string elementCode, int year, int version, int? sumUpVersion, bool isCountComments, bool? isShowFile = null)
        {
            ...
            if (IsLeaf(centerCode))
            {
                ...
                foreach (var key in lookupData.Select(x => x.Key))
                {
                    ...
                    {
                        var isNewestVersion = UnitOfWork.Repository<CostCFVersionRepo>()
                        ...
                        {
                            // get data from base
                            var baseData = UnitOfWork.Repository<CostCFDataBaseRepo>()
                                .GetManyWithFetch(x => x.COST_CF_ELEMENT_CODE == elementCode && x.COST_CENTER_CODE == centerCode && x.TEMPLATE_CODE == key && x.VERSION == lookupData[key].First().VERSION && x.TIME_YEAR == year);
                        ...
                        else
                        {
                            ...
                            var baseDataHistory = UnitOfWork.Repository<CostCFDataBaseHistoryRepo>()
                                .GetManyWithFetch(x => x.COST_CF_ELEMENT_CODE == elementCode && x.COST_CENTER_CODE == centerCode && x.TEMPLATE_CODE == key && x.VERSION == lookupData[key].First().VERSION && x.TIME_YEAR == year);
            ...
            {
                var comments = new List<T_BP_COST_CF_REVIEW_COMMENT>();
                if (isCountComments)
                {
        ...
        public override IEnumerable<T_MD_COST_CF_ELEMENT> GetDetailSumUpTemplate(string elementCode, int year, int version, string templateCode, string centerCode)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    var baseData = UnitOfWork.Repository<CostCFDataBaseHistoryRepo>()
                        .GetManyWithFetch(x => x.TEMPLATE_CODE == templateCode
                        && x.TIME_YEAR == year
                        && x.COST_CF_ELEMENT_CODE == elementCode
                        && x.VERSION == version
        ...
        public IList<T_MD_COST_CF_ELEMENT> PreparePureList(IList<T_MD_TEMPLATE_DETAIL_COST_CF> detailCostElements, int year)
        {
        ...
        /// <param name="centerCode"></param>
        /// <returns></returns>
        public IList<T_MD_COST_CF_ELEMENT> PreparePureList(out IList<T_MD_TEMPLATE_DETAIL_COST_CF> detailCostElements,
            string templateId,
            int year,
            string centerCode = "",
            bool ignoreAuth = false)
        {
            ...
            {
                detailCostElements = UnitOfWork.Repository<TemplateDetailCostCFRepo>()
        ...
        /// <param name="templateId"></param>
        /// <returns></returns>
        public IList<T_MD_COST_CF_ELEMENT> PreparePureListForTemplate(out IList<T_MD_TEMPLATE_DETAIL_COST_CF> detailCostElements,
            string templateId,
            int year)
        {
        ...
        {
            // Tìm mẫu nộp hộ
            var listTemplateCodes = this.UnitOfWork.Repository<TemplateDetailCostCFRepo>()
                .GetManyByExpression(x => centerCodes.Contains(x.CENTER_CODE) && x.TIME_YEAR == year)
                .Select(x => x.TEMPLATE_CODE).Distinct().ToList();
            var findCostCF = this.CurrentRepository.GetManyByExpression(
                    x => listTemplateCodes.Contains(x.TEMPLATE_CODE) && !centerCodes.Contains(x.ORG_CODE));
            var lst = new List<T_MD_TEMPLATE_DETAIL_COST_CF>();
            detailCostElements = new List<T_MD_TEMPLATE_DETAIL_COST_CF>();
            foreach (var template in listTemplateCodes)
            {
                lst.AddRange(UnitOfWork.Repository<TemplateDetailCostCFRepo>()
        ...
        public override void SumUpDataCenter(out T_BP_COST_CF_VERSION costCF, string centerCode, int year)
        {
            ...
            var lstData = new List<T_BP_COST_CF_DATA>();
            costCF = new T_BP_COST_CF_VERSION();

            try
            {
                UnitOfWork.BeginTransaction();
                var sumUpDetailRepo = UnitOfWork.Repository<CostCFSumUpDetailRepo>();
                var costCFDataRepo = UnitOfWork.Repository<CostCFDataRepo>();
                ...
                var CostCFDataApproved = costCFDataRepo.GetManyByExpression(x => (x.COST_CENTER_CODE == centerCode && !string.IsNullOrEmpty(x.TEMPLATE_CODE)) ||
                ...
                foreach (var code in lookup.Select(x => x.Key))
                {
                    ...
                    {
                        lstData.Add((T_BP_COST_CF_DATA)lookup[code].First().CloneObject());
                    }
                    else
                    {
                        lstData.Add(new T_BP_COST_CF_DATA
                        {
                ...
                var newestCF = UnitOfWork.Repository<CostCFRepo>()
                ...
                UnitOfWork.Repository<CostCFVersionRepo>().Create(costCF);
                ...
                _ = UnitOfWork.GetSession().CreateSQLQuery($"DELETE FROM T_BP_COST_CF_DATA WHERE COST_CENTER_CODE = '{centerCode}' AND TIME_YEAR = {year}")
                    .ExecuteUpdate();

                // insert to pl data history
                UnitOfWork.Repository<CostCFDataHistoryRepo>().Create((from pl in lstCostCFDataOldVersion
                                                                       select (T_BP_COST_CF_DATA_HISTORY)pl).ToList());
                ...
                var lstSumupDetails = CostCFDataApproved.ToLookup(x => new { OrgCode = x.ORG_CODE, TemplateCode = x.TEMPLATE_CODE, TemplateVersion = x.VERSION });
                // create list sum up detail
                sumUpDetailRepo.Create((from c in lstSumupDetails.Select(x => x.Key)
                                        select new T_BP_COST_CF_SUM_UP_DETAIL
                                        {
                                            PKID = Guid.NewGuid().ToString(),
            ...
            string templateId = "")
        {
            ...
            plDataCostElements = UnitOfWork.Repository<CostCFDataRepo>()
        ...
        private IEnumerable<T_MD_COST_CF_ELEMENT> GetSumDescendants(
            IEnumerable<T_MD_TEMPLATE_DETAIL_COST_CF> details,
            ...
            foreach (var centerCode in lookupCenter.Select(x => x.Key))
            {
                ...
                while (st.Count > 0)
                {
                    ...
                    foreach (var i in lookup[item.CODE])
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (detail != null)
                                {
                                    detail.CFData = costCFData.FirstOrDefault(x => x.COST_CF_ELEMENT_CODE == detail.ELEMENT_CODE && x.COST_CENTER_CODE == detail.CENTER_CODE);
                                }
                    ...
                    if (shouldReturn)
                    {
                        ...
                        {
                            var data = costCFData.Where(x => x.COST_CF_ELEMENT_CODE == item.CODE && x.COST_CENTER_CODE == item.CENTER_CODE);
                            if (data != null)
                            {
        ...
        private IList<T_BP_COST_CF_DATA> GetVersionData(
            string templateCode,
            string centerCode,
            int? year = null,
            int? version = null)
        {
            ...
            {
                return UnitOfWork.Repository<CostCFDataRepo>().GetCFDataByOrgCode(orgCode, year.Value, templateCode, version);
            }
            else if (lstChildren.Contains(template.ORG_CODE))
            {
                return UnitOfWork.Repository<CostCFDataRepo>().GetCFDataByOrgCode(template.ORG_CODE, year.Value, templateCode, version);
            }
            else
            {
                return UnitOfWork.Repository<CostCFDataRepo>().GetCFDataByCenterCode(template.ORG_CODE, new List<string> { centerCode }, year.Value, templateCode, version);
                // get 
            }
        }
        ...
        public IList<T_MD_COST_CF_ELEMENT> SummaryCenterOut(out IList<T_BP_COST_CF_DATA> plDataCostElements,
                                                              string centerCode,
                                                              int year,
                                                              int? version,
                                                              bool? isHasValue = null)
        ...
        /// <returns></returns>
        public IList<T_MD_COST_CF_ELEMENT> SummaryCenterVersion(out IList<T_BP_COST_CF_DATA> plDataCostElements,
                                                              string centerCode,
                                                              int year,
                                                              int? version,
        ...
        /// <returns></returns>
        private IList<T_BP_COST_CF_DATA> GetAllSumUpDetails(string centerCode, int year, int version)
        {
            var detailsSumUp = UnitOfWork.Repository<CostCFSumUpDetailRepo>()
               .GetManyWithFetch(x => x.ORG_CODE == centerCode && x.TIME_YEAR == year && x.SUM_UP_VERSION == version);

            var lstChildren = GetListOfChildrenCenter(centerCode).Select(x => x.CODE);
            var lstDetails = new List<T_BP_COST_CF_DATA>();
            var lstResult = new List<T_BP_COST_CF_DATA>();
            var cfDataRepo = UnitOfWork.Repository<CostCFDataRepo>();
                ...
                else
                {
                    ...
                    foreach (var orgCode in lookupCenter.Select(x => x.Key))
                    {
                        ...
                        else
                        {
                            ...
                            lstResult.Add(new T_BP_COST_CF_DATA
                            {
                                ORG_CODE = orgCode,
        ...
        private IList<T_MD_COST_CF_ELEMENT> SummaryCenter(IList<T_BP_COST_CF_DATA> plDataCostElements, string centerCode, int year,
                                                          bool? isHasValue = null)
        {
        ...
        private IEnumerable<T_MD_COST_CF_ELEMENT> GetSumDescendants(
            IList<T_BP_COST_CF_DATA> costCFData,
        ...
        public override void GenerateExportExcel(ref MemoryStream outFileStream, string html, string path, int year, string centerCode, int? version, string templateId, string unit, decimal exchangeRate)
        {
            ...
            workbook.SetSheetName(0, ModulType.GetTextSheetName(ModulType.CostCF));
