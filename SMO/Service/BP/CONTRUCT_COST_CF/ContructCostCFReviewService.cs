...
{
    public class ContructCostCFReviewService : BaseBPReviewService<T_BP_CONTRUCT_COST_CF_REVIEW, T_BP_CONTRUCT_COST_CF_REVIEW_RESULT, ContructCostCFReviewRepo>
    {
        /// <summary>
        /// List lịch sử thẩm định và tổng kiểm soát
        /// </summary>
        public IList<T_BP_CONTRUCT_COST_CF_HISTORY> ListHistory { get; set; }
        /// <summary>
        /// List tổng kiểm soát dữ liệu
        /// </summary>
        public IList<T_BP_CONTRUCT_COST_CF_REVIEW> ListControl { get; set; }

        public ContructCostCFReviewService()
        {
            ListHistory = new List<T_BP_CONTRUCT_COST_CF_HISTORY>();
            ListControl = new List<T_BP_CONTRUCT_COST_CF_REVIEW>();
        }
        ...
        /// </summary>
        /// <returns></returns>
        internal IList<T_MD_COST_CF_ELEMENT> GetLastReview()
        {
        ...
        /// </summary>
        /// <returns></returns>
        internal IList<T_MD_COST_CF_ELEMENT> GetLastReviewSummary()
        {
            var header = GetNewestByExpression(x =>
            x.ORG_CODE == ObjDetail.ORG_CODE &&
            x.TIME_YEAR == ObjDetail.TIME_YEAR &&
        ...
        internal ReviewCenterViewModelBase<ContructCostCFElementReviewCenter> SummaryReviewDataCenter(ReviewDataCenterModel model)
        {
            var corp = GetCorp()?.CODE;

            var dataCost = SummaryCenterVersion(out IList<T_BP_CONTRUCT_COST_CF_DATA> detailCostElements, model, corp);
            dataCost = dataCost.OrderBy(x => x.C_ORDER)
                                            .GroupBy(x => x.CODE)
                                            .Select(x => x.First()).ToList();

            return new ReviewCenterViewModelBase<ContructCostCFElementReviewCenter>
            {
        ...
        private IList<ContructCostCFElementReviewCenter> PrepareListReviewSummary(IList<T_MD_COST_CF_ELEMENT> dataCost, ReviewDataCenterModel model, string corp)
        {
            if (!model.IS_COMPLETED && !model.IS_NOT_COMPLETED)
            {
                return new List<ContructCostCFElementReviewCenter>();
            ...
            var comments = UnitOfWork.Repository<ContructCostCFReviewCommentRepo>()
            ...
            if (lstReviews == null || lstReviews.Count == 0)
            {
                ...
                else
                {
                    return (from d in dataCost
                            let result = objReview?.Results.FirstOrDefault(x => x.COST_CF_ELEMENT_CODE.Equals(d.CODE))
                            let status = result?.RESULT
                            let elementComments = comments.Where(x => x.COST_CF_ELEMENT_CODE == d.CODE)
                            let elementCommentsInOrg = elementComments.Where(x => x.ON_ORG_CODE == d.ORG_CODE)
                            select new ContructCostCFElementReviewCenter(
                                d,
                                null,
                                null,
                                null,
            ...
            var lstReviewResult = UnitOfWork.Repository<ContructCostCFReviewResultRepo>()
                .GetManyByExpression(x => lstReviews.Any(y => y.PKID.Equals(x.HEADER_ID)));

            var lookupReviewByElements = lstReviewResult.ToLookup(x => x.COST_CF_ELEMENT_CODE);
        ...
        internal T_BP_CONTRUCT_COST_CF GetHeader()
        {
            return new ContructCostCFService().GetBPHeader(string.Empty, ObjDetail.DATA_VERSION, ObjDetail.TIME_YEAR, GetCorp().CODE);
        }
        ...
        internal void SearchReview()
        {
            ...
            // Dữ liệu thẩm định
            ObjList = UnitOfWork.Repository<ContructCostCFReviewRepo>().GetManyWithFetch(x => x.TIME_YEAR == ObjDetail.TIME_YEAR
        ...
        internal IList<T_BP_CONTRUCT_COST_CF_HISTORY> GetHistory(int year, IEnumerable<string> lstActionsFilter, int? version = null)
        {
            var corp = GetCorp().CODE;
            if (version == null)
            {
                return UnitOfWork.Repository<ContructCostCFHistoryRepo>()
        ...
        /// <returns></returns>
        private T_BP_CONTRUCT_COST_CF_REVIEW GetControlReview(ReviewDataCenterModel model, string org)
        {
            T_BP_CONTRUCT_COST_CF_REVIEW objReview = null;
            if (model.IS_CONTROL)
            {
        ...
        /// <param name="org"></param>
        /// <returns></returns>
        private IList<T_BP_CONTRUCT_COST_CF_REVIEW> GetCouncils(ReviewDataCenterModel model, string org)
        {
            IList<T_BP_CONTRUCT_COST_CF_REVIEW> lstReviews = null;
        ...
        /// <summary>
        /// Cập nhật dữ liệu thẩm định hoặc tổng kiểm soát
        /// </summary>
        /// <param name="model"></param>
        internal void UpdateReview(ContructCostCFReviewViewModel model)
        {
            // check budget period
            if (model.IsSummary)
            {
            ...
            if (resultCheckConstraint)
            {
                try
                {
                    ...
                    T_BP_CONTRUCT_COST_CF_REVIEW headerFound;
                    ...
                    if (headerFound == null)
                    {
                        ...
                        ObjDetail = new T_BP_CONTRUCT_COST_CF_REVIEW
                        {
                            PKID = headerId,
                    ...
                    else
                    {
                        ...
                        if (model.Status.HasValue)
                        {
                            ...
                            UnitOfWork.Repository<ContructCostCFReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId) && x.RESULT == model.Status.Value);
                        }
                        else
                        {
                            UnitOfWork.Repository<ContructCostCFReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId));
                        }
                    }

                    // create history
                    var history = new T_BP_CONTRUCT_COST_CF_REVIEW_HISTORY
                    {
                    ...
                    UnitOfWork.Repository<ContructCostCFReviewHistoryRepo>().Create(history);

                    // create history
                    var historyCF = new T_BP_CONTRUCT_COST_CF_HISTORY
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = model.OrgCode,
                        TIME_YEAR = model.Year,
                    ...
                    UnitOfWork.Repository<ContructCostCFHistoryRepo>().Create(historyCF);
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private bool CheckConstraintComments(ContructCostCFReviewViewModel model, out IDictionary<string, string> elementsNotValid)
        {
            ...
            var queryable = UnitOfWork.GetSession().Query<T_BP_CONTRUCT_COST_CF_REVIEW_COMMENT>();
            ...
            queryable = queryable.Where(x => elementsNotSuccess.Contains(x.COST_CF_ELEMENT_CODE));
            queryable = queryable.FetchMany(x => x.Comments);

            var lookupComments = queryable.ToList().ToLookup(x => x.COST_CF_ELEMENT_CODE);
            var allElements = GetAllMasterData<CostCFElementRepo, T_MD_COST_CF_ELEMENT>().Where(x => x.TIME_YEAR == model.Year);
        ...
        {
            var corp = GetCorp()?.CODE;
            return UnitOfWork.Repository<ContructCostCFVersionRepo>().GetManyByExpression(x => x.TIME_YEAR == year && x.ORG_CODE == corp)
                .Select(x => x.VERSION)
                .Distinct()
        ...
        internal IList<T_MD_COST_CF_ELEMENT> SummaryCenterVersion(out IList<T_BP_CONTRUCT_COST_CF_DATA> detailCostElements, int? version = null)
        {
            return new ContructCostCFService().SummaryCenterVersion(out detailCostElements, ObjDetail.ORG_CODE, ObjDetail.TIME_YEAR, version);
        }

        internal IList<T_MD_COST_CF_ELEMENT> SummaryCenterVersion(out IList<T_BP_CONTRUCT_COST_CF_DATA> detailCostElements, ReviewDataCenterModel model, string corp)
        {
            return new ContructCostCFService().SummaryCenterVersion(out detailCostElements, corp, model.YEAR, model.VERSION);
        }

        internal IList<ContructCostCFElementReview> PrepareListReview(IList<T_MD_COST_CF_ELEMENT> dataCost)
        {
            var comments = UnitOfWork.Repository<ContructCostCFReviewCommentRepo>()
                    .GetManyByExpression(x =>
                    x.ORG_CODE == ObjDetail.ORG_CODE &&
            ...
            var objReview = new T_BP_CONTRUCT_COST_CF_REVIEW();
            if (!IsReview)
            {
                objReview = GetFirstByExpression(
            ...
            return (from d in dataCost
                    ...
                    let elementComments = comments.Where(x => x.COST_CF_ELEMENT_CODE == d.CODE)
                    let elementCommentsInOrg = elementComments.Where(x => x.ON_ORG_CODE == d.ORG_CODE)
                    select new ContructCostCFElementReview(
                        d,
                        result?.RESULT,
                        elementComments.Sum(x => x.NUMBER_COMMENTS),
                        elementCommentsInOrg.Sum(x => x.NUMBER_COMMENTS)
