...
using SMO.Repository.Implement.BP.CONTRUCT_COST_CF.CONTRUCT_COST_CF_DATA_BASE;
using SMO.Repository.Implement.BP.COST_CF;
using SMO.Repository.Implement.MD;
using SMO.Service.BP.COST_CF;
using SMO.Service.Class;
using SMO.Service.Common;
using SMO.ServiceInterface.BP.ContructCostCF;
...
{
    public class ContructCostCFService : BaseBPService<T_BP_CONTRUCT_COST_CF, ContructCostCFRepo, T_MD_COST_CF_ELEMENT, T_BP_CONTRUCT_COST_CF_VERSION, T_BP_CONTRUCT_COST_CF_HISTORY, ContructCostCFHistoryRepo>, IContructCostCFService
    {
        ...
        public List<T_BP_CONTRUCT_COST_CF_HISTORY> ObjListHistory { get; set; }
        public List<T_BP_CONTRUCT_COST_CF_VERSION> ObjListVersion { get; set; }
        public List<T_BP_CONTRUCT_COST_CF_SUM_UP_DETAIL> ObjListSumUpHistory { get; set; }
        public ContructCostCFService()
        {
            this.StartRowData = 6;
            ...
            this.ObjListHistory = new List<T_BP_CONTRUCT_COST_CF_HISTORY>();
            this.ObjListVersion = new List<T_BP_CONTRUCT_COST_CF_VERSION>();
            this.ObjListSumUpHistory = new List<T_BP_CONTRUCT_COST_CF_SUM_UP_DETAIL>();
        }
        ...
        public override bool ShowReviewBtn(int year)
        {
            ...
            else
            {
                ...
                var historyReview = UnitOfWork.Repository<ContructCostCFReviewRepo>()
        ...
        public override void GetListOfChild()
        {
            ...
            var lstChildOrg = GetListOfChildrenCenter(orgCode).Select(x => x.CODE).ToList();
            ...
            // Tìm mẫu nộp hộ
            var listTemplateCode = this.UnitOfWork.Repository<TemplateDetailContructCostCFRepo>().Queryable().Where(
        ...
        {
            var query = UnitOfWork.GetSession().QueryOver<T_BP_CONTRUCT_COST_CF_SUM_UP_DETAIL>();
        ...
        public IList<T_MD_COST_CF_ELEMENT> GetDataCFCost(out IList<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_CF> detailCostElements,
            out IList<T_BP_CONTRUCT_COST_CF_DATA> detailCostData,
        ...
        {
            var plDataCostElements = UnitOfWork.Repository<ContructCostCFDataRepo>()
                .GetCFDataByCenterCode(null, new List<string> { centerCode }, year, null, null);

            return plDataCostElements.Where(x => x.COST_CF_ELEMENT_CODE == elementCode)
                .Select(x => (T_MD_COST_CF_ELEMENT)x)
                .OrderBy(x => x.C_ORDER).ToList();
        }

        private IList<T_MD_COST_CF_ELEMENT> GetDataCostPreview(out IList<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_CF> detailCostElements, string templateId, List<string> lstOrgs, int year, int? version, bool? isHasValue)
        {
            detailCostElements = new List<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_CF>();
            var lstElements = new List<T_MD_COST_CF_ELEMENT>();
            foreach (var orgCode in lstOrgs)
            {
                var elements = GetDataCostPreview(out IList<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_CF> detail, templateId, orgCode, year, version, isHasValue);
        ...
        public override T_BP_CONTRUCT_COST_CF_VERSION GetHeader(string templateCode, string centerCode, int year, int? version)
        {
            templateCode = templateCode ?? string.Empty;
            if (version == null)
            {
                return UnitOfWork.Repository<ContructCostCFVersionRepo>()
        ...
        {
            ObjListVersion = UnitOfWork.Repository<ContructCostCFVersionRepo>()
        ...
        {
            var query = this.UnitOfWork.GetSession().QueryOver<T_BP_CONTRUCT_COST_CF_HISTORY>();
            query = query.Where(x => x.ORG_CODE == orgCode && x.TIME_YEAR == year)
                .Fetch(x => x.Template).Eager
                .Fetch(x => x.USER_CREATE).Eager;
            ObjListHistory = query.List().ToList();
        }
        ...
        private void SendNotify(Activity activity)
        {
            NotifyUtilities.CreateNotify(
                    new NotifyPara()
                    {
                        ...
                        ModulType = ModulType.ContructCostCF,
        ...
        public override void TrinhDuyet(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<ContructCostCFHistoryRepo>()
                    .Create(new T_BP_CONTRUCT_COST_CF_HISTORY()
                    {
                ...
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_CONTRUCT_COST_CF_DATA SET STATUS = '{Approve_Status.ChoPheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void HuyTrinhDuyet(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<ContructCostCFHistoryRepo>().Create(new T_BP_CONTRUCT_COST_CF_HISTORY()
                {
                    PKID = Guid.NewGuid().ToString(),
                    ORG_CODE = ObjDetail.ORG_CODE,
                    TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_CONTRUCT_COST_CF_DATA SET STATUS = '{Approve_Status.ChuaTrinhDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void PheDuyet(string code)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_CONTRUCT_COST_CF_DATA SET STATUS = '{Approve_Status.DaPheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void HuyNop(string code)
        {
            ...
            try
            {
                ...
                UnitOfWork.Repository<ContructCostCFVersionRepo>().Update(x => x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.VERSION == ObjDetail.VERSION, x => x.IS_DELETED = 1, x => x.UPDATE_BY = currentUser);
        ...
        public override void TuChoi(string code)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_CONTRUCT_COST_CF_DATA SET STATUS = '{Approve_Status.TuChoi}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void ChuyenTKS(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<ContructCostCFReviewRepo>()
        ...
        public override void KetThucThamDinh(string orgCode, int year, int version)
        {
            try
            {
                ...
                UnitOfWork.BeginTransaction();
                var currentUser = ProfileUtilities.User?.USER_NAME;
                var reviewHeader = UnitOfWork.Repository<ContructCostCFReviewRepo>()
                ...
                if (reviewHeader != null)
                {
                    ...
                    UnitOfWork.Repository<ContructCostCFReviewRepo>().Update(reviewHeader);
                }
                ...
                var totalUsersEndReview = UnitOfWork.Repository<ContructCostCFReviewRepo>()
                ...
                var review = UnitOfWork.Repository<ContructCostCFReviewRepo>()
                ...
                if (review == null)
                {
                    ...
                    UnitOfWork.Repository<ContructCostCFReviewRepo>().Create(
                        new T_BP_CONTRUCT_COST_CF_REVIEW
                        {
                            PKID = reviewId,
                    ...
                    var plReviewService = new ContructCostCFReviewService();
                    plReviewService.ObjDetail.TIME_YEAR = year;
                    plReviewService.ObjDetail.ORG_CODE = orgCode;
                    var dataCost = plReviewService.SummaryCenterVersion(out IList<T_BP_CONTRUCT_COST_CF_DATA> detailCostElements);
                    dataCost = dataCost.OrderBy(x => x.C_ORDER)
                                                    .GroupBy(x => x.CODE)
                                                    .Select(x => x.First()).ToList();
                    var elements = plReviewService.PrepareListReview(dataCost);
                    UnitOfWork.Repository<ContructCostCFReviewResultRepo>()
                        .Create((from e in elements.Where(x => !x.IS_GROUP)
                                 select new T_BP_CONTRUCT_COST_CF_REVIEW_RESULT
                                 {
                                     PKID = Guid.NewGuid().ToString(),
                ...
                {
                    review.IS_END = true;
                    UnitOfWork.Repository<ContructCostCFReviewRepo>().Update(review);
                }
        ...
        public override void PheDuyetTongKiemSoat(string orgCode, int year, int version)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_CONTRUCT_COST_CF_DATA SET STATUS = '{Approve_Status.TKS_PheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        internal override void TuChoiTongKiemSoat(string orgCode, int year, int version, string comment)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_CONTRUCT_COST_CF_DATA SET STATUS = '{Approve_Status.TKS_TuChoi}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
                UnitOfWork.Repository<ContructCostCFReviewRepo>()
        ...
        public override void TrinhDuyetTongKiemSoat(string orgCode, int year, int version)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_CONTRUCT_COST_CF_DATA SET STATUS = '{Approve_Status.TKS_TrinhDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
                UnitOfWork.Repository<ContructCostCFReviewRepo>()
                    .Update(x => x.ORG_CODE == orgCode
                                && x.TIME_YEAR == year
                                && x.DATA_VERSION == version
        ...
        public override void ChuyenHDNS(string code)
        {
            try
            {
                ...
                {
                    UnitOfWork.Repository<ContructCostCFReviewRepo>().Update(x => x.DATA_VERSION == ObjDetail.VERSION
        ...
        public override void GetHistory(string orgCode, string templateId, int? year)
        {
            if (year.HasValue)
            {
                this.ObjListHistory = UnitOfWork.Repository<ContructCostCFHistoryRepo>().GetManyByExpression(
        ...
        public override void ValidateData(DataTable dataTable, bool isDataBase)
        {
            ...
            var lstDetailTemplate = this.UnitOfWork.Repository<TemplateDetailContructCostCFRepo>().GetManyByExpression(x => x.TIME_YEAR == this.ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == this.ObjDetail.TEMPLATE_CODE);
            ...
            {
                var orgCodeInExcel = dataTable.Rows[i][8].ToString().Trim();
                var elementCodeInExcel = dataTable.Rows[i][10].ToString().Trim();
            ...
            if (isDataBase)
            {
                ...
                this.ConvertData(dataTable, lstElement.ToList(), startColumn: 14, endColumn: ListColumnNameDataBase.Count - 5, isDataBase);
            }
            else
            {
                ...
                for (int i = StartRowData; i < dataTable.Rows.Count; i++)
                {
                    var foundItem = lstDetailTemplate
                        .FirstOrDefault(x => x.CENTER_CODE == dataTable.Rows[i][8].ToString().Trim() &&
                        x.ELEMENT_CODE == dataTable.Rows[i][10].ToString().Trim());
                ...
                this.ConvertData(dataTable, lstElement.ToList(), startColumn: 12, endColumn: ListColumnName.Count - 3, isDataBase);
            }
        ...
        public override void ImportExcel(HttpRequestBase request)
        {
            ...
            var costCFCurrent = CurrentRepository.Queryable().FirstOrDefault(x => x.ORG_CODE == orgCode
                && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE);

            if (costCFCurrent != null && !(costCFCurrent.STATUS == Approve_Status.TuChoi || costCFCurrent.STATUS == Approve_Status.ChuaTrinhDuyet))
            {
                this.State = false;
            ...
            var versionNext = 1;
            if (costCFCurrent != null)
            {
                versionNext = costCFCurrent.VERSION + 1;
            }

            // Lấy dữ liệu của version hiện tại
            var dataCurrent = new List<T_BP_CONTRUCT_COST_CF_DATA>();
            if (costCFCurrent != null)
            {
                dataCurrent = UnitOfWork.Repository<ContructCostCFDataRepo>().Queryable().Where(x => x.ORG_CODE == orgCode
            ...
            try
            {
                ...
                if (costCFCurrent != null)
                {
                    ...
                    costCFCurrent.VERSION = versionNext;
                    costCFCurrent.IS_DELETED = false;
                    CurrentRepository.Update(costCFCurrent);
                }
                else
                {
                    // Tạo mới bản ghi cost pl
                    CurrentRepository.Create(new T_BP_CONTRUCT_COST_CF()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                        TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                UnitOfWork.Repository<ContructCostCFVersionRepo>().Create(new T_BP_CONTRUCT_COST_CF_VERSION()
                {
                    PKID = Guid.NewGuid().ToString(),
                    ORG_CODE = orgCode,
                    TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                {
                    var costDataHis = (T_BP_CONTRUCT_COST_CF_DATA_HISTORY)item;
                    UnitOfWork.Repository<ContructCostCFDataHistoryRepo>().Create(costDataHis);
                    UnitOfWork.Repository<ContructCostCFDataRepo>().Delete(item);
                }
                ...
                for (int i = this.StartRowData; i < actualRows; i++)
                {
                    ...
                    var costData = new T_BP_CONTRUCT_COST_CF_DATA()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                        INTERNAL_ORDER_CODE = tableData.Rows[i][8].ToString().Trim(),
                        TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                        TIME_YEAR = ObjDetail.TIME_YEAR,
                        VERSION = versionNext,
                        STATUS = Approve_Status.ChuaTrinhDuyet,
                        COST_CF_ELEMENT_CODE = tableData.Rows[i][10].ToString().Trim(),
                        VALUE_JAN = tableData.Rows[i][12] as decimal?,
                        VALUE_FEB = tableData.Rows[i][13] as decimal?,
                        VALUE_MAR = tableData.Rows[i][14] as decimal?,
                        VALUE_APR = tableData.Rows[i][15] as decimal?,
                        VALUE_MAY = tableData.Rows[i][16] as decimal?,
                        VALUE_JUN = tableData.Rows[i][17] as decimal?,
                        VALUE_JUL = tableData.Rows[i][18] as decimal?,
                        VALUE_AUG = tableData.Rows[i][19] as decimal?,
                        VALUE_SEP = tableData.Rows[i][20] as decimal?,
                        VALUE_OCT = tableData.Rows[i][21] as decimal?,
                        VALUE_NOV = tableData.Rows[i][22] as decimal?,
                        VALUE_DEC = tableData.Rows[i][23] as decimal?,
                        DESCRIPTION = tableData.Rows[i][26].ToString(),
                        CREATE_BY = currentUser
                    };
                    ...
                    UnitOfWork.Repository<ContructCostCFDataRepo>().Create(costData);
                }
                UnitOfWork.Commit();
                NotifyUtilities.CreateNotify(
                    new NotifyPara()
        ...
        public override void ImportExcelBase(HttpRequestBase request)
        {
            ...
            var orgCode = ProfileUtilities.User?.ORGANIZE_CODE;
            var costPLDataRepo = UnitOfWork.Repository<ContructCostCFDataRepo>();
            var costPLDataBaseRepo = UnitOfWork.Repository<ContructCostCFDataBaseRepo>();
            var currentUser = ProfileUtilities.User?.USER_NAME;
            ...
            // Xác định version dữ liệu
            var costCFCurrent = GetFirstWithFetch(x => x.ORG_CODE == orgCode
                && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE);

            if (false && costCFCurrent != null && !(costCFCurrent.STATUS == Approve_Status.TuChoi || costCFCurrent.STATUS == Approve_Status.ChuaTrinhDuyet))
            {
                this.State = false;
                this.ErrorMessage = "Mẫu khai báo này không ở trạng thái Từ chối hoặc Chưa trình duyệt!";
                return;
            ...
            // Lấy dữ liệu của version hiện tại
            var dataCurrent = new List<T_BP_CONTRUCT_COST_CF_DATA>();
            var dataCurrentBase = new List<T_BP_CONTRUCT_COST_CF_DATA_BASE>();
            if (costCFCurrent != null)
            {
            ...
            try
            {
                ...
                new
                {
                    CenterCode = x[8].ToString(),
                    ElementCode = x[10].ToString()
                });
                ...
                foreach (var key in lookUp.Select(x => x.Key))
                {
                    ...
                    var costData = new T_BP_CONTRUCT_COST_CF_DATA()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                        INTERNAL_ORDER_CODE = key.CenterCode,
                        TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                        TIME_YEAR = ObjDetail.TIME_YEAR,
                        VERSION = versionNext,
                        STATUS = Approve_Status.ChuaTrinhDuyet,
                        COST_CF_ELEMENT_CODE = key.ElementCode,
                        VALUE_JAN = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[17].ToString())),
                        VALUE_FEB = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[21].ToString())),
                        VALUE_MAR = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[25].ToString())),
                        VALUE_APR = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[29].ToString())),
                        VALUE_MAY = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[33].ToString())),
                        VALUE_JUN = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[37].ToString())),
                        VALUE_JUL = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[41].ToString())),
                        VALUE_AUG = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[45].ToString())),
                        VALUE_SEP = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[49].ToString())),
                        VALUE_OCT = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[53].ToString())),
                        VALUE_NOV = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[57].ToString())),
                        VALUE_DEC = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[61].ToString())),
                        CREATE_BY = currentUser
                    };
                ...
                var lstBaseDataHistory = (from x in dataCurrentBase
                                          select (T_BP_CONTRUCT_COST_CF_DATA_BASE_HISTORY)x).ToList();
                UnitOfWork.Repository<ContructCostCFDataBaseHistoryRepo>().Create(lstObj: lstBaseDataHistory);
                costPLDataBaseRepo.Delete(dataCurrentBase);

                // Insert dữ liệu vào bảng data
                for (int i = this.StartRowData; i < actualRows; i++)
                {
                    var centerCode = tableData.Rows[i][8].ToString().Trim();

                    var j = 14;
                    var costData = new T_BP_CONTRUCT_COST_CF_DATA_BASE()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                        INTERNAL_ORDER_CODE = centerCode,
                        TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                        TIME_YEAR = ObjDetail.TIME_YEAR,
                        VERSION = versionNext,
                        COST_CF_ELEMENT_CODE = tableData.Rows[i][10].ToString().Trim(),
                        MATERIAL = tableData.Rows[i][12].ToString().Trim(),
                        UNIT = tableData.Rows[i][13].ToString().Trim(),
        ...
        public override IList<T_BP_CONTRUCT_COST_CF_VERSION> GetVersions(string orgCode, string templateId, int year)
        {
        ...
        {
            return UnitOfWork.Repository<ContructCostCFDataRepo>()
        ...
        {
            return UnitOfWork.Repository<ContructCostCFDataHistoryRepo>()
                .GetManyByExpression(x => lstOrgCodes.Contains(x.ORG_CODE) && x.TEMPLATE_CODE != string.Empty && (!year.HasValue || x.TIME_YEAR == year.Value))
                .Select(x => x.TEMPLATE_CODE)
                .Distinct()
                .ToList();
        ...
        private IList<int> GetDataHistoryYear(List<string> lstOrgCodes, string templateId)
        {
            ...
            {
                return UnitOfWork.Repository<ContructCostCFDataHistoryRepo>()
                    .GetManyByExpression(x => lstOrgCodes.Contains(x.INTERNAL_ORDER_CODE))
                    .Select(x => x.TIME_YEAR)
                    .Distinct()
                    .ToList();
            }
            ...
            {
                return UnitOfWork.Repository<ContructCostCFDataHistoryRepo>()
                    .GetManyByExpression(x => lstOrgCodes.Contains(x.INTERNAL_ORDER_CODE) && x.TEMPLATE_CODE == templateId)
                    .Select(x => x.TIME_YEAR)
                    .Distinct()
                    .ToList();
            }
        ...
        public override IList<int> GetTemplateVersion(string templateId, string centerCode, int year)
        {
            if (string.IsNullOrEmpty(templateId))
            {
                return UnitOfWork.Repository<ContructCostCFSumUpDetailRepo>()
        ...
        {
            var dataCost = PreparePureList(out IList<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_CF> detailCostElements, templateId, year, ignoreAuth: true);
            ...
            try
            {
                ...
                templateWorkbook.SetSheetName(0, ModulType.GetTextSheetName(ModulType.ContructCostCF));
                ...
                rowHeader2.Cells[25].SetCellValue(rowHeader2.Cells[25].StringCellValue + $" {year}");
                    ...
                    .OrderByDescending(x => x.CENTER_CODE))
                {
                            ...
                            .Select(x => x.First()))
                    {
                        ...
                        rowCur.Cells[1].SetCellValue(detail.Center.PROJECT_NAME);
                        rowCur.Cells[1].CellStyle = styleCellDetail;

                        rowCur.Cells[2].SetCellValue(detail.Center.BLOCK_CODE);
                        rowCur.Cells[2].CellStyle = styleCellDetail;

                        rowCur.Cells[3].SetCellValue(detail.Center.BLOCK_NAME);
                        rowCur.Cells[3].CellStyle = styleCellDetail;

                        rowCur.Cells[4].SetCellValue(detail.Center.IO_LEVEL1_CODE);
                        rowCur.Cells[4].CellStyle = styleCellDetail;

                        rowCur.Cells[5].SetCellValue(detail.Center.IO_LEVEL1_NAME);
                        rowCur.Cells[5].CellStyle = styleCellDetail;

                        rowCur.Cells[6].SetCellValue(detail.Center.IO_LEVEL2_CODE);
                        rowCur.Cells[6].CellStyle = styleCellDetail;

                        rowCur.Cells[7].SetCellValue(detail.Center.IO_LEVEL2_NAME);
                        rowCur.Cells[7].CellStyle = styleCellDetail;

                        rowCur.Cells[8].SetCellValue(detail.CENTER_CODE);
                        rowCur.Cells[8].CellStyle = styleCellDetail;

                        rowCur.Cells[9].SetCellValue(detail.Center.NAME);
                        rowCur.Cells[9].CellStyle = styleCellDetail;

                        rowCur.Cells[10].SetCellValue(item.CODE);
                        rowCur.Cells[10].CellStyle = styleCellDetail;

                        rowCur.Cells[11].SetCellValue($"{space}{item.NAME}");
                        if (item.IS_GROUP)
                        {
                            rowCur.Cells[11].CellStyle = styleCellBold;
                            rowCur.Cells[11].CellStyle.SetFont(fontBold);
                        }
                        else
                        {
                            rowCur.Cells[11].CellStyle = styleCellDetail;
                        }
                        ...
                        rowCur.Cells[24].SetCellFormula($"SUM(M{numRowCur + 1}:X{numRowCur + 1})");
                        rowCur.Cells[24].CellStyle = styleCellNumberColor;
                        rowCur.Cells[24].SetCellType(CellType.Formula);

                        rowCur.Cells[25].SetCellFormula($"Y{numRowCur + 1}/12");
                        rowCur.Cells[25].CellStyle = styleCellNumberColor;
                        rowCur.Cells[25].SetCellType(CellType.Formula);

                        rowCur.Cells[26].SetCellValue(string.Empty);
                        rowCur.Cells[26].CellStyle = styleCellDetail;
            ...
            try
            {
                    ...
                    .OrderByDescending(x => x.CENTER_CODE))
                {
                            ...
                            .Select(x => x.First()))
                    {
                        ...
                        rowCur.Cells[12].SetCellValue(string.Empty);
                        rowCur.Cells[12].CellStyle = styleCellDetail;

                        rowCur.Cells[13].SetCellValue(string.Empty);
                        rowCur.Cells[13].CellStyle = styleCellDetail;

                        int j = 14;
                        for (; j < NUM_CELL - 1; j++)
                        {
                            rowCur.Cells[j].SetCellValue(string.Empty);
                            if (j < NUM_CELL - 5 && (j - 13) % 4 == 2)
                            {
        ...
        public override IList<NodeDataFlow> BuildDataFlowTree(string orgCode, int year, int? version, int? sumUpVersion)
        {
            ...
            if (costCenterRepo.GetManyByExpression(x => x.PARENT_CODE == orgCode).Count > 0 && version == null)
            {
                ...
                {
                    var costCFData = UnitOfWork.Repository<ContructCostCFSumUpDetailRepo>()
                        .GetManyByExpression(x => x.ORG_CODE == orgCode && x.TIME_YEAR == year);
                    return (from data in costCFData.GroupBy(x => x.SUM_UP_VERSION).Select(x => x.First())
                ...
                else
                {
                    ...
                    return (from data in costCFData.GroupBy(x => x.FROM_ORG_CODE).Select(x => x.First())
            ...
            else
            {
                // level 2: department
                // get templates
                var sumupDetails = UnitOfWork.Repository<ContructCostCFSumUpDetailRepo>()
        ...
        /// <returns></returns>
        private (IList<T_BP_CONTRUCT_COST_CF_DATA>, bool) GetDataSumUp(int year, string centerCode, string elementCode, int sumUpVersion)
        {
            var plDataRepo = UnitOfWork.Repository<ContructCostCFDataRepo>();
            var costPlReviewCommentRepo = UnitOfWork.Repository<ContructCostCFReviewCommentRepo>();
            var plVersionRepo = UnitOfWork.Repository<ContructCostCFRepo>();
            var plDataHistoryRepo = UnitOfWork.Repository<ContructCostCFDataHistoryRepo>();

            var lstDetails = UnitOfWork.Repository<ContructCostCFSumUpDetailRepo>()
            ...
            var data = new List<T_BP_CONTRUCT_COST_CF_DATA>();
            foreach (var key in lookup.Select(x => x.Key))
            {
                var costPL = new List<T_BP_CONTRUCT_COST_CF_DATA>();
        ...
        public override IEnumerable<T_MD_COST_CF_ELEMENT> GetDetailSumUp(string centerCode, string elementCode, int year, int version, int? sumUpVersion, bool isCountComments, bool? isShowFile = null)
        {
            ...
            if (IsLeaf(centerCode))
            {
                ...
                foreach (var key in lookupData.Select(x => x.Key))
                {
                    ...
                    {
                        var isNewestVersion = UnitOfWork.Repository<ContructCostCFVersionRepo>()
                        ...
                        {
                            var lstElements = lookupData[key].Where(x => x.INTERNAL_ORDER_CODE == item.INTERNAL_ORDER_CODE);
                        ...
                        {
                            // get data from base
                            var baseData = UnitOfWork.Repository<ContructCostCFDataBaseRepo>()
                        ...
                        else
                        {
                            ...
                            var baseDataHistory = UnitOfWork.Repository<ContructCostCFDataBaseHistoryRepo>()
                                .GetManyWithFetch(x => x.COST_CF_ELEMENT_CODE == elementCode && x.ORG_CODE == centerCode && x.TEMPLATE_CODE == key && x.VERSION == lookupData[key].First().VERSION && x.TIME_YEAR == year);
            ...
            {
                var comments = new List<T_BP_CONTRUCT_COST_CF_REVIEW_COMMENT>();
                if (isCountComments)
                {
        ...
        public override IEnumerable<T_MD_COST_CF_ELEMENT> GetDetailSumUpTemplate(string elementCode, int year, int version, string templateCode, string centerCode)
        {
            ...
            else
            {
                if (newestVersionInDb.Value == version)
                {
                    ...
                    var baseData = UnitOfWork.Repository<ContructCostCFDataBaseRepo>().GetManyWithFetch(x => x.TEMPLATE_CODE == templateCode && x.TIME_YEAR == year && x.COST_CF_ELEMENT_CODE == elementCode && x.VERSION == version && x.INTERNAL_ORDER_CODE == centerCode);
                ...
                else
                {
                    ...
                    var baseData = UnitOfWork.Repository<ContructCostCFDataBaseHistoryRepo>().GetManyWithFetch(x => x.TEMPLATE_CODE == templateCode && x.TIME_YEAR == year && x.COST_CF_ELEMENT_CODE == elementCode && x.VERSION == version && x.INTERNAL_ORDER_CODE == centerCode);
                    return from item in baseData
                           select (T_MD_COST_CF_ELEMENT)item;
                }
            }
        ...
        public IList<T_MD_COST_CF_ELEMENT> PreparePureList(IList<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_CF> detailCostElements, int year)
        {
            // get all cost elements
            var allCostElements = UnitOfWork.Repository<CostCFElementRepo>().GetManyByExpression(x => x.TIME_YEAR == year);
        ...
        /// <param name="centerCode"></param>
        /// <returns></returns>
        public IList<T_MD_COST_CF_ELEMENT> PreparePureList(out IList<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_CF> detailCostElements,
            string templateId,
            int year,
            string centerCode = "",
            bool ignoreAuth = false)
        {
            ...
            {
                detailCostElements = UnitOfWork.Repository<TemplateDetailContructCostCFRepo>()
        ...
        /// <param name="centerCode"></param>
        /// <returns></returns>
        public IList<T_MD_COST_CF_ELEMENT> PreparePureListForTemplate(out IList<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_CF> detailCostElements,
            string templateId,
            int year)
        {
        ...
        {
            // Tìm mẫu nộp hộ
            var listTemplateCodes = this.UnitOfWork.Repository<TemplateDetailContructCostCFRepo>()
                .GetManyByExpression(x => centerCodes.Contains(x.CENTER_CODE) && x.TIME_YEAR == year)
                .Select(x => x.TEMPLATE_CODE).Distinct().ToList();
            var findContructCostCF = this.CurrentRepository.GetManyByExpression(
                    x => listTemplateCodes.Contains(x.TEMPLATE_CODE) && !centerCodes.Contains(x.ORG_CODE));
            var lst = new List<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_CF>();
            detailCostElements = new List<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_CF>();
            foreach (var template in listTemplateCodes)
            {
                lst.AddRange(UnitOfWork.Repository<TemplateDetailContructCostCFRepo>()
        ...
        /// </summary>
        /// <param name="costCF">Output header cost pl</param>
        /// <param name="centerCode">Cost center code want to sum up</param>
        /// <param name="year">Year want to sum up</param>
        public override void SumUpDataCenter(out T_BP_CONTRUCT_COST_CF_VERSION costCF, string centerCode, int year)
        {
            ...
            var lstData = new List<T_BP_CONTRUCT_COST_CF_DATA>();
            costCF = new T_BP_CONTRUCT_COST_CF_VERSION();

            try
            {
                UnitOfWork.BeginTransaction();
                var sumUpDetailRepo = UnitOfWork.Repository<ContructCostCFSumUpDetailRepo>();
                var costCFDataRepo = UnitOfWork.Repository<ContructCostCFDataRepo>();

                // get list all children centers in cost center tree
                var lstInternalOrders = GetListOfChildrenCenter(centerCode);

                // get all data have approved
                var costCFDataApproved = costCFDataRepo.GetManyByExpression(x => (x.ORG_CODE == centerCode && !string.IsNullOrEmpty(x.TEMPLATE_CODE)) ||
                (x.STATUS == Approve_Status.DaPheDuyet &&
                x.TIME_YEAR == year &&
                lstInternalOrders.Any(c => c.CODE.Equals(x.ORG_CODE))));

                if (costCFDataApproved.Count == 0)
                {
                ...
                var lookup = costCFDataApproved.ToLookup(x => x.COST_CF_ELEMENT_CODE);
                foreach (var code in lookup.Select(x => x.Key))
                {
                    if (lookup[code].Count() == 1)
                    {
                        lstData.Add((T_BP_CONTRUCT_COST_CF_DATA)lookup[code].First().CloneObject());
                    }
                    else
                    {
                        lstData.Add(new T_BP_CONTRUCT_COST_CF_DATA
                        {
                ...
                var newestCF = UnitOfWork.Repository<ContructCostCFRepo>()
                ...
                UnitOfWork.Repository<ContructCostCFVersionRepo>().Create(costCF);

                foreach (var item in lstData)
                {
                    item.ORG_CODE = centerCode;
                    item.INTERNAL_ORDER_CODE = string.Empty;
                ...
                var lstCostCFDataOldVersion = costCFDataRepo
                ...
                _ = UnitOfWork.GetSession().CreateSQLQuery($"DELETE FROM T_BP_CONTRUCT_COST_CF_DATA WHERE ORG_CODE = '{centerCode}' AND TIME_YEAR = {year}")
                    .ExecuteUpdate();

                // insert to pl data history
                UnitOfWork.Repository<ContructCostCFDataHistoryRepo>().Create((from pl in lstCostCFDataOldVersion
                                                                               select (T_BP_CONTRUCT_COST_CF_DATA_HISTORY)pl).ToList());
                ...
                UnitOfWork.Repository<ContructCostCFHistoryRepo>().Create(new T_BP_CONTRUCT_COST_CF_HISTORY
                {
                    ...
                    TIME_YEAR = year,
                    TEMPLATE_CODE = string.Empty,
                    VERSION = costCF.VERSION,
                });

                // create new lstData
                costCFDataRepo.Create(lstData.ToList());

                // get list cost pl which have org cost in cost center summary
                var lstContructCostCF = costCFDataApproved.ToLookup(x => new { OrgCode = x.ORG_CODE, TemplateCode = x.TEMPLATE_CODE, TemplateVersion = x.VERSION });

                // create list sum up detail
                sumUpDetailRepo.Create((from c in lstContructCostCF.Select(x => x.Key)
                                        select new T_BP_CONTRUCT_COST_CF_SUM_UP_DETAIL
                                        {
                                            PKID = Guid.NewGuid().ToString(),
            ...
            string templateId = "")
        {
            ...
            plDataCostElements = UnitOfWork.Repository<ContructCostCFDataRepo>()
        ...
        private IEnumerable<T_MD_COST_CF_ELEMENT> GetSumDescendants(
            IEnumerable<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_CF> details,
            ...
            int? version = null)
        {
            ...
            {
                var costCFData = GetVersionData(templateId, centerCode, year, version);
                ...
                while (st.Count > 0)
                {
                    ...
                    foreach (var i in lookup[item.CODE])
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (detail != null)
                                {
                                    detail.CFData = costCFData.FirstOrDefault(x => x.COST_CF_ELEMENT_CODE == detail.ELEMENT_CODE && x.INTERNAL_ORDER_CODE == detail.CENTER_CODE);
                                }
                    ...
                    if (shouldReturn)
                    {
                        ...
                        {
                            var data = costCFData.Where(x => x.COST_CF_ELEMENT_CODE == item.CODE && x.INTERNAL_ORDER_CODE == item.CENTER_CODE);
                            if (data != null)
                            {
        ...
        private IList<T_BP_CONTRUCT_COST_CF_DATA> GetVersionData(
            string templateCode,
            string centerCode,
            int? year = null,
            int? version = null)
        {
            ...
            {
                return UnitOfWork.Repository<ContructCostCFDataRepo>().GetCFDataByOrgCode(template.ORG_CODE, year.Value, templateCode, version);
            }
            ...
            {
                return UnitOfWork.Repository<ContructCostCFDataRepo>().GetCFDataByCenterCode(template.ORG_CODE, new List<string> { centerCode }, year.Value, templateCode, version);
                // get 
            }
        }
        ...
        public IList<T_MD_COST_CF_ELEMENT> SummaryCenterOut(out IList<T_BP_CONTRUCT_COST_CF_DATA> plDataCostElements,
                                                              string centerCode,
                                                              int year,
                                                              int? version,
                                                              bool? isHasValue = null)
        {
            if (!version.HasValue)
            {
                // get newest cost pl data by center code
                plDataCostElements = UnitOfWork.Repository<ContructCostCFDataRepo>()
                    .GetManyByExpression(x => x.INTERNAL_ORDER_CODE.Equals(centerCode) && x.TIME_YEAR == year);
            }
            else
            {
                ...
                if (IsLeaf())
                {
                    // get all data have approved
                    plDataCostElements = UnitOfWork.Repository<ContructCostCFDataRepo>()
                        .GetManyByExpression(x => x.INTERNAL_ORDER_CODE == centerCode && !string.IsNullOrEmpty(x.TEMPLATE_CODE));
                }
                else
                {
                    // get list all children centers in cost center tree
                    ...
                    x.TIME_YEAR == year &&
                    lstInternalOrders.Any(c => c.CODE.Equals(x.INTERNAL_ORDER_CODE))));
                }
            }
        ...
        /// <returns></returns>
        public IList<T_MD_COST_CF_ELEMENT> SummaryCenterVersion(out IList<T_BP_CONTRUCT_COST_CF_DATA> plDataCostElements,
                                                              string centerCode,
                                                              int year,
                                                              int? version,
        ...
        /// <returns></returns>
        private IList<T_BP_CONTRUCT_COST_CF_DATA> GetAllSumUpDetails(string centerCode, int year, int version)
        {
            var detailsSumUp = UnitOfWork.Repository<ContructCostCFSumUpDetailRepo>()
               .GetManyWithFetch(x => x.ORG_CODE == centerCode && x.TIME_YEAR == year && x.SUM_UP_VERSION == version);

            var lstChildren = GetListOfChildrenCenter(centerCode).Select(x => x.CODE);
            var lstDetails = new List<T_BP_CONTRUCT_COST_CF_DATA>();
            var lstResult = new List<T_BP_CONTRUCT_COST_CF_DATA>();
            var plDataRepo = UnitOfWork.Repository<ContructCostCFDataRepo>();
                ...
                else
                {
                    ...
                    foreach (var orgCode in lookupCenter.Select(x => x.Key))
                    {
                        ...
                        else
                        {
                            ...
                            lstResult.Add(new T_BP_CONTRUCT_COST_CF_DATA
                            {
                                ORG_CODE = orgCode,
                                COST_CF_ELEMENT_CODE = key,
                                InternalOrder = lookupCenter[orgCode].First().InternalOrder,
        ...
        private IList<T_MD_COST_CF_ELEMENT> SummaryCenter(IList<T_BP_CONTRUCT_COST_CF_DATA> plDataCostElements, string centerCode, int year,
                                                          bool? isHasValue = null)
        {

            // get all cost elements
            var allCostElement = UnitOfWork.Repository<CostCFElementRepo>().GetManyByExpression(x => x.TIME_YEAR == year);
            // get all child
            ...
            var lookupElements = plDataCostElements.GroupBy(x => x.COST_CF_ELEMENT_CODE)
                .Select(x => x.First())
            ...
            foreach (var code in lookupElements.Select(l => l.Key))
            {
                ...
                lst.AddRange(from item in lookupElements[code]
                             let orgCode = childrenCodes.Contains(item.ORG_CODE) || item.ORG_CODE == centerCode ? item.ORG_CODE : item.INTERNAL_ORDER_CODE
                             select new T_MD_COST_CF_ELEMENT
                             {
                                 ...
                                 LEVEL = 0,
                                 IS_GROUP = false,
                                 ORG_NAME = childrenCode.Contains(item.ORG_CODE) ? item.Organize.NAME : item.InternalOrder.NAME,
        ...
        /// </summary>
        /// <param name="costCFData">List cost pl data containt value</param>
        ...
        private IEnumerable<T_MD_COST_CF_ELEMENT> GetSumDescendants(
            IList<T_BP_CONTRUCT_COST_CF_DATA> costCFData,
            ...
            string parentId)
        {
            ...
            while (st.Count > 0)
            {
                ...
                bool shouldReturn = true;
                // lst to store children of item which have children
                var lstCostElements = new List<T_MD_COST_CF_ELEMENT>();
                // loop through items which have parent id = item id
                foreach (var i in lookup[item.CODE])
                {
                    if (lookup[i.CODE].Count() > 0)
        ...
        public override void GenerateExportExcel(ref MemoryStream outFileStream, string html, string path, int year, string centerCode, int? version, string templateId, string unit, decimal exchangeRate)
        {
            ...
            workbook.SetSheetName(0, ModulType.GetTextSheetName(ModulType.ContructCostCF));
        ...
        {
            var name = "DỮ LIỆU KẾ HOẠCH CHI PHÍ DÒNG TIỀN XÂY DỰNG DỰ ÁN";
        ...
        {
            return UnitOfWork.Repository<TypeRepo>().GetFirstWithFetch(x => x.OBJECT_TYPE == TemplateObjectType.DevelopProject && x.ELEMENT_TYPE == ElementType.ChiPhi && x.BUDGET_TYPE == BudgetType.DongTien);
        }
    }
}
