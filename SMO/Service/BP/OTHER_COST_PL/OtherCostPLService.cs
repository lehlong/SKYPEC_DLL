...
using SMO.Repository.Implement.BP.OTHER_COST_PL.OTHER_COST_PL_DATA_BASE;
using SMO.Repository.Implement.MD;
using SMO.Service.BP.COST_PL;
using SMO.Service.Class;
using SMO.Service.Common;
using SMO.ServiceInterface.BP.OtherCostPL;
...
{
    public class OtherCostPLService : BaseBPService<T_BP_OTHER_COST_PL, OtherCostPLRepo, T_MD_COST_PL_ELEMENT, T_BP_OTHER_COST_PL_VERSION, T_BP_OTHER_COST_PL_HISTORY, OtherCostPLHistoryRepo>, IOtherCostPLService
    {
        ...
        public List<T_BP_OTHER_COST_PL_HISTORY> ObjListHistory { get; set; }
        public List<T_BP_OTHER_COST_PL_VERSION> ObjListVersion { get; set; }
        public List<T_BP_OTHER_COST_PL_SUM_UP_DETAIL> ObjListSumUpHistory { get; set; }
        public OtherCostPLService()
        {
            this.StartRowData = 6;
            ...
            this.ObjListHistory = new List<T_BP_OTHER_COST_PL_HISTORY>();
            this.ObjListVersion = new List<T_BP_OTHER_COST_PL_VERSION>();
            this.ObjListSumUpHistory = new List<T_BP_OTHER_COST_PL_SUM_UP_DETAIL>();
        }
        ...
        public override bool ShowReviewBtn(int year)
        {
            ...
            else
            {
                ...
                var historyReview = UnitOfWork.Repository<OtherCostPLReviewRepo>()
        ...
        private void SendNotify(Activity activity)
        {
            NotifyUtilities.CreateNotify(
                    new NotifyPara()
                    {
                        ...
                        ModulType = ModulType.OtherCostPL,
        ...
        public override void TrinhDuyet(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<OtherCostPLHistoryRepo>()
                    .Create(new T_BP_OTHER_COST_PL_HISTORY()
                    {
                ...
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_OTHER_COST_PL_DATA SET STATUS = '{Approve_Status.ChoPheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void HuyTrinhDuyet(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<OtherCostPLHistoryRepo>().Create(new T_BP_OTHER_COST_PL_HISTORY()
                {
                    PKID = Guid.NewGuid().ToString(),
                    ORG_CODE = ObjDetail.ORG_CODE,
                    TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_OTHER_COST_PL_DATA SET STATUS = '{Approve_Status.ChuaTrinhDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void PheDuyet(string code)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_OTHER_COST_PL_DATA SET STATUS = '{Approve_Status.DaPheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void HuyNop(string code)
        {
            ...
            try
            {
                ...
                UnitOfWork.Repository<OtherCostPLVersionRepo>().Update(x => x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.VERSION == ObjDetail.VERSION, x => x.IS_DELETED = 1, x => x.UPDATE_BY = currentUser);
        ...
        public override void TuChoi(string code)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_OTHER_COST_PL_DATA SET STATUS = '{Approve_Status.TuChoi}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void ChuyenTKS(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<OtherCostPLReviewRepo>()
        ...
        public override void KetThucThamDinh(string orgCode, int year, int version)
        {
            try
            {
                ...
                UnitOfWork.BeginTransaction();
                var currentUser = ProfileUtilities.User?.USER_NAME;
                var reviewHeader = UnitOfWork.Repository<OtherCostPLReviewRepo>()
                ...
                if (reviewHeader != null)
                {
                    ...
                    UnitOfWork.Repository<OtherCostPLReviewRepo>().Update(reviewHeader);
                }

                var totalReviewUsers = UnitOfWork.Repository<UserReviewRepo>()
                    .GetManyWithFetch(x => x.TIME_YEAR == ObjDetail.TIME_YEAR).Count;

                var totalUsersEndReview = UnitOfWork.Repository<OtherCostPLReviewRepo>()
                ...
                var review = UnitOfWork.Repository<OtherCostPLReviewRepo>()
                ...
                if (review == null)
                {
                    ...
                    UnitOfWork.Repository<OtherCostPLReviewRepo>().Create(
                        new T_BP_OTHER_COST_PL_REVIEW
                        {
                            PKID = reviewId,
                    ...
                    var plReviewService = new OtherCostPLReviewService();
                    plReviewService.ObjDetail.TIME_YEAR = year;
                    plReviewService.ObjDetail.ORG_CODE = orgCode;
                    var dataCost = plReviewService.SummaryCenterVersion(out IList<T_BP_OTHER_COST_PL_DATA> detailCostElements);
                    dataCost = dataCost.OrderBy(x => x.C_ORDER)
                                                    .GroupBy(x => x.CODE)
                                                    .Select(x => x.First()).ToList();
                    var elements = plReviewService.PrepareListReview(dataCost);
                    UnitOfWork.Repository<OtherCostPLReviewResultRepo>()
                        .Create((from e in elements.Where(x => !x.IS_GROUP)
                                 select new T_BP_OTHER_COST_PL_REVIEW_RESULT
                                 {
                                     PKID = Guid.NewGuid().ToString(),
                ...
                {
                    review.IS_END = true;
                    UnitOfWork.Repository<OtherCostPLReviewRepo>().Update(review);
                }
        ...
        public override void PheDuyetTongKiemSoat(string orgCode, int year, int version)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_OTHER_COST_PL_DATA SET STATUS = '{Approve_Status.TKS_PheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        internal override void TuChoiTongKiemSoat(string orgCode, int year, int version, string comment)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_OTHER_COST_PL_DATA SET STATUS = '{Approve_Status.TKS_TuChoi}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
                UnitOfWork.Repository<OtherCostPLReviewRepo>()
        ...
        public override void TrinhDuyetTongKiemSoat(string orgCode, int year, int version)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_OTHER_COST_PL_DATA SET STATUS = '{Approve_Status.TKS_TrinhDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
                UnitOfWork.Repository<OtherCostPLReviewRepo>()
                    .Update(x => x.ORG_CODE == orgCode
                                && x.TIME_YEAR == year
                                && x.DATA_VERSION == version
        ...
        public override void ChuyenHDNS(string code)
        {
            try
            {
                ...
                {
                    UnitOfWork.Repository<OtherCostPLReviewRepo>().Update(x => x.DATA_VERSION == ObjDetail.VERSION
        ...
        public override void GetListOfChild()
        {
            ...
            // Tìm mẫu nộp hộ
            var listTemplateCode = this.UnitOfWork.Repository<TemplateDetailOtherCostPLRepo>().Queryable().Where(
            ...
            var findOtherCostPL = this.CurrentRepository.Queryable().Where(
        ...
        {
            var query = UnitOfWork.GetSession().QueryOver<T_BP_OTHER_COST_PL_SUM_UP_DETAIL>();
        ...
        /// <returns></returns>
        private (IList<T_BP_OTHER_COST_PL_DATA>, bool) GetDataSumUp(int year, string centerCode, string elementCode, int sumUpVersion)
        {
            var plDataRepo = UnitOfWork.Repository<OtherCostPLDataRepo>();
            var costPlReviewCommentRepo = UnitOfWork.Repository<OtherCostPLReviewCommentRepo>();
            var plVersionRepo = UnitOfWork.Repository<OtherCostPLRepo>();
            var plDataHistoryRepo = UnitOfWork.Repository<OtherCostPLDataHistoryRepo>();

            var lstDetails = UnitOfWork.Repository<OtherCostPLSumUpDetailRepo>()
            ...
            var data = new List<T_BP_OTHER_COST_PL_DATA>();
            foreach (var key in lookup.Select(x => x.Key))
            {
                var costPL = new List<T_BP_OTHER_COST_PL_DATA>();
        ...
        public override IEnumerable<T_MD_COST_PL_ELEMENT> GetDetailSumUp(string centerCode, string elementCode, int year, int version, int? sumUpVersion, bool isCountComments, bool? isShowFile = null)
        {
            ...
            if (IsLeaf(centerCode))
            {
                ...
                foreach (var key in lookupData.Select(x => x.Key))
                {
                    ...
                    {
                        var isNewestVersion = UnitOfWork.Repository<OtherCostPLVersionRepo>()
                        ...
                        {
                            // get data from base
                            var baseData = UnitOfWork.Repository<OtherCostPLDataBaseRepo>()
                                .GetManyWithFetch(x => x.COST_PL_ELEMENT_CODE == elementCode && x.ORG_CODE == centerCode && x.TEMPLATE_CODE == key && x.VERSION == lookupData[key].First().VERSION && x.TIME_YEAR == year);
                        ...
                        else
                        {
                            ...
                            var baseDataHistory = UnitOfWork.Repository<OtherCostPLDataBaseHistoryRepo>()
                                .GetManyWithFetch(x => x.COST_PL_ELEMENT_CODE == elementCode && x.ORG_CODE == centerCode && x.TEMPLATE_CODE == key && x.VERSION == lookupData[key].First().VERSION && x.TIME_YEAR == year);
            ...
            {
                var comments = new List<T_BP_OTHER_COST_PL_REVIEW_COMMENT>();
                if (isCountComments)
                {
        ...
        public override IEnumerable<T_MD_COST_PL_ELEMENT> GetDetailSumUpTemplate(string elementCode, int year, int version, string templateCode, string centerCode)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    var baseData = UnitOfWork.Repository<OtherCostPLDataBaseHistoryRepo>()
                        .GetManyWithFetch(x => x.TEMPLATE_CODE == templateCode
                        && x.TIME_YEAR == year
                        && x.COST_PL_ELEMENT_CODE == elementCode
                        && x.VERSION == version
        ...
        public IList<T_MD_COST_PL_ELEMENT> GetDataCost(out IList<T_MD_TEMPLATE_DETAIL_OTHER_COST_PL> detailOtherCostElements,
            out IList<T_BP_OTHER_COST_PL_DATA> detailOtherCostData,
        ...
        public override IList<T_MD_COST_PL_ELEMENT> GetDetailPreviewSumUp(string centerCode, string elementCode, int year)
        {
            ...
            var plDataOtherCostElements = UnitOfWork.Repository<OtherCostPLDataRepo>().GetPLDataByCenterCode(null, new List<string> { centerCode }, year, null, null);

            return plDataOtherCostElements.Where(x => x.COST_PL_ELEMENT_CODE == elementCode)
                .Select(x => (T_MD_COST_PL_ELEMENT)x)
                .OrderBy(x => x.C_ORDER).ToList();
        }

        private IList<T_MD_COST_PL_ELEMENT> GetDataCostPreview(out IList<T_MD_TEMPLATE_DETAIL_OTHER_COST_PL> detailOtherCostElements, string templateId, List<string> lstOrgs, int year, int? version, bool? isHasValue)
        {
            detailOtherCostElements = new List<T_MD_TEMPLATE_DETAIL_OTHER_COST_PL>();
            var lstElements = new List<T_MD_COST_PL_ELEMENT>();
            foreach (var orgCode in lstOrgs)
            {
                var elements = GetDataCostPreview(out IList<T_MD_TEMPLATE_DETAIL_OTHER_COST_PL> detail, templateId, orgCode, year, version, isHasValue);
        ...
        public override T_BP_OTHER_COST_PL_VERSION GetHeader(string templateCode, string centerCode, int year, int? version)
        {
            templateCode = templateCode ?? string.Empty;
            if (version == null)
            {
                return UnitOfWork.Repository<OtherCostPLVersionRepo>()
        ...
        {
            ObjListVersion = UnitOfWork.Repository<OtherCostPLVersionRepo>()
        ...
        {
            var query = this.UnitOfWork.GetSession().QueryOver<T_BP_OTHER_COST_PL_HISTORY>();
        ...
        public override void GetHistory(string orgCode, string templateId, int? year)
        {
            if (year.HasValue)
            {
                this.ObjListHistory = UnitOfWork.Repository<OtherCostPLHistoryRepo>().GetManyByExpression(
        ...
        public override void ValidateData(DataTable dataTable, bool isDataBase)
        {
            ...
            var lstDetailTemplate = this.UnitOfWork.Repository<TemplateDetailOtherCostPLRepo>()
        ...
        public override void ImportExcel(HttpRequestBase request)
        {
            ...
            var otherCostPLCurrent = CurrentRepository.Queryable().FirstOrDefault(x => x.ORG_CODE == orgCode
                && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE);

            if (otherCostPLCurrent != null && !(otherCostPLCurrent.STATUS == Approve_Status.TuChoi || otherCostPLCurrent.STATUS == Approve_Status.ChuaTrinhDuyet))
            {
                this.State = false;
            ...
            var versionNext = 1;
            if (otherCostPLCurrent != null)
            {
                versionNext = otherCostPLCurrent.VERSION + 1;
            }

            // Lấy dữ liệu của version hiện tại
            var dataCurrent = new List<T_BP_OTHER_COST_PL_DATA>();
            if (otherCostPLCurrent != null)
            {
                dataCurrent = UnitOfWork.Repository<OtherCostPLDataRepo>().Queryable().Where(x => x.ORG_CODE == orgCode
            ...
            try
            {
                ...
                if (otherCostPLCurrent != null)
                {
                    ...
                    otherCostPLCurrent.VERSION = versionNext;
                    otherCostPLCurrent.IS_DELETED = false;
                    CurrentRepository.Update(otherCostPLCurrent);
                }
                else
                {
                    // Tạo mới bản ghi revenue pl
                    CurrentRepository.Create(new T_BP_OTHER_COST_PL()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                        TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                UnitOfWork.Repository<OtherCostPLVersionRepo>().Create(new T_BP_OTHER_COST_PL_VERSION()
                {
                    PKID = Guid.NewGuid().ToString(),
                    ORG_CODE = orgCode,
                    TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                {
                    var revenueDataHis = (T_BP_OTHER_COST_PL_DATA_HISTORY)item;
                    UnitOfWork.Repository<OtherCostPLDataHistoryRepo>().Create(revenueDataHis);
                    UnitOfWork.Repository<OtherCostPLDataRepo>().Delete(item);
                }
                ...
                for (int i = this.StartRowData; i < actualRows; i++)
                {
                    ...
                    var costData = new T_BP_OTHER_COST_PL_DATA()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                        ...
                        STATUS = Approve_Status.ChuaTrinhDuyet,
                        VERSION = versionNext,
                        COST_PL_ELEMENT_CODE = tableData.Rows[i][4].ToString().Trim(),
                    ...
                    UnitOfWork.Repository<OtherCostPLDataRepo>().Create(costData);
                }
                UnitOfWork.Commit();
                NotifyUtilities.CreateNotify(
                    new NotifyPara()
        ...
        public override void ImportExcelBase(HttpRequestBase request)
        {
            ...
            this.StartRowData = 6;
            var orgCode = ProfileUtilities.User?.ORGANIZE_CODE;
            var otherCostPLDataRepo = UnitOfWork.Repository<OtherCostPLDataRepo>();
            var otherCostPLDataBaseRepo = UnitOfWork.Repository<OtherCostPLDataBaseRepo>();
            var currentUser = ProfileUtilities.User?.USER_NAME;
            ...
            // Xác định version dữ liệu
            var otherCostPLCurrent = GetFirstWithFetch(x => x.ORG_CODE == orgCode
                && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE);
            ...
            // Lấy dữ liệu của version hiện tại
            var dataCurrent = new List<T_BP_OTHER_COST_PL_DATA>();
            var dataCurrentBase = new List<T_BP_OTHER_COST_PL_DATA_BASE>();
            if (otherCostPLCurrent != null)
            {
                dataCurrent = otherCostPLDataRepo.GetManyWithFetch(x => x.ORG_CODE == orgCode
                    && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE).ToList();

                dataCurrentBase = otherCostPLDataBaseRepo.GetManyWithFetch(x => x.ORG_CODE == orgCode
                    && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE).ToList();
            }
            ...
            try
            {
                ...
                foreach (var item in dataCurrent)
                {
                    var costDataHis = (T_BP_OTHER_COST_PL_DATA_HISTORY)item;
                    UnitOfWork.Repository<OtherCostPLDataHistoryRepo>().Create(costDataHis);
                    otherCostPLDataRepo.Delete(item);
                }
                ...
                foreach (var key in lookUp.Select(x => x.Key))
                {
                    ...
                    var costData = new T_BP_OTHER_COST_PL_DATA()
                    {
                        ...
                        VALUE_JAN = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[10].ToString())),
                        VALUE_FEB = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[13].ToString())),
                        VALUE_MAR = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[16].ToString())),
                        VALUE_APR = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[19].ToString())),
                        VALUE_MAY = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[22].ToString())),
                        VALUE_JUN = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[25].ToString())),
                        VALUE_JUL = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[28].ToString())),
                        VALUE_AUG = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[31].ToString())),
                        VALUE_SEP = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[34].ToString())),
                        VALUE_OCT = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[37].ToString())),
                        VALUE_NOV = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[40].ToString())),
                        VALUE_DEC = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[42].ToString())),
                        CREATE_BY = currentUser
                    };
                    costData.VALUE_SUM_YEAR = costData.VALUE_JAN + costData.VALUE_FEB + costData.VALUE_MAR + costData.VALUE_APR + costData.VALUE_MAY + costData.VALUE_JUN + costData.VALUE_JUL + costData.VALUE_AUG + costData.VALUE_SEP + costData.VALUE_OCT + costData.VALUE_NOV + costData.VALUE_DEC;

                    costData.VALUE_SUM_YEAR_PREVENTIVE = costData.VALUE_SUM_YEAR * percentagePreventive;
                    otherCostPLDataRepo.Create(costData);
                }

                // Insert data vào base data history
                var lstBaseDataHistory = (from x in dataCurrentBase
                                          select (T_BP_OTHER_COST_PL_DATA_BASE_HISTORY)x).ToList();
                UnitOfWork.Repository<OtherCostPLDataBaseHistoryRepo>().Create(lstObj: lstBaseDataHistory);
                otherCostPLDataBaseRepo.Delete(dataCurrentBase);
                ...
                for (int i = this.StartRowData; i < actualRows; i++)
                {
                    ...
                    var costData = new T_BP_OTHER_COST_PL_DATA_BASE()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                        ...
                        QUANTITY_M1 = UtilsCore.StringToDecimal(tableData.Rows[i][8].ToString().Trim()),
                        PRICE_M1 = UtilsCore.StringToDecimal(tableData.Rows[i][9].ToString().Trim()),
                        AMOUNT_M1 = UtilsCore.StringToDecimal(tableData.Rows[i][10].ToString().Trim()),

                        QUANTITY_M2 = UtilsCore.StringToDecimal(tableData.Rows[i][11].ToString().Trim()),
                        PRICE_M2 = UtilsCore.StringToDecimal(tableData.Rows[i][12].ToString().Trim()),
                        AMOUNT_M2 = UtilsCore.StringToDecimal(tableData.Rows[i][13].ToString().Trim()),

                        QUANTITY_M3 = UtilsCore.StringToDecimal(tableData.Rows[i][14].ToString().Trim()),
                        PRICE_M3 = UtilsCore.StringToDecimal(tableData.Rows[i][15].ToString().Trim()),
                        AMOUNT_M3 = UtilsCore.StringToDecimal(tableData.Rows[i][16].ToString().Trim()),

                        QUANTITY_M4 = UtilsCore.StringToDecimal(tableData.Rows[i][17].ToString().Trim()),
                        PRICE_M4 = UtilsCore.StringToDecimal(tableData.Rows[i][18].ToString().Trim()),
                        AMOUNT_M4 = UtilsCore.StringToDecimal(tableData.Rows[i][19].ToString().Trim()),

                        QUANTITY_M5 = UtilsCore.StringToDecimal(tableData.Rows[i][20].ToString().Trim()),
                        PRICE_M5 = UtilsCore.StringToDecimal(tableData.Rows[i][21].ToString().Trim()),
                        AMOUNT_M5 = UtilsCore.StringToDecimal(tableData.Rows[i][22].ToString().Trim()),

                        QUANTITY_M6 = UtilsCore.StringToDecimal(tableData.Rows[i][23].ToString().Trim()),
                        PRICE_M6 = UtilsCore.StringToDecimal(tableData.Rows[i][24].ToString().Trim()),
                        AMOUNT_M6 = UtilsCore.StringToDecimal(tableData.Rows[i][25].ToString().Trim()),

                        QUANTITY_M7 = UtilsCore.StringToDecimal(tableData.Rows[i][26].ToString().Trim()),
                        PRICE_M7 = UtilsCore.StringToDecimal(tableData.Rows[i][27].ToString().Trim()),
                        AMOUNT_M7 = UtilsCore.StringToDecimal(tableData.Rows[i][28].ToString().Trim()),

                        QUANTITY_M8 = UtilsCore.StringToDecimal(tableData.Rows[i][29].ToString().Trim()),
                        PRICE_M8 = UtilsCore.StringToDecimal(tableData.Rows[i][30].ToString().Trim()),
                        AMOUNT_M8 = UtilsCore.StringToDecimal(tableData.Rows[i][31].ToString().Trim()),

                        QUANTITY_M9 = UtilsCore.StringToDecimal(tableData.Rows[i][32].ToString().Trim()),
                        PRICE_M9 = UtilsCore.StringToDecimal(tableData.Rows[i][33].ToString().Trim()),
                        AMOUNT_M9 = UtilsCore.StringToDecimal(tableData.Rows[i][34].ToString().Trim()),

                        QUANTITY_M10 = UtilsCore.StringToDecimal(tableData.Rows[i][35].ToString().Trim()),
                        PRICE_M10 = UtilsCore.StringToDecimal(tableData.Rows[i][36].ToString().Trim()),
                        AMOUNT_M10 = UtilsCore.StringToDecimal(tableData.Rows[i][37].ToString().Trim()),

                        QUANTITY_M11 = UtilsCore.StringToDecimal(tableData.Rows[i][38].ToString().Trim()),
                        PRICE_M11 = UtilsCore.StringToDecimal(tableData.Rows[i][39].ToString().Trim()),
                        AMOUNT_M11 = UtilsCore.StringToDecimal(tableData.Rows[i][40].ToString().Trim()),

                        QUANTITY_M12 = UtilsCore.StringToDecimal(tableData.Rows[i][41].ToString().Trim()),
                        PRICE_M12 = UtilsCore.StringToDecimal(tableData.Rows[i][42].ToString().Trim()),
                        AMOUNT_M12 = UtilsCore.StringToDecimal(tableData.Rows[i][43].ToString().Trim()),
                    ...
                    otherCostPLDataBaseRepo.Create(costData);
                }
                UnitOfWork.Commit();
        ...
        public override IList<T_BP_OTHER_COST_PL_VERSION> GetVersions(string orgCode, string templateId, int year)
        {
        ...
        {
            return UnitOfWork.Repository<OtherCostPLDataRepo>()
        ...
        {
            return UnitOfWork.Repository<OtherCostPLDataHistoryRepo>()
                .GetManyByExpression(x => lstOrgCodes.Contains(x.ORG_CODE) && x.TEMPLATE_CODE != string.Empty && (!year.HasValue || x.TIME_YEAR == year.Value))
                .Select(x => x.TEMPLATE_CODE)
                .Distinct()
                .ToList();
        ...
        public override IList<int> GetTemplateVersion(string templateId, string centerCode, int year)
        {
            if (string.IsNullOrEmpty(templateId))
            {
                return UnitOfWork.Repository<OtherCostPLSumUpDetailRepo>()
        ...
        {
            var dataOtherCost = PreparePureList(out IList<T_MD_TEMPLATE_DETAIL_OTHER_COST_PL> detailOtherCostElements, templateId, year, ignoreAuth: true);
            ...
            try
            {
                ...
                templateWorkbook.SetSheetName(0, ModulType.GetTextSheetName(ModulType.OtherCostPL));
                fs.Close();
        ...
        {
            var dataOtherCost = PreparePureList(out IList<T_MD_TEMPLATE_DETAIL_OTHER_COST_PL> detailCostElements, templateId, year, ignoreAuth: true);
            ...
            try
            {
                    ...
                    .OrderByDescending(x => x.CENTER_CODE))
                {
                            ...
                            .Select(x => x.First()))
                    {
                        ...
                        rowCur.Cells[6].SetCellValue(string.Empty);
                        rowCur.Cells[6].CellStyle = styleCellDetail;

                        rowCur.Cells[7].SetCellValue(string.Empty);
                        rowCur.Cells[7].CellStyle = styleCellDetail;

                        for (int j = 8; j < NUM_CELL - 1; j++)
                        {
        ...
        public override IList<NodeDataFlow> BuildDataFlowTree(string orgCode, int year, int? version, int? sumUpVersion)
        {
            ...
            if (costCenterRepo.GetManyByExpression(x => x.PARENT_CODE == orgCode).Count > 0 && version == null)
            {
                ...
                {
                    var revenuePLData = UnitOfWork.Repository<OtherCostPLSumUpDetailRepo>()
            ...
            else
            {
                // level 2: department
                // get templates
                var sumupDetails = UnitOfWork.Repository<OtherCostPLSumUpDetailRepo>()
        ...
        public IList<T_MD_COST_PL_ELEMENT> GetDataCostPreview(
            out IList<T_MD_TEMPLATE_DETAIL_OTHER_COST_PL> detailOtherCostElements,
            string templateId,
            string centerCode = "",
            int? year = null,
            int? version = null,
        ...
        public IList<T_MD_COST_PL_ELEMENT> PreparePureList(IList<T_MD_TEMPLATE_DETAIL_OTHER_COST_PL> detailOtherCostElements, int year)
        {
            // get all revenue elements
            var allOtherCostElements = UnitOfWork.Repository<CostPLElementRepo>().GetManyByExpression(x => x.TIME_YEAR == year);
        ...
        /// <param name="centerCode"></param>
        /// <returns></returns>
        public IList<T_MD_COST_PL_ELEMENT> PreparePureList(out IList<T_MD_TEMPLATE_DETAIL_OTHER_COST_PL> detailOtherCostElements,
            string templateId,
            int year,
            string centerCode = "",
            bool ignoreAuth = false)
        {
            ...
            {
                detailOtherCostElements = UnitOfWork.Repository<TemplateDetailOtherCostPLRepo>()
        ...
        /// <param name="centerCode"></param>
        /// <returns></returns>
        public IList<T_MD_COST_PL_ELEMENT> PreparePureListForTemplate(out IList<T_MD_TEMPLATE_DETAIL_OTHER_COST_PL> detailOtherCostElements,
            string templateId,
            int year)
        {
        ...
        {
            // Tìm mẫu nộp hộ
            var listTemplateCodes = this.UnitOfWork.Repository<TemplateDetailOtherCostPLRepo>()
                .GetManyByExpression(x => centerCodes.Contains(x.CENTER_CODE) && x.TIME_YEAR == year)
                .Select(x => x.TEMPLATE_CODE).Distinct().ToList();
            var findOtherCostPL = this.CurrentRepository.GetManyByExpression(
                    x => listTemplateCodes.Contains(x.TEMPLATE_CODE) && !centerCodes.Contains(x.ORG_CODE));
            var lst = new List<T_MD_TEMPLATE_DETAIL_OTHER_COST_PL>();
            detailOtherCostElements = new List<T_MD_TEMPLATE_DETAIL_OTHER_COST_PL>();
            foreach (var template in listTemplateCodes)
            {
                lst.AddRange(UnitOfWork.Repository<TemplateDetailOtherCostPLRepo>()
        ...
        public override void SumUpDataCenter(out T_BP_OTHER_COST_PL_VERSION revenuePL, string centerCode, int year)
        {
            ...
            var lstData = new List<T_BP_OTHER_COST_PL_DATA>();
            revenuePL = new T_BP_OTHER_COST_PL_VERSION();

            try
            {
                UnitOfWork.BeginTransaction();
                var sumUpDetailRepo = UnitOfWork.Repository<OtherCostPLSumUpDetailRepo>();
                var revenuePLDataRepo = UnitOfWork.Repository<OtherCostPLDataRepo>();
                ...
                var lookup = revenuePlDataApproved.ToLookup(x => x.COST_PL_ELEMENT_CODE);
                foreach (var code in lookup.Select(x => x.Key))
                {
                    // TODO: check if all value of months are equal 0
                    if (lookup[code].Count() == 1)
                    {
                        lstData.Add((T_BP_OTHER_COST_PL_DATA)lookup[code].First().CloneObject());
                    }
                    else
                    {
                        lstData.Add(new T_BP_OTHER_COST_PL_DATA
                        {
                ...
                var newestPL = UnitOfWork.Repository<OtherCostPLRepo>()
                ...
                UnitOfWork.Repository<OtherCostPLVersionRepo>().Create(revenuePL);
                ...
                _ = UnitOfWork.GetSession().CreateSQLQuery($"DELETE FROM T_BP_OTHER_COST_PL_DATA WHERE ORG_CODE = '{centerCode}' AND TIME_YEAR = {year}")
                    .ExecuteUpdate();

                // insert to pl data history
                UnitOfWork.Repository<OtherCostPLDataHistoryRepo>().Create((from pl in lstOtherCostPlDataOldVersion
                                                                            select (T_BP_OTHER_COST_PL_DATA_HISTORY)pl).ToList());
                ...
                var lstOtherCostPL = revenuePlDataApproved.ToLookup(x => new { OrgCode = x.ORG_CODE, TemplateCode = x.TEMPLATE_CODE, TemplateVersion = x.VERSION });

                // create list sum up detail
                sumUpDetailRepo.Create((from c in lstOtherCostPL.Select(x => x.Key)
                                        select new T_BP_OTHER_COST_PL_SUM_UP_DETAIL
                                        {
                                            PKID = Guid.NewGuid().ToString(),
        ...
        public IList<T_MD_COST_PL_ELEMENT> SummarySumUpCenter(
            out IList<T_BP_OTHER_COST_PL_DATA> plDataOtherCostElements,
            int year,
            string centerCode,
            int? version,
            bool? isHasValue = null,
            string templateId = "")
        {
            ...
            plDataOtherCostElements = UnitOfWork.Repository<OtherCostPLDataRepo>()
        ...
        private IEnumerable<T_MD_COST_PL_ELEMENT> GetSumDescendants(
            IEnumerable<T_MD_TEMPLATE_DETAIL_OTHER_COST_PL> details,
            ...
            foreach (var centerCode in lookupCenter.Select(x => x.Key))
            {
                ...
                while (st.Count > 0)
                {
                    ...
                    foreach (var i in lookup[item.CODE])
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (detail != null)
                                {
                                    detail.PLData = revenuePlData.FirstOrDefault(x => x.COST_PL_ELEMENT_CODE == detail.ELEMENT_CODE && x.OTHER_PROFIT_CENTER_CODE == detail.CENTER_CODE);
                                }
                    ...
                    if (shouldReturn)
                    {
                        ...
                        {
                            var data = revenuePlData.Where(x => x.COST_PL_ELEMENT_CODE == item.CODE && x.OTHER_PROFIT_CENTER_CODE == item.CENTER_CODE);
                            if (data != null)
                            {
        ...
        /// <summary>
        /// Get revenue PL data by version
        /// </summary>
        ...
        private IList<T_BP_OTHER_COST_PL_DATA> GetVersionData(
            string templateCode,
            string centerCode,
            int? year = null,
            int? version = null)
        {
            ...
            {
                return UnitOfWork.Repository<OtherCostPLDataRepo>().GetPLDataByOrgCode(orgCode, year.Value, templateCode, version);
            }
            else if (lstChildren.Contains(template.ORG_CODE))
            {
                return UnitOfWork.Repository<OtherCostPLDataRepo>().GetPLDataByOrgCode(template.ORG_CODE, year.Value, templateCode, version);
            }
            else
            {
                return UnitOfWork.Repository<OtherCostPLDataRepo>().GetPLDataByCenterCode(template.ORG_CODE, new List<string> { centerCode }, year.Value, templateCode, version);
                // get 
            }
        }
        ...
        public IList<T_MD_COST_PL_ELEMENT> SummaryCenterOut(out IList<T_BP_OTHER_COST_PL_DATA> plDataOtherCostElements,
                                                              string centerCode,
                                                              int year,
                                                              int? version,
                                                              bool? isHasValue = null)
        ...
        /// <returns></returns>
        public IList<T_MD_COST_PL_ELEMENT> SummaryCenterVersion(out IList<T_BP_OTHER_COST_PL_DATA> plDataOtherCostElements,
                                                              string centerCode,
                                                              int year,
                                                              int? version,
        ...
        /// <returns></returns>
        private IList<T_BP_OTHER_COST_PL_DATA> GetAllSumUpDetails(string centerCode, int year, int version)
        {
            var detailsSumUp = UnitOfWork.Repository<OtherCostPLSumUpDetailRepo>()
               .GetManyWithFetch(x => x.ORG_CODE == centerCode && x.TIME_YEAR == year && x.SUM_UP_VERSION == version);

            var lstChildren = GetListOfChildrenCenter(centerCode).Select(x => x.CODE);
            var lstDetails = new List<T_BP_OTHER_COST_PL_DATA>();
            var lstResult = new List<T_BP_OTHER_COST_PL_DATA>();
            var plDataRepo = UnitOfWork.Repository<OtherCostPLDataRepo>();
            ...
            var lookupElement = lstDetails.ToLookup(x => x.COST_PL_ELEMENT_CODE);
                ...
                else
                {
                    ...
                    foreach (var orgCode in lookupCenter.Select(x => x.Key))
                    {
                        ...
                        else
                        {
                            ...
                            lstResult.Add(new T_BP_OTHER_COST_PL_DATA
                            {
                                ORG_CODE = orgCode,
                                COST_PL_ELEMENT_CODE = key,
        ...
        private IList<T_MD_COST_PL_ELEMENT> SummaryCenter(IList<T_BP_OTHER_COST_PL_DATA> plDataOtherCostElements, string centerCode, int year,
                                                          bool? isHasValue = null)
        {

            // get all revenue elements
            var allOtherCostElement = UnitOfWork.Repository<CostPLElementRepo>().GetManyByExpression(x => x.TIME_YEAR == year);
            // get all child
            ...
            var lookupElements = plDataOtherCostElements.GroupBy(x => x.COST_PL_ELEMENT_CODE)
                .Select(x => x.First())
        ...
        /// </summary>
        /// <param name="costPLData">List revenue pl data containt value</param>
        ...
        private IEnumerable<T_MD_COST_PL_ELEMENT> GetSumDescendants(
            IList<T_BP_OTHER_COST_PL_DATA> costPLData,
            ...
            string parentId)
        {
            ...
            while (st.Count > 0)
            {
                ...
                bool shouldReturn = true;
                // lst to store children of item which have children
                var lstOtherCostElements = new List<T_MD_COST_PL_ELEMENT>();
                // loop through items which have parent id = item id
                foreach (var i in lookup[item.CODE])
                {
                    if (lookup[i.CODE].Count() > 0)
                ...
                {
                    var treeData = costPLData.Where(x => x.COST_PL_ELEMENT_CODE.Equals(item.CODE));
                    if (treeData != null && treeData.Count() > 0)
                    {
                        item.Values[0] += treeData.Sum(x => x.VALUE_JAN) ?? 0;
                        item.Values[1] += treeData.Sum(x => x.VALUE_FEB) ?? 0;
                    ...
                    var clone = (T_MD_COST_PL_ELEMENT)item.Clone();
                    lstResult.Add(clone);
                }
                // remove all child of item
                // include children have child
                ...
                if (shouldReturn)
                {
                    ...
                    {
                        var data = costPLData.Where(x => x.COST_PL_ELEMENT_CODE == item.CODE && x.ORG_CODE == item.CENTER_CODE);
                        if (data != null)
                        {
                            item.Values = new decimal[14]
                            {
        ...
        public override void GenerateExportExcel(ref MemoryStream outFileStream, string html, string path, int year, string centerCode, int? version, string templateId, string unit, decimal exchangeRate)
        {
            ...
            workbook.SetSheetName(0, ModulType.GetTextSheetName(ModulType.OtherCostPL));
            fs.Close();
        ...
        {
            var name = "DỮ LIỆU KẾ HOẠCH CHI PHÍ KINH DOANH KHÁC CỦA DỰ ÁN";
        ...
        {
            return UnitOfWork.Repository<TypeRepo>().GetFirstWithFetch(x => x.OBJECT_TYPE == TemplateObjectType.Project && x.ELEMENT_TYPE == ElementType.ChiPhi && x.BUDGET_TYPE == BudgetType.KinhDoanh);
        }
    }
}
