...
using SMO.Repository.Implement.BP.REVENUE_PL.REVENUE_PL_DATA_BASE;
using SMO.Repository.Implement.MD;
using SMO.Service.BP.REVENUE_PL;
using SMO.Service.Class;
using SMO.Service.Common;
using SMO.ServiceInterface.BP.RevenuePL;
...
{
    public class RevenuePLService : BaseBPService<T_BP_REVENUE_PL, RevenuePLRepo, T_MD_REVENUE_PL_ELEMENT, T_BP_REVENUE_PL_VERSION, T_BP_REVENUE_PL_HISTORY, RevenuePLHistoryRepo>, IRevenuePLService
    {
        ...
        public List<T_BP_REVENUE_PL_HISTORY> ObjListHistory { get; set; }
        public List<T_BP_REVENUE_PL_VERSION> ObjListVersion { get; set; }
        public List<T_BP_REVENUE_PL_SUM_UP_DETAIL> ObjListSumUpHistory { get; set; }
        public RevenuePLService()
        {
            this.StartRowData = 6;
            ...
            this.ObjListHistory = new List<T_BP_REVENUE_PL_HISTORY>();
            this.ObjListVersion = new List<T_BP_REVENUE_PL_VERSION>();
            this.ObjListSumUpHistory = new List<T_BP_REVENUE_PL_SUM_UP_DETAIL>();
        }
        ...
        public override bool ShowReviewBtn(int year)
        {
            ...
            else
            {
                ...
                var historyReview = UnitOfWork.Repository<RevenuePLReviewRepo>()
        ...
        public override void TrinhDuyet(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<RevenuePLHistoryRepo>()
                    .Create(new T_BP_REVENUE_PL_HISTORY()
                    {
                ...
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_REVENUE_PL_DATA SET STATUS = '{Approve_Status.ChoPheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void HuyTrinhDuyet(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<RevenuePLHistoryRepo>().Create(new T_BP_REVENUE_PL_HISTORY()
                {
                    PKID = Guid.NewGuid().ToString(),
                    ORG_CODE = ObjDetail.ORG_CODE,
                    TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_REVENUE_PL_DATA SET STATUS = '{Approve_Status.ChuaTrinhDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void PheDuyet(string code)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_REVENUE_PL_DATA SET STATUS = '{Approve_Status.DaPheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void HuyNop(string code)
        {
            ...
            // get history
            var lstHistory = UnitOfWork.Repository<RevenuePLHistoryRepo>()
            ...
            if (lstHistory.Count == 1)
            {
                ...
                try
                {
                    ...
                    UnitOfWork.Repository<RevenuePLVersionRepo>().Update(x => x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.VERSION == ObjDetail.VERSION, x => x.IS_DELETED = 1, x => x.UPDATE_BY = currentUser);
        ...
        public override void TuChoi(string code)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_REVENUE_PL_DATA SET STATUS = '{Approve_Status.TuChoi}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void ChuyenTKS(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<RevenuePLReviewRepo>()
        ...
        public override void KetThucThamDinh(string orgCode, int year, int version)
        {
            try
            {
                ...
                UnitOfWork.BeginTransaction();
                var currentUser = ProfileUtilities.User?.USER_NAME;
                var reviewHeader = UnitOfWork.Repository<RevenuePLReviewRepo>()
                ...
                if (reviewHeader != null)
                {
                    ...
                    UnitOfWork.Repository<RevenuePLReviewRepo>().Update(reviewHeader);
                }
                var totalReviewUsers = UnitOfWork.Repository<UserReviewRepo>()
                    .GetManyWithFetch(x => x.TIME_YEAR == ObjDetail.TIME_YEAR).Count;

                var totalUsersEndReview = UnitOfWork.Repository<RevenuePLReviewRepo>()
                ...
                var review = UnitOfWork.Repository<RevenuePLReviewRepo>()
                ...
                if (review == null)
                {
                    ...
                    UnitOfWork.Repository<RevenuePLReviewRepo>().Create(
                        new T_BP_REVENUE_PL_REVIEW
                        {
                            PKID = reviewId,
                    ...
                    var plReviewService = new RevenuePLReviewService();
                    plReviewService.ObjDetail.TIME_YEAR = year;
                    plReviewService.ObjDetail.ORG_CODE = orgCode;
                    var dataCost = plReviewService.SummaryCenterVersion(out IList<T_BP_REVENUE_PL_DATA> detailCostElements);
                    ...
                    UnitOfWork.Repository<RevenuePLReviewResultRepo>()
                        .Create((from e in elements.Where(x => !x.IS_GROUP)
                                 ...
                                 {
                                     PKID = Guid.NewGuid().ToString(),
                                     REVENUE_PL_ELEMENT_CODE = e.CODE,
                                     HEADER_ID = reviewId,
                                     RESULT = false,
                                     TIME_YEAR = year,
                                     CREATE_BY = currentUser
                ...
                {
                    review.IS_END = true;
                    UnitOfWork.Repository<RevenuePLReviewRepo>().Update(review);
                }
        ...
        public override void PheDuyetTongKiemSoat(string orgCode, int year, int version)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_REVENUE_PL_DATA SET STATUS = '{Approve_Status.TKS_PheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        internal override void TuChoiTongKiemSoat(string orgCode, int year, int version, string comment)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_REVENUE_PL_DATA SET STATUS = '{Approve_Status.TKS_TuChoi}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
                UnitOfWork.Repository<RevenuePLReviewRepo>()
        ...
        public override void TrinhDuyetTongKiemSoat(string orgCode, int year, int version)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_REVENUE_PL_DATA SET STATUS = '{Approve_Status.TKS_TrinhDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void ChuyenHDNS(string code)
        {
            try
            {
                ...
                {
                    UnitOfWork.Repository<RevenuePLReviewRepo>().Update(x => x.DATA_VERSION == ObjDetail.VERSION
        ...
        public override void GetListOfChild()
        {
            ...
            // Tìm mẫu nộp hộ
            var listTemplateCode = this.UnitOfWork.Repository<TemplateDetailRevenuePLRepo>().Queryable().Where(
        ...
        {
            var query = UnitOfWork.GetSession().QueryOver<T_BP_REVENUE_PL_SUM_UP_DETAIL>();
        ...
        /// <returns></returns>
        private (IList<T_BP_REVENUE_PL_DATA>, bool) GetDataSumUp(int year, string centerCode, string elementCode, int sumUpVersion)
        {
            var plDataRepo = UnitOfWork.Repository<RevenuePLDataRepo>();
            var revenuePlReviewCommentRepo = UnitOfWork.Repository<RevenuePLReviewCommentRepo>();
            var plVersionRepo = UnitOfWork.Repository<RevenuePLRepo>();
            var plDataHistoryRepo = UnitOfWork.Repository<RevenuePLDataHistoryRepo>();

            var lstDetails = UnitOfWork.Repository<RevenuePLSumUpDetailRepo>()
            ...
            var data = new List<T_BP_REVENUE_PL_DATA>();
            foreach (var key in lookup.Select(x => x.Key))
            {
                var revenuePL = new List<T_BP_REVENUE_PL_DATA>();
                ...
                if (isLeaf)
                {
                    foreach (var item in lookup[key])
                    {
                        ...
                        {
                            // đơn vị con tự nộp
                            revenuePL.AddRange(plDataRepo.GetPLDataByCenterCode(item.FROM_ORG_CODE, lstChildren.ToList(), year, item.TEMPLATE_CODE, item.DATA_VERSION).ToList());
                        }
                        else
                        {
                            // được nộp hộ
                            var dataFromOrg = plDataRepo.GetPLDataByOrgCode(item.FROM_ORG_CODE, year, item.TEMPLATE_CODE, item.DATA_VERSION).ToList();
                            revenuePL.AddRange(dataFromOrg.Where(x => lstChildren.Contains(x.ORG_CODE)));
                        }
                    }
                }
                else
                {
                    if (isCorp)
                    {
                        foreach (var item in lookup[key])
                        {
                            revenuePL.AddRange(plDataRepo.GetPLDataByOrgCode(key, year, string.Empty, item.DATA_VERSION).ToList());
                        }
                    }
                    else
                    {
                        revenuePL = plDataRepo.GetPLDataByCenterCode(key, lstChildren.ToList(), year, null, sumUpVersion).ToList();
                    }
                }
                data.AddRange(revenuePL.Where(x => x.REVENUE_PL_ELEMENT_CODE == elementCode &&
        ...
        public override IEnumerable<T_MD_REVENUE_PL_ELEMENT> GetDetailSumUp(string centerCode, string elementCode, int year, int version, int? sumUpVersion, bool isCountComments, bool? isShowFile = null)
        {
            var plDataRepo = UnitOfWork.Repository<RevenuePLDataRepo>();
            var revenuePlReviewCommentRepo = UnitOfWork.Repository<RevenuePLReviewCommentRepo>();
            var plVersionRepo = UnitOfWork.Repository<RevenuePLRepo>();
            ...
            if (IsLeaf(centerCode))
            {
                ...
                foreach (var key in lookupData.Select(x => x.Key))
                {
                    ...
                    {
                        var isNewestVersion = UnitOfWork.Repository<RevenuePLVersionRepo>()
                        ...
                        foreach (var item in lookupData[key])
                        {
                            ...
                            var element = (T_MD_REVENUE_PL_ELEMENT)item;
                            element.IsBase = true;
                        ...
                        {
                            // get data from base
                            var baseData = UnitOfWork.Repository<RevenuePLDataBaseRepo>()
                                .GetManyWithFetch(x => x.REVENUE_PL_ELEMENT_CODE == elementCode && x.ORG_CODE == centerCode && x.TEMPLATE_CODE == key && x.VERSION == lookupData[key].First().VERSION && x.TIME_YEAR == year);
                            foreach (var item in baseData)
                            {
                                yield return (T_MD_REVENUE_PL_ELEMENT)item;
                            }
                        }
                        else
                        {
                            // get data from base history
                            var baseDataHistory = UnitOfWork.Repository<RevenuePLDataBaseHistoryRepo>()
                                .GetManyWithFetch(x => x.REVENUE_PL_ELEMENT_CODE == elementCode && x.ORG_CODE == centerCode && x.TEMPLATE_CODE == key && x.VERSION == lookupData[key].First().VERSION && x.TIME_YEAR == year);
            ...
            {
                var comments = new List<T_BP_REVENUE_PL_REVIEW_COMMENT>();
                if (isCountComments)
                {
                    comments = revenuePlReviewCommentRepo.GetManyWithFetch(
                            x => x.TIME_YEAR == year &&
                            x.ORG_CODE == GetCorp().CODE &&
                            x.REVENUE_PL_ELEMENT_CODE == elementCode).ToList();
                }
        ...
        public override IEnumerable<T_MD_REVENUE_PL_ELEMENT> GetDetailSumUpTemplate(string elementCode, int year, int version, string templateCode, string centerCode)
        {
            ...
            else
            {
                if (newestVersionInDb.Value == version)
                {
                    ...
                    var baseData = UnitOfWork.Repository<RevenuePLDataBaseRepo>()
                        ...
                        && x.TIME_YEAR == year
                        && x.REVENUE_PL_ELEMENT_CODE == elementCode
                        && x.VERSION == version
                        && x.PROFIT_CENTER_CODE == centerCode);
                    return from item in baseData
                           select (T_MD_REVENUE_PL_ELEMENT)item;
                }
                else
                {
                    // old data
                    // get data in table base data history
                    var baseData = UnitOfWork.Repository<RevenuePLDataBaseHistoryRepo>()
                        .GetManyWithFetch(x => x.TEMPLATE_CODE == templateCode
                        && x.TIME_YEAR == year
                        && x.REVENUE_PL_ELEMENT_CODE == elementCode
                        && x.VERSION == version
        ...
        public IList<T_MD_REVENUE_PL_ELEMENT> GetDataRevenue(out IList<T_MD_TEMPLATE_DETAIL_REVENUE_PL> detailRevenueElements,
            out IList<T_BP_REVENUE_PL_DATA> detailRevenueData,
            ...
            ViewDataCenterModel model)
        {
            ...
            if (!string.IsNullOrEmpty(model.TEMPLATE_CODE))
            {
                ...
                var sumElements = new T_MD_REVENUE_PL_ELEMENT
                {
                    // set tổng năm
                    // tạo element tổng
                    NAME = "TỔNG CỘNG",
        ...
        public override IList<T_MD_REVENUE_PL_ELEMENT> GetDetailPreviewSumUp(string centerCode, string elementCode, int year)
        {
            var plVersionRepo = UnitOfWork.Repository<RevenuePLRepo>();
            var plDataRepo = UnitOfWork.Repository<RevenuePLDataRepo>();
            var plDataHistoryRepo = UnitOfWork.Repository<RevenuePLDataHistoryRepo>();

            var plDataRevenueElements = UnitOfWork.Repository<RevenuePLDataRepo>().GetPLDataByCenterCode(null, new List<string> { centerCode }, year, null, null);

            return plDataRevenueElements.Where(x => x.REVENUE_PL_ELEMENT_CODE == elementCode)
                .Select(x => (T_MD_REVENUE_PL_ELEMENT)x)
                .OrderBy(x => x.C_ORDER).ToList();
        }

        private IList<T_MD_REVENUE_PL_ELEMENT> GetDataRevenuePreview(out IList<T_MD_TEMPLATE_DETAIL_REVENUE_PL> detailRevenueElements, string templateId, List<string> lstOrgs, int year, int? version, bool? isHasValue)
        {
            detailRevenueElements = new List<T_MD_TEMPLATE_DETAIL_REVENUE_PL>();
            var lstElements = new List<T_MD_REVENUE_PL_ELEMENT>();
            foreach (var orgCode in lstOrgs)
            {
                var elements = GetDataRevenuePreview(out IList<T_MD_TEMPLATE_DETAIL_REVENUE_PL> detail, templateId, orgCode, year, version, isHasValue);
        ...
        /// <returns></returns>
        private IList<T_MD_REVENUE_PL_ELEMENT> SummaryUpElement(IList<T_MD_REVENUE_PL_ELEMENT> lst1, IList<T_MD_REVENUE_PL_ELEMENT> lst2)
        {
            if (lst1.Count == 0)
            {
                return lst2;
        ...
        public override T_BP_REVENUE_PL_VERSION GetHeader(string templateCode, string centerCode, int year, int? version)
        {
        ...
        {
            ObjListVersion = UnitOfWork.Repository<RevenuePLVersionRepo>()
        ...
        {
            var query = this.UnitOfWork.GetSession().QueryOver<T_BP_REVENUE_PL_HISTORY>();
        ...
        public override void GetHistory(string orgCode, string templateId, int? year)
        {
            if (year.HasValue)
            {
                this.ObjListHistory = UnitOfWork.Repository<RevenuePLHistoryRepo>().GetManyByExpression(
        ...
        public override void ValidateData(DataTable dataTable, bool isDataBase)
        {
            ...
            var lstDetailTemplate = this.UnitOfWork.Repository<TemplateDetailRevenuePLRepo>().GetManyByExpression(x => x.TIME_YEAR == this.ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == this.ObjDetail.TEMPLATE_CODE);

            var lstElement = this.UnitOfWork.Repository<RevenuePLElementRepo>().GetManyByExpression(x => x.TIME_YEAR == this.ObjDetail.TIME_YEAR);
        ...
        public override void ConvertData(DataTable dataTable, List<T_MD_REVENUE_PL_ELEMENT> lstElement, int startColumn, int endColumn, bool isDataBase)
        {
        ...
        public override void ImportExcel(HttpRequestBase request)
        {
            ...
            var revenuePLCurrent = CurrentRepository.Queryable().FirstOrDefault(x => x.ORG_CODE == orgCode
                && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE);

            if (revenuePLCurrent != null && !(revenuePLCurrent.STATUS == Approve_Status.TuChoi || revenuePLCurrent.STATUS == Approve_Status.ChuaTrinhDuyet))
            {
                this.State = false;
            ...
            var versionNext = 1;
            if (revenuePLCurrent != null)
            {
                versionNext = revenuePLCurrent.VERSION + 1;
            }

            // Lấy dữ liệu của version hiện tại
            var dataCurrent = new List<T_BP_REVENUE_PL_DATA>();
            if (revenuePLCurrent != null)
            {
                dataCurrent = UnitOfWork.Repository<RevenuePLDataRepo>().Queryable().Where(x => x.ORG_CODE == orgCode
            ...
            try
            {
                ...
                if (revenuePLCurrent != null)
                {
                    ...
                    revenuePLCurrent.VERSION = versionNext;
                    revenuePLCurrent.IS_DELETED = false;
                    CurrentRepository.Update(revenuePLCurrent);
                }
                else
                {
                    // Tạo mới bản ghi revenue pl
                    CurrentRepository.Create(new T_BP_REVENUE_PL()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                        TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                UnitOfWork.Repository<RevenuePLVersionRepo>().Create(new T_BP_REVENUE_PL_VERSION()
                {
                    PKID = Guid.NewGuid().ToString(),
                    ORG_CODE = orgCode,
                    TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                {
                    var revenueDataHis = (T_BP_REVENUE_PL_DATA_HISTORY)item;
                    UnitOfWork.Repository<RevenuePLDataHistoryRepo>().Create(revenueDataHis);
                    UnitOfWork.Repository<RevenuePLDataRepo>().Delete(item);
                }
                ...
                {
                    var revenueData = new T_BP_REVENUE_PL_DATA()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                        ...
                        STATUS = Approve_Status.ChuaTrinhDuyet,
                        VERSION = versionNext,
                        REVENUE_PL_ELEMENT_CODE = tableData.Rows[i][6].ToString().Trim(),
                    ...
                    UnitOfWork.Repository<RevenuePLDataRepo>().Create(revenueData);
                }
                UnitOfWork.Commit();
                NotifyUtilities.CreateNotify(
                    new NotifyPara()
        ...
        public override void ImportExcelBase(HttpRequestBase request)
        {
            ...
            var orgCode = ProfileUtilities.User?.ORGANIZE_CODE;
            var revenuePLDataRepo = UnitOfWork.Repository<RevenuePLDataRepo>();
            var revenuePLDataBaseRepo = UnitOfWork.Repository<RevenuePLDataBaseRepo>();
            var currentUser = ProfileUtilities.User?.USER_NAME;
            ...
            // Xác định version dữ liệu
            var revenuePLCurrent = GetFirstWithFetch(x => x.ORG_CODE == orgCode
                && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE);
            ...
            // Lấy dữ liệu của version hiện tại
            var dataCurrent = new List<T_BP_REVENUE_PL_DATA>();
            var dataCurrentBase = new List<T_BP_REVENUE_PL_DATA_BASE>();
            if (revenuePLCurrent != null)
            {
                dataCurrent = revenuePLDataRepo.GetManyWithFetch(x => x.ORG_CODE == orgCode
                    && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE).ToList();

                dataCurrentBase = revenuePLDataBaseRepo.GetManyWithFetch(x => x.ORG_CODE == orgCode
                    && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE).ToList();
            }
            ...
            try
            {
                ...
                foreach (var key in lookUp.Select(x => x.Key))
                {
                    var revenueData = new T_BP_REVENUE_PL_DATA()
                    {
                        ...
                        STATUS = Approve_Status.ChuaTrinhDuyet,
                        VERSION = versionNext,
                        REVENUE_PL_ELEMENT_CODE = key.ElementCode,
                        VALUE_JAN = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[12].ToString())),
                        VALUE_FEB = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[15].ToString())),
                        VALUE_MAR = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[18].ToString())),
                        VALUE_APR = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[21].ToString())),
                        VALUE_MAY = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[24].ToString())),
                        VALUE_JUN = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[27].ToString())),
                        VALUE_JUL = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[30].ToString())),
                        VALUE_AUG = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[33].ToString())),
                        VALUE_SEP = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[36].ToString())),
                        VALUE_OCT = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[39].ToString())),
                        VALUE_NOV = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[42].ToString())),
                        VALUE_DEC = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[45].ToString())),
                        CREATE_BY = currentUser
                    };
                    revenuePLDataRepo.Create(revenueData);
                }

                // Insert data vào base data history
                var lstBaseDataHistory = (from x in dataCurrentBase
                                          select (T_BP_REVENUE_PL_DATA_BASE_HISTORY)x).ToList();
                UnitOfWork.Repository<RevenuePLDataBaseHistoryRepo>().Create(lstObj: lstBaseDataHistory);
                revenuePLDataBaseRepo.Delete(dataCurrentBase);
                ...
                for (int i = this.StartRowData; i < actualRows; i++)
                {
                    ...
                    var j = 10;
                    var revenueData = new T_BP_REVENUE_PL_DATA_BASE()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                    ...
                    revenuePLDataBaseRepo.Create(revenueData);
                }
                UnitOfWork.Commit();
        ...
        public override IList<T_BP_REVENUE_PL_VERSION> GetVersions(string orgCode, string templateId, int year)
        {
        ...
        {
            return UnitOfWork.Repository<RevenuePLDataRepo>()
                .GetManyByExpression(x => lstOrgCodes.Contains(x.ORG_CODE)
                && x.TEMPLATE_CODE != string.Empty
                && (!year.HasValue || x.TIME_YEAR == year.Value))
                .Select(x => x.TEMPLATE_CODE)
                .Distinct()
                .ToList();
        }
        ...
        {
            return UnitOfWork.Repository<RevenuePLDataHistoryRepo>()
                .GetManyByExpression(x => lstOrgCodes.Contains(x.ORG_CODE)
                && x.TEMPLATE_CODE != string.Empty
                && (!year.HasValue || x.TIME_YEAR == year.Value))
                .Select(x => x.TEMPLATE_CODE)
        ...
        public override IList<int> GetTemplateVersion(string templateId, string centerCode, int year)
        {
            if (string.IsNullOrEmpty(templateId))
            {
                return UnitOfWork.Repository<RevenuePLSumUpDetailRepo>()
        ...
        {
            var dataRevenue = PreparePureList(out IList<T_MD_TEMPLATE_DETAIL_REVENUE_PL> detailRevenueElements, templateId, year, ignoreAuth: true);
            ...
            try
            {
                ...
                templateWorkbook.SetSheetName(0, ModulType.GetTextSheetName(ModulType.RevenuePL));
        ...
        {
            var dataCost = PreparePureList(out IList<T_MD_TEMPLATE_DETAIL_REVENUE_PL> detailCostElements, templateId, year, ignoreAuth: true);
        ...
        public override IList<NodeDataFlow> BuildDataFlowTree(string orgCode, int year, int? version, int? sumUpVersion)
        {
            ...
            if (revenueCenterRepo.GetManyByExpression(x => x.PARENT_CODE == orgCode).Count > 0 && version == null)
            {
                ...
                {
                    var revenuePLData = UnitOfWork.Repository<RevenuePLSumUpDetailRepo>()
            ...
            else
            {
                // level 2: department
                // get templates
                var sumupDetails = UnitOfWork.Repository<RevenuePLSumUpDetailRepo>()
        ...
        public IList<T_MD_REVENUE_PL_ELEMENT> GetDataRevenuePreview(
        ...
        public IList<T_MD_REVENUE_PL_ELEMENT> PreparePureList(IList<T_MD_TEMPLATE_DETAIL_REVENUE_PL> detailRevenueElements, int year)
        {
            // get all revenue elements
            var allRevenueElements = UnitOfWork.Repository<RevenuePLElementRepo>().GetManyByExpression(x => x.TIME_YEAR == year);
            ...
            var pureLstItems = new List<T_MD_REVENUE_PL_ELEMENT>();
                ...
                foreach (var code in lookupElements.Select(l => l.Key))
                {
                    ...
                    while (!string.IsNullOrEmpty(parentCode))
                    {
                        ...
                        if (element != null)
                        {
                            ...
                            lst.Add((T_MD_REVENUE_PL_ELEMENT)element.CloneObject());     // must to clone to other object because it reference to other center
                        }
                        else
                        {
                            break;
        ...
        /// <param name="centerCode"></param>
        /// <returns></returns>
        public IList<T_MD_REVENUE_PL_ELEMENT> PreparePureList(out IList<T_MD_TEMPLATE_DETAIL_REVENUE_PL> detailRevenueElements,
            string templateId,
            int year,
            string centerCode = "",
            bool ignoreAuth = false)
        {
            ...
            {
                detailRevenueElements = UnitOfWork.Repository<TemplateDetailRevenuePLRepo>()
        ...
        /// <param name="centerCode"></param>
        /// <returns></returns>
        public IList<T_MD_REVENUE_PL_ELEMENT> PreparePureListForTemplate(out IList<T_MD_TEMPLATE_DETAIL_REVENUE_PL> detailRevenueElements,
            string templateId,
            int year)
        {
        ...
        {
            // Tìm mẫu nộp hộ
            var listTemplateCodes = this.UnitOfWork.Repository<TemplateDetailRevenuePLRepo>()
                .GetManyByExpression(x => centerCodes.Contains(x.CENTER_CODE) && x.TIME_YEAR == year)
                .Select(x => x.TEMPLATE_CODE).Distinct().ToList();
            var findRevenuePL = this.CurrentRepository.GetManyByExpression(
                    x => listTemplateCodes.Contains(x.TEMPLATE_CODE) && !centerCodes.Contains(x.ORG_CODE));
            var lst = new List<T_MD_TEMPLATE_DETAIL_REVENUE_PL>();
            detailRevenueElements = new List<T_MD_TEMPLATE_DETAIL_REVENUE_PL>();
            foreach (var template in listTemplateCodes)
            {
                lst.AddRange(UnitOfWork.Repository<TemplateDetailRevenuePLRepo>()
        ...
        public override void SumUpDataCenter(out T_BP_REVENUE_PL_VERSION revenuePL, string centerCode, int year)
        {
            ...
            var lstData = new List<T_BP_REVENUE_PL_DATA>();
            revenuePL = new T_BP_REVENUE_PL_VERSION();

            try
            {
                UnitOfWork.BeginTransaction();
                var sumUpDetailRepo = UnitOfWork.Repository<RevenuePLSumUpDetailRepo>();
                var revenuePLDataRepo = UnitOfWork.Repository<RevenuePLDataRepo>();
                ...
                var lookup = revenuePlDataApproved.ToLookup(x => x.REVENUE_PL_ELEMENT_CODE);
                foreach (var code in lookup.Select(x => x.Key))
                {
                    // TODO: check if all value of months are equal 0
                    if (lookup[code].Count() == 1)
                    {
                        lstData.Add((T_BP_REVENUE_PL_DATA)lookup[code].First().CloneObject());
                    }
                    else
                    {
                        lstData.Add(new T_BP_REVENUE_PL_DATA
                        {
                            ...
                            REVENUE_PL_ELEMENT_CODE = lookup[code].First().REVENUE_PL_ELEMENT_CODE
                        });
                    }
                }

                // get current version in revenue pl data
                var newestPL = UnitOfWork.Repository<RevenuePLRepo>()
                ...
                UnitOfWork.Repository<RevenuePLVersionRepo>().Create(revenuePL);
                ...
                var lstRevenuePlDataOldVersion = revenuePLDataRepo
                ...
                _ = UnitOfWork.GetSession().CreateSQLQuery($"DELETE FROM T_BP_REVENUE_PL_DATA WHERE ORG_CODE = '{centerCode}' AND TIME_YEAR = {year}")
                    .ExecuteUpdate();

                // insert to pl data history
                UnitOfWork.Repository<RevenuePLDataHistoryRepo>().Create((from pl in lstRevenuePlDataOldVersion
                                                                          select (T_BP_REVENUE_PL_DATA_HISTORY)pl).ToList());
                ...
                var lstRevenuePL = revenuePlDataApproved.ToLookup(x => new { OrgCode = x.ORG_CODE, TemplateCode = x.TEMPLATE_CODE, TemplateVersion = x.VERSION });

                // create list sum up detail
                sumUpDetailRepo.Create((from c in lstRevenuePL.Select(x => x.Key)
                                        select new T_BP_REVENUE_PL_SUM_UP_DETAIL
                                        {
                                            PKID = Guid.NewGuid().ToString(),
        ...
        public IList<T_MD_REVENUE_PL_ELEMENT> SummarySumUpCenter(
            ...
            string templateId = "")
        {
            ...
            plDataRevenueElements = UnitOfWork.Repository<RevenuePLDataRepo>()
        ...
        private IEnumerable<T_MD_REVENUE_PL_ELEMENT> GetSumDescendants(
            IEnumerable<T_MD_TEMPLATE_DETAIL_REVENUE_PL> details,
            IEnumerable<T_MD_REVENUE_PL_ELEMENT> pureItems,
            string parent_id,
            string templateId,
            int? year = null,
            int? version = null)
        {
            var lookup = pureItems.ToLookup(x => x.CENTER_CODE);
            foreach (var centerCode in lookup.Select(x => x.Key))
            {
                var revenuePlData = GetVersionData(templateId, centerCode, year, version);

                Queue<T_MD_REVENUE_PL_ELEMENT> st = new Queue<T_MD_REVENUE_PL_ELEMENT>(lookup[parent_id]);
                ...
                while (st.Count > 0)
                {
                    ...
                    var lstHasChild = new List<T_MD_REVENUE_PL_ELEMENT>();
                    ...
                    foreach (var i in lookup[item.CODE])
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (detail != null)
                                {
                                    detail.PlData = revenuePlData.FirstOrDefault(x => x.REVENUE_PL_ELEMENT_CODE == detail.ELEMENT_CODE && x.PROFIT_CENTER_CODE == detail.CENTER_CODE);
                                }
                    ...
                    if (shouldReturn)
                    {
                        ...
                        {
                            var data = revenuePlData.Where(x => x.REVENUE_PL_ELEMENT_CODE == item.CODE && x.PROFIT_CENTER_CODE == item.CENTER_CODE);
                            if (data != null)
                            {
        ...
        /// <param name="year">Which year of data</param>
        /// <param name="version">Which version of data</param>
        /// <returns>Returns Revenue PL Data</returns>
        private IList<T_BP_REVENUE_PL_DATA> GetVersionData(
            string templateCode,
            string centerCode,
            int? year = null,
            int? version = null)
        {
            ...
            {
                return UnitOfWork.Repository<RevenuePLDataRepo>().GetPLDataByOrgCode(orgCode, year.Value, templateCode, version);
            }
            else if (lstChildren.Contains(template.ORG_CODE))
            {
                return UnitOfWork.Repository<RevenuePLDataRepo>().GetPLDataByOrgCode(template.ORG_CODE, year.Value, templateCode, version);
            }
            else
            {
                return UnitOfWork.Repository<RevenuePLDataRepo>().GetPLDataByCenterCode(template.ORG_CODE, new List<string> { centerCode }, year.Value, templateCode, version);
                // get 
            }
        }
        ...
        public IList<T_MD_REVENUE_PL_ELEMENT> SummaryCenterOut(out IList<T_BP_REVENUE_PL_DATA> plDataRevenueElements,
                                                              string centerCode,
                                                              int year,
                                                              int? version,
                                                              bool? isHasValue = null)
        ...
        /// <returns></returns>
        public IList<T_MD_REVENUE_PL_ELEMENT> SummaryCenterVersion(out IList<T_BP_REVENUE_PL_DATA> plDataRevenueElements,
                                                              string centerCode,
                                                              int year,
                                                              int? version,
        ...
        /// <returns></returns>
        private IList<T_BP_REVENUE_PL_DATA> GetAllSumUpDetails(string centerCode, int year, int version)
        {
            var detailsSumUp = UnitOfWork.Repository<RevenuePLSumUpDetailRepo>()
               .GetManyWithFetch(x => x.ORG_CODE == centerCode && x.TIME_YEAR == year && x.SUM_UP_VERSION == version);

            var lstChildren = GetListOfChildrenCenter(centerCode).Select(x => x.CODE);
            var lstDetails = new List<T_BP_REVENUE_PL_DATA>();
            var lstResult = new List<T_BP_REVENUE_PL_DATA>();
            var plDataRepo = UnitOfWork.Repository<RevenuePLDataRepo>();
            ...
            var lookupElement = lstDetails.ToLookup(x => x.REVENUE_PL_ELEMENT_CODE);
                ...
                else
                {
                    ...
                    foreach (var orgCode in lookupCenter.Select(x => x.Key))
                    {
                        ...
                        else
                        {
                            ...
                            lstResult.Add(new T_BP_REVENUE_PL_DATA
                            {
                                ORG_CODE = orgCode,
                                REVENUE_PL_ELEMENT_CODE = key,
        ...
        private IList<T_MD_REVENUE_PL_ELEMENT> SummaryCenter(IList<T_BP_REVENUE_PL_DATA> plDataRevenueElements, string centerCode, int year,
                                                          bool? isHasValue = null)
        {

            // get all revenue elements
            var allRevenueElement = UnitOfWork.Repository<RevenuePLElementRepo>().GetManyByExpression(x => x.TIME_YEAR == year);
            // get all child
            ...
            var lookupElements = plDataRevenueElements.GroupBy(x => x.REVENUE_PL_ELEMENT_CODE)
                .Select(x => x.First())
        ...
        /// </summary>
        /// <param name="revenuePLData">List revenue pl data containt value</param>
        ...
        private IEnumerable<T_MD_REVENUE_PL_ELEMENT> GetSumDescendants(
            IList<T_BP_REVENUE_PL_DATA> revenuePLData,
            IList<T_MD_REVENUE_PL_ELEMENT> pureItems,
            string parentId)
        {
            // set tổng năm
            // tạo element tổng
            var lstResult = new List<T_MD_REVENUE_PL_ELEMENT>
            {
                // set tổng năm
                // tạo element tổng
                new T_MD_REVENUE_PL_ELEMENT
                {
                    NAME = "TỔNG CỘNG",
                    LEVEL = 0,
                    PARENT_CODE = null,
                    ...
                    Values = new decimal[13]
                    {
                        revenuePLData.Sum(x => x.VALUE_JAN) ?? 0,
                        revenuePLData.Sum(x => x.VALUE_FEB) ?? 0,
                        revenuePLData.Sum(x => x.VALUE_MAR) ?? 0,
                        revenuePLData.Sum(x => x.VALUE_APR) ?? 0,
                        revenuePLData.Sum(x => x.VALUE_MAY) ?? 0,
                        revenuePLData.Sum(x => x.VALUE_JUN) ?? 0,
                        revenuePLData.Sum(x => x.VALUE_JUL) ?? 0,
                        revenuePLData.Sum(x => x.VALUE_AUG) ?? 0,
                        revenuePLData.Sum(x => x.VALUE_SEP) ?? 0,
                        revenuePLData.Sum(x => x.VALUE_OCT) ?? 0,
                        revenuePLData.Sum(x => x.VALUE_NOV) ?? 0,
                        revenuePLData.Sum(x => x.VALUE_DEC) ?? 0,
                        0
                    }
                }
            };
            ...
            Queue<T_MD_REVENUE_PL_ELEMENT> st = new Queue<T_MD_REVENUE_PL_ELEMENT>(lookup[parentId]);
            while (st.Count > 0)
            {
                // get first item in queue
                var item = st.Dequeue();
                ...
                bool shouldReturn = true;
                // lst to store children of item which have children
                var lstRevenueElements = new List<T_MD_REVENUE_PL_ELEMENT>();
                // loop through items which have parent id = item id
                foreach (var i in lookup[item.CODE])
                {
                    if (lookup[i.CODE].Count() > 0)
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            var treeData = revenuePLData.Where(x => x.REVENUE_PL_ELEMENT_CODE.Equals(i.CODE));
                            if (treeData != null && treeData.Count() > 0)
                            {
                                ...
                                foreach (var d in treeData)
                                {
                                    ...
                                    i.Values = values;
                                    var clone = (T_MD_REVENUE_PL_ELEMENT)i.Clone();
                                    //yield return clone;
                                    lstResult.Add(clone);
                                    break;
                                }
                ...
                if (shouldReturn)
                {
                    ...
                    {
                        var data = revenuePLData.Where(x => x.REVENUE_PL_ELEMENT_CODE == item.CODE && x.ORG_CODE == item.CENTER_CODE);
                        if (data != null)
                        {
                            item.Values = new decimal[12]
                            {
        ...
        public override void GenerateExportExcel(ref MemoryStream outFileStream, string html, string path, int year, string centerCode, int? version, string templateId, string unit, decimal exchangeRate)
        {
            ...
            workbook.SetSheetName(0, ModulType.GetTextSheetName(ModulType.RevenuePL));
        ...
        {
            var name = "DỮ LIỆU KẾ HOẠCH DOANH THU KINH DOANH";
        ...
        {
            return UnitOfWork.Repository<TypeRepo>().GetFirstWithFetch(x => x.OBJECT_TYPE == TemplateObjectType.Department && x.ELEMENT_TYPE == ElementType.DoanhThu && x.BUDGET_TYPE == BudgetType.KinhDoanh);
        }
    }
}
