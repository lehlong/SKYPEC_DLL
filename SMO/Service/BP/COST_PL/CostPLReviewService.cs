...
{
    public class CostPLReviewService : BaseBPReviewService<T_BP_COST_PL_REVIEW, T_BP_COST_PL_REVIEW_RESULT, CostPLReviewRepo>
    {
        /// <summary>
        /// List lịch sử thẩm định và tổng kiểm soát
        /// </summary>
        public IList<T_BP_COST_PL_HISTORY> ListHistory { get; set; }
        /// <summary>
        /// List tổng kiểm soát dữ liệu
        /// </summary>
        public IList<T_BP_COST_PL_REVIEW> ListControl { get; set; }

        public CostPLReviewService()
        {
            ListHistory = new List<T_BP_COST_PL_HISTORY>();
            ListControl = new List<T_BP_COST_PL_REVIEW>();
        }
        /// <summary>
        ...
        internal ReviewCenterViewModelBase<CostPLElementReviewCenter> SummaryReviewDataCenter(ReviewDataCenterModel model)
        {
            var corp = GetCorp()?.CODE;

            var dataCost = SummaryCenterVersion(out _, model, corp);
            dataCost = dataCost.OrderBy(x => x.C_ORDER)
                                            .GroupBy(x => x.CODE)
                                            .Select(x => x.First()).ToList();

            return new ReviewCenterViewModelBase<CostPLElementReviewCenter>
            {
        ...
        private IList<CostPLElementReviewCenter> PrepareListReviewSummary(IList<T_MD_COST_PL_ELEMENT> dataCost, ReviewDataCenterModel model, string corp)
        {
            if (!model.IS_COMPLETED && !model.IS_NOT_COMPLETED)
            {
                return new List<CostPLElementReviewCenter>();
            }
            ...
            var comments = UnitOfWork.Repository<CostPLReviewCommentRepo>()
            ...
            if (lstReviews == null || lstReviews.Count == 0)
            {
                ...
                else
                {
                    return (from d in dataCost
                            ...
                            select new CostPLElementReviewCenter(
                                d,
                                null,
                                null,
                                null,
            ...
            var lstReviewResult = UnitOfWork.Repository<CostPLReviewResultRepo>()
        ...
        internal T_BP_COST_PL GetHeader()
        {
            return new CostPLService().GetBPHeader(string.Empty, ObjDetail.DATA_VERSION, ObjDetail.TIME_YEAR, GetCorp().CODE);
        }
        ...
        internal void SearchReview()
        {
            ...
            // Dữ liệu thẩm định
            ObjList = UnitOfWork.Repository<CostPLReviewRepo>().GetManyWithFetch(x => x.TIME_YEAR == ObjDetail.TIME_YEAR
        ...
        internal IList<T_BP_COST_PL_HISTORY> GetHistory(int year, IEnumerable<string> lstActionsFilter, int? version = null)
        {
            var corp = GetCorp().CODE;
            if (version == null)
            {
                return UnitOfWork.Repository<CostPLHistoryRepo>()
        ...
        /// <returns></returns>
        private T_BP_COST_PL_REVIEW GetControlReview(ReviewDataCenterModel model, string org)
        {
            T_BP_COST_PL_REVIEW objReview = null;
            if (model.IS_CONTROL)
            {
        ...
        /// <param name="org"></param>
        /// <returns></returns>
        private IList<T_BP_COST_PL_REVIEW> GetCouncils(ReviewDataCenterModel model, string org)
        {
            IList<T_BP_COST_PL_REVIEW> lstReviews = null;
        ...
        internal void UpdateReview(CostPLReviewViewModel model)
        {
            // check budget period
            if (model.IsSummary)
            {
            ...
            if (resultCheckConstraint)
            {
                try
                {
                    ...
                    T_BP_COST_PL_REVIEW headerFound;
                    ...
                    if (headerFound == null)
                    {
                        ...
                        ObjDetail = new T_BP_COST_PL_REVIEW
                        {
                            PKID = headerId,
                    ...
                    else
                    {
                        ...
                        if (model.Status.HasValue)
                        {
                            ...
                            UnitOfWork.Repository<CostPLReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId) && x.RESULT == model.Status.Value);
                        }
                        else
                        {
                            UnitOfWork.Repository<CostPLReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId));
                        }
                    }

                    // create history
                    var history = new T_BP_COST_PL_REVIEW_HISTORY
                    {
                    ...
                    UnitOfWork.Repository<CostPLReviewHistoryRepo>().Create(history);

                    // create history
                    var historyPL = new T_BP_COST_PL_HISTORY
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = model.OrgCode,
                        TIME_YEAR = model.Year,
                    ...
                    UnitOfWork.Repository<CostPLHistoryRepo>().Create(historyPL);
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private bool CheckConstraintComments(CostPLReviewViewModel model, out IDictionary<string, string> elementsNotValid)
        {
            ...
            var queryable = UnitOfWork.GetSession().Query<T_BP_COST_PL_REVIEW_COMMENT>();
        ...
        {
            var corp = GetCorp()?.CODE;
            return UnitOfWork.Repository<CostPLVersionRepo>().GetManyByExpression(x => x.TIME_YEAR == year && x.ORG_CODE == corp)
                .Select(x => x.VERSION)
                .Distinct()
        ...
        internal IList<T_MD_COST_PL_ELEMENT> SummaryCenterVersion(out IList<T_BP_COST_PL_DATA> detailCostElements, int? version = null)
        {
            return new CostPLService().SummaryCenterVersion(out detailCostElements, ObjDetail.ORG_CODE, ObjDetail.TIME_YEAR, version);
        }

        internal IList<T_MD_COST_PL_ELEMENT> SummaryCenterVersion(out IList<T_BP_COST_PL_DATA> detailCostElements, ReviewDataCenterModel model, string corp)
        {
            return new CostPLService().SummaryCenterVersion(out detailCostElements, corp, model.YEAR, model.VERSION);
        }

        internal IList<CostPLElementReview> PrepareListReview(IList<T_MD_COST_PL_ELEMENT> dataCost)
        {
            var comments = UnitOfWork.Repository<CostPLReviewCommentRepo>()
                    .GetManyByExpression(x =>
                    x.ORG_CODE == ObjDetail.ORG_CODE &&
            ...
            var objReview = new T_BP_COST_PL_REVIEW();
            if (!IsReview)
            {
                objReview = GetFirstByExpression(
            ...
            return (from d in dataCost
                    ...
                    let elementComments = comments.Where(x => x.COST_PL_ELEMENT_CODE == d.CODE)
                    let elementCommentsInOrg = elementComments.Where(x => x.ON_ORG_CODE == d.ORG_CODE)
                    select new CostPLElementReview(
                        d,
                        result?.RESULT,
                        elementComments.Sum(x => x.NUMBER_COMMENTS),
                        elementCommentsInOrg.Sum(x => x.NUMBER_COMMENTS)
