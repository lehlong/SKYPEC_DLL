...
using SMO.Repository.Implement.BP.CONTRUCT_COST_PL.CONTRUCT_COST_PL_DATA_BASE;
using SMO.Repository.Implement.BP.COST_PL;
using SMO.Repository.Implement.MD;
using SMO.Service.BP.COST_PL;
using SMO.Service.Class;
using SMO.Service.Common;
using SMO.ServiceInterface.BP.ContructCostPL;
...
{
    public class ContructCostPLService : BaseBPService<T_BP_CONTRUCT_COST_PL, ContructCostPLRepo, T_MD_COST_PL_ELEMENT, T_BP_CONTRUCT_COST_PL_VERSION, T_BP_CONTRUCT_COST_PL_HISTORY, ContructCostPLHistoryRepo>, IContructCostPLService
    {
        ...
        public List<T_BP_CONTRUCT_COST_PL_HISTORY> ObjListHistory { get; set; }
        public List<T_BP_CONTRUCT_COST_PL_VERSION> ObjListVersion { get; set; }
        public List<T_BP_CONTRUCT_COST_PL_SUM_UP_DETAIL> ObjListSumUpHistory { get; set; }
        public ContructCostPLService()
        {
            this.StartRowData = 6;
            ...
            this.ObjListHistory = new List<T_BP_CONTRUCT_COST_PL_HISTORY>();
            this.ObjListVersion = new List<T_BP_CONTRUCT_COST_PL_VERSION>();
            this.ObjListSumUpHistory = new List<T_BP_CONTRUCT_COST_PL_SUM_UP_DETAIL>();
        }
        ...
        public override bool ShowReviewBtn(int year)
        {
            ...
            else
            {
                ...
                var historyReview = UnitOfWork.Repository<ContructCostPLReviewRepo>()
        ...
        private void SendNotify(Activity activity)
        {
            NotifyUtilities.CreateNotify(
                    new NotifyPara()
                    {
                        ...
                        ModulType = ModulType.ContructCostPL,
        ...
        public override void TrinhDuyet(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<ContructCostPLHistoryRepo>()
                    .Create(new T_BP_CONTRUCT_COST_PL_HISTORY()
                    {
                ...
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_CONTRUCT_COST_PL_DATA SET STATUS = '{Approve_Status.ChoPheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void HuyTrinhDuyet(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<ContructCostPLHistoryRepo>().Create(new T_BP_CONTRUCT_COST_PL_HISTORY()
                {
                    PKID = Guid.NewGuid().ToString(),
                    ORG_CODE = ObjDetail.ORG_CODE,
                    TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_CONTRUCT_COST_PL_DATA SET STATUS = '{Approve_Status.ChuaTrinhDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void PheDuyet(string code)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_CONTRUCT_COST_PL_DATA SET STATUS = '{Approve_Status.DaPheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void HuyNop(string code)
        {
            ...
            try
            {
                ...
                UnitOfWork.Repository<ContructCostPLVersionRepo>().Update(x => x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.VERSION == ObjDetail.VERSION, x => x.IS_DELETED = 1, x => x.UPDATE_BY = currentUser);
        ...
        public override void TuChoi(string code)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_CONTRUCT_COST_PL_DATA SET STATUS = '{Approve_Status.TuChoi}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void ChuyenTKS(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<ContructCostPLReviewRepo>()
        ...
        public override void KetThucThamDinh(string orgCode, int year, int version)
        {
            try
            {
                ...
                UnitOfWork.BeginTransaction();
                var currentUser = ProfileUtilities.User?.USER_NAME;
                var reviewHeader = UnitOfWork.Repository<ContructCostPLReviewRepo>()
                ...
                if (reviewHeader != null)
                {
                    ...
                    UnitOfWork.Repository<ContructCostPLReviewRepo>().Update(reviewHeader);
                }

                var totalReviewUsers = UnitOfWork.Repository<UserReviewRepo>()
                    .GetManyWithFetch(x => x.TIME_YEAR == ObjDetail.TIME_YEAR).Count;

                var totalUsersEndReview = UnitOfWork.Repository<ContructCostPLReviewRepo>()
                ...
                var review = UnitOfWork.Repository<ContructCostPLReviewRepo>()
                ...
                if (review == null)
                {
                    ...
                    UnitOfWork.Repository<ContructCostPLReviewRepo>().Create(
                        new T_BP_CONTRUCT_COST_PL_REVIEW
                        {
                            PKID = reviewId,
                    ...
                    var plReviewService = new ContructCostPLReviewService();
                    plReviewService.ObjDetail.TIME_YEAR = year;
                    plReviewService.ObjDetail.ORG_CODE = orgCode;
                    var dataCost = plReviewService.SummaryCenterVersion(out IList<T_BP_CONTRUCT_COST_PL_DATA> detailCostElements);
                    dataCost = dataCost.OrderBy(x => x.C_ORDER)
                                                    .GroupBy(x => x.CODE)
                                                    .Select(x => x.First()).ToList();
                    var elements = plReviewService.PrepareListReview(dataCost);
                    UnitOfWork.Repository<ContructCostPLReviewResultRepo>()
                        .Create((from e in elements.Where(x => !x.IS_GROUP)
                                 select new T_BP_CONTRUCT_COST_PL_REVIEW_RESULT
                                 {
                                     PKID = Guid.NewGuid().ToString(),
                ...
                {
                    review.IS_END = true;
                    UnitOfWork.Repository<ContructCostPLReviewRepo>().Update(review);
                }
        ...
        public override void PheDuyetTongKiemSoat(string orgCode, int year, int version)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_CONTRUCT_COST_PL_DATA SET STATUS = '{Approve_Status.TKS_PheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        internal override void TuChoiTongKiemSoat(string orgCode, int year, int version, string comment)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_CONTRUCT_COST_PL_DATA SET STATUS = '{Approve_Status.TKS_TuChoi}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
                UnitOfWork.Repository<ContructCostPLReviewRepo>()
        ...
        public override void TrinhDuyetTongKiemSoat(string orgCode, int year, int version)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_CONTRUCT_COST_PL_DATA SET STATUS = '{Approve_Status.TKS_TrinhDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
                UnitOfWork.Repository<ContructCostPLReviewRepo>()
                    .Update(x => x.ORG_CODE == orgCode
                                && x.TIME_YEAR == year
                                && x.DATA_VERSION == version
        ...
        public override void ChuyenHDNS(string code)
        {
            try
            {
                ...
                {
                    UnitOfWork.Repository<ContructCostPLReviewRepo>().Update(x => x.DATA_VERSION == ObjDetail.VERSION
        ...
        public override void GetListOfChild()
        {
            ...
            // Tìm mẫu nộp hộ
            var listTemplateCode = this.UnitOfWork.Repository<TemplateDetailContructCostPLRepo>().Queryable().Where(
        ...
        {
            var query = UnitOfWork.GetSession().QueryOver<T_BP_CONTRUCT_COST_PL_SUM_UP_DETAIL>();
        ...
        public IList<T_MD_COST_PL_ELEMENT> GetDataCost(out IList<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_PL> detailCostElements,
            out IList<T_BP_CONTRUCT_COST_PL_DATA> detailCostData,
        ...
        {
            var plDataCostElements = UnitOfWork.Repository<ContructCostPLDataRepo>()
                .GetPLDataByCenterCode(null, new List<string> { centerCode }, year, null, null);
        ...
        private IList<T_MD_COST_PL_ELEMENT> GetDataCostPreview(out IList<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_PL> detailCostElements, string templateId, List<string> lstOrgs, int year, int? version, bool? isHasValue)
        {
            detailCostElements = new List<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_PL>();
            var lstElements = new List<T_MD_COST_PL_ELEMENT>();
            foreach (var orgCode in lstOrgs)
            {
                var elements = GetDataCostPreview(out IList<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_PL> detail, templateId, orgCode, year, version, isHasValue);
        ...
        public T_BP_CONTRUCT_COST_PL_VERSION GetContructCostPLHeader(ViewDataCenterModel model)
        {
            return GetHeader(model.TEMPLATE_CODE, model.ORG_CODE, model.YEAR, model.VERSION);
        }

        public override T_BP_CONTRUCT_COST_PL_VERSION GetHeader(string templateCode, string orgCode, int year, int? version)
        {
            templateCode = templateCode ?? string.Empty;
            if (version == null)
            {
                return UnitOfWork.Repository<ContructCostPLVersionRepo>()
        ...
        {
            ObjListVersion = UnitOfWork.Repository<ContructCostPLVersionRepo>()
        ...
        {
            var query = this.UnitOfWork.GetSession().QueryOver<T_BP_CONTRUCT_COST_PL_HISTORY>();
        ...
        public override void GetHistory(string orgCode, string templateId, int? year)
        {
            if (year.HasValue)
            {
                this.ObjListHistory = UnitOfWork.Repository<ContructCostPLHistoryRepo>().GetManyByExpression(
        ...
        public override void ValidateData(DataTable dataTable, bool isDataBase)
        {
            ...
            var lstDetailTemplate = this.UnitOfWork.Repository<TemplateDetailContructCostPLRepo>().GetManyByExpression(x => x.TIME_YEAR == this.ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == this.ObjDetail.TEMPLATE_CODE);
        ...
        public override void ImportExcel(HttpRequestBase request)
        {
            ...
            var dataCurrent = new List<T_BP_CONTRUCT_COST_PL_DATA>();
            if (costPLCurrent != null)
            {
                dataCurrent = UnitOfWork.Repository<ContructCostPLDataRepo>().Queryable().Where(x => x.ORG_CODE == orgCode
            ...
            try
            {
                ...
                else
                {
                    ...
                    CurrentRepository.Create(new T_BP_CONTRUCT_COST_PL()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                        TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                UnitOfWork.Repository<ContructCostPLVersionRepo>().Create(new T_BP_CONTRUCT_COST_PL_VERSION()
                {
                    PKID = Guid.NewGuid().ToString(),
                    ORG_CODE = orgCode,
                    TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                {
                    var costDataHis = (T_BP_CONTRUCT_COST_PL_DATA_HISTORY)item;
                    UnitOfWork.Repository<ContructCostPLDataHistoryRepo>().Create(costDataHis);
                    UnitOfWork.Repository<ContructCostPLDataRepo>().Delete(item);
                }
                ...
                for (int i = this.StartRowData; i < actualRows; i++)
                {
                    ...
                    var costData = new T_BP_CONTRUCT_COST_PL_DATA()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                        ...
                        STATUS = Approve_Status.ChuaTrinhDuyet,
                        VERSION = versionNext,
                        COST_PL_ELEMENT_CODE = tableData.Rows[i][10].ToString().Trim(),
                    ...
                    UnitOfWork.Repository<ContructCostPLDataRepo>().Create(costData);
                }
                UnitOfWork.Commit();
                NotifyUtilities.CreateNotify(
                    new NotifyPara()
        ...
        public override void ImportExcelBase(HttpRequestBase request)
        {
            ...
            var orgCode = ProfileUtilities.User?.ORGANIZE_CODE;
            var costPLDataRepo = UnitOfWork.Repository<ContructCostPLDataRepo>();
            var costPLDataBaseRepo = UnitOfWork.Repository<ContructCostPLDataBaseRepo>();
            var currentUser = ProfileUtilities.User?.USER_NAME;
            ...
            // Lấy dữ liệu của version hiện tại
            var dataCurrent = new List<T_BP_CONTRUCT_COST_PL_DATA>();
            var dataCurrentBase = new List<T_BP_CONTRUCT_COST_PL_DATA_BASE>();
            if (costPLCurrent != null)
            {
            ...
            try
            {
                ...
                foreach (var key in lookUp.Select(x => x.Key))
                {
                    var costData = new T_BP_CONTRUCT_COST_PL_DATA()
                    {
                        ...
                        VALUE_JAN = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[16].ToString())),
                        VALUE_FEB = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[19].ToString())),
                        VALUE_MAR = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[22].ToString())),
                        VALUE_APR = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[25].ToString())),
                        VALUE_MAY = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[28].ToString())),
                        VALUE_JUN = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[31].ToString())),
                        VALUE_JUL = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[34].ToString())),
                        VALUE_AUG = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[37].ToString())),
                        VALUE_SEP = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[40].ToString())),
                        VALUE_OCT = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[43].ToString())),
                        VALUE_NOV = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[46].ToString())),
                        VALUE_DEC = lookUp[key].Sum(x => UtilsCore.StringToDecimal(x[49].ToString())),
                        CREATE_BY = currentUser
                    };
                ...
                var lstBaseDataHistory = (from x in dataCurrentBase
                                          select (T_BP_CONTRUCT_COST_PL_DATA_BASE_HISTORY)x).ToList();
                UnitOfWork.Repository<ContructCostPLDataBaseHistoryRepo>().Create(lstObj: lstBaseDataHistory);
                ...
                for (int i = this.StartRowData; i < actualRows; i++)
                {
                    ...
                    var j = 14;
                    var costData = new T_BP_CONTRUCT_COST_PL_DATA_BASE()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
        ...
        public override IList<T_BP_CONTRUCT_COST_PL_VERSION> GetVersions(string orgCode, string templateId, int year)
        {
        ...
        {
            return UnitOfWork.Repository<ContructCostPLDataRepo>()
        ...
        {
            return UnitOfWork.Repository<ContructCostPLDataHistoryRepo>()
                .GetManyByExpression(x => lstOrgCodes.Contains(x.ORG_CODE) && x.TEMPLATE_CODE != string.Empty && (!year.HasValue || x.TIME_YEAR == year.Value))
                .Select(x => x.TEMPLATE_CODE)
                .Distinct()
                .ToList();
        ...
        public override IList<int> GetTemplateVersion(string templateId, string centerCode, int year)
        {
            if (string.IsNullOrEmpty(templateId))
            {
                return UnitOfWork.Repository<ContructCostPLSumUpDetailRepo>()
        ...
        {
            var dataCost = PreparePureList(out IList<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_PL> detailCostElements, templateId, year, ignoreAuth: true);
            ...
            try
            {
                ...
                templateWorkbook.SetSheetName(0, ModulType.GetTextSheetName(ModulType.ContructCostPL));
        ...
        public override IList<NodeDataFlow> BuildDataFlowTree(string orgCode, int year, int? version, int? sumUpVersion)
        {
            ...
            if (costCenterRepo.GetManyByExpression(x => x.PARENT_CODE == orgCode).Count > 0 && version == null)
            {
                ...
                {
                    var costPLData = UnitOfWork.Repository<ContructCostPLSumUpDetailRepo>()
            ...
            else
            {
                // level 2: department
                // get templates
                var sumupDetails = UnitOfWork.Repository<ContructCostPLSumUpDetailRepo>()
        ...
        /// <returns></returns>
        private (IList<T_BP_CONTRUCT_COST_PL_DATA>, bool) GetDataSumUp(int year, string centerCode, string elementCode, int sumUpVersion)
        {
            var plDataRepo = UnitOfWork.Repository<ContructCostPLDataRepo>();
            var costPlReviewCommentRepo = UnitOfWork.Repository<ContructCostPLReviewCommentRepo>();
            var plVersionRepo = UnitOfWork.Repository<ContructCostPLRepo>();
            var plDataHistoryRepo = UnitOfWork.Repository<ContructCostPLDataHistoryRepo>();

            var lstDetails = UnitOfWork.Repository<ContructCostPLSumUpDetailRepo>()
            ...
            var data = new List<T_BP_CONTRUCT_COST_PL_DATA>();
            foreach (var key in lookup.Select(x => x.Key))
            {
                var costPL = new List<T_BP_CONTRUCT_COST_PL_DATA>();
        ...
        public override IEnumerable<T_MD_COST_PL_ELEMENT> GetDetailSumUp(string centerCode, string elementCode, int year, int version, int? sumUpVersion, bool isCountComments, bool? isShowFile = null)
        {
            ...
            if (IsLeaf(centerCode))
            {
                ...
                foreach (var key in lookupData.Select(x => x.Key))
                {
                    ...
                    {
                        var isNewestVersion = UnitOfWork.Repository<ContructCostPLVersionRepo>()
                        ...
                        {
                            // get data from base
                            var baseData = UnitOfWork.Repository<ContructCostPLDataBaseRepo>()
                        ...
                        else
                        {
                            ...
                            var baseDataHistory = UnitOfWork.Repository<ContructCostPLDataBaseHistoryRepo>()
                                .GetManyWithFetch(x => x.COST_PL_ELEMENT_CODE == elementCode && x.ORG_CODE == centerCode && x.TEMPLATE_CODE == key && x.VERSION == lookupData[key].First().VERSION && x.TIME_YEAR == year);
            ...
            {
                var comments = new List<T_BP_CONTRUCT_COST_PL_REVIEW_COMMENT>();
                if (isCountComments)
                {
        ...
        public override IEnumerable<T_MD_COST_PL_ELEMENT> GetDetailSumUpTemplate(string elementCode, int year, int version, string templateCode, string centerCode)
        {
            ...
            else
            {
                if (newestVersionInDb.Value == version)
                {
                    ...
                    var baseData = UnitOfWork.Repository<ContructCostPLDataBaseRepo>().GetManyWithFetch(x => x.TEMPLATE_CODE == templateCode && x.TIME_YEAR == year && x.COST_PL_ELEMENT_CODE == elementCode && x.VERSION == version && x.INTERNAL_ORDER_CODE == centerCode);
                ...
                else
                {
                    ...
                    var baseData = UnitOfWork.Repository<ContructCostPLDataBaseHistoryRepo>().GetManyWithFetch(x => x.TEMPLATE_CODE == templateCode && x.TIME_YEAR == year && x.COST_PL_ELEMENT_CODE == elementCode && x.VERSION == version && x.INTERNAL_ORDER_CODE == centerCode);
                    return from item in baseData
                           select (T_MD_COST_PL_ELEMENT)item;
                }
            }
        ...
        public IList<T_MD_COST_PL_ELEMENT> PreparePureList(IList<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_PL> detailCostElements, int year)
        {
        ...
        /// <param name="centerCode"></param>
        /// <returns></returns>
        public IList<T_MD_COST_PL_ELEMENT> PreparePureList(out IList<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_PL> detailCostElements,
            string templateId,
            int year,
            string centerCode = "",
            bool ignoreAuth = false)
        {
            ...
            {
                detailCostElements = UnitOfWork.Repository<TemplateDetailContructCostPLRepo>()
        ...
        /// <param name="templateId"></param>
        /// <returns></returns>
        public IList<T_MD_COST_PL_ELEMENT> PreparePureListForTemplate(out IList<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_PL> detailCostElements,
            string templateId,
            int year)
        {
        ...
        {
            // Tìm mẫu nộp hộ
            var listTemplateCodes = this.UnitOfWork.Repository<TemplateDetailContructCostPLRepo>()
                .GetManyByExpression(x => centerCodes.Contains(x.CENTER_CODE) && x.TIME_YEAR == year)
                .Select(x => x.TEMPLATE_CODE).Distinct().ToList();
            var findContructCostPL = this.CurrentRepository.GetManyByExpression(
                    x => listTemplateCodes.Contains(x.TEMPLATE_CODE) && !centerCodes.Contains(x.ORG_CODE));
            var lst = new List<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_PL>();
            detailCostElements = new List<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_PL>();
            foreach (var template in listTemplateCodes)
            {
                lst.AddRange(UnitOfWork.Repository<TemplateDetailContructCostPLRepo>()
        ...
        public override void SumUpDataCenter(out T_BP_CONTRUCT_COST_PL_VERSION costPL, string centerCode, int year)
        {
            ...
            var lstData = new List<T_BP_CONTRUCT_COST_PL_DATA>();
            costPL = new T_BP_CONTRUCT_COST_PL_VERSION();

            try
            {
                UnitOfWork.BeginTransaction();
                var sumUpDetailRepo = UnitOfWork.Repository<ContructCostPLSumUpDetailRepo>();
                var costPLDataRepo = UnitOfWork.Repository<ContructCostPLDataRepo>();
                ...
                var costPLDataApproved = costPLDataRepo.GetManyByExpression(x => (x.ORG_CODE == centerCode && !string.IsNullOrEmpty(x.TEMPLATE_CODE)) ||
                ...
                foreach (var code in lookup.Select(x => x.Key))
                {
                    ...
                    {
                        lstData.Add((T_BP_CONTRUCT_COST_PL_DATA)lookup[code].First().CloneObject());
                    }
                    else
                    {
                        lstData.Add(new T_BP_CONTRUCT_COST_PL_DATA
                        {
                ...
                var newestPL = UnitOfWork.Repository<ContructCostPLRepo>()
                ...
                UnitOfWork.Repository<ContructCostPLVersionRepo>().Create(costPL);
                ...
                _ = UnitOfWork.GetSession().CreateSQLQuery($"DELETE FROM T_BP_CONTRUCT_COST_PL_DATA WHERE ORG_CODE = '{centerCode}' AND TIME_YEAR = {year}")
                    .ExecuteUpdate();

                // insert to pl data history
                UnitOfWork.Repository<ContructCostPLDataHistoryRepo>().Create((from pl in lstCostPLDataOldVersion
                                                                               select (T_BP_CONTRUCT_COST_PL_DATA_HISTORY)pl).ToList());
                ...
                var lstContructCostPL = costPLDataApproved.ToLookup(x => new { OrgCode = x.ORG_CODE, TemplateCode = x.TEMPLATE_CODE, TemplateVersion = x.VERSION });

                // create list sum up detail
                sumUpDetailRepo.Create((from c in lstContructCostPL.Select(x => x.Key)
                                        select new T_BP_CONTRUCT_COST_PL_SUM_UP_DETAIL
                                        {
                                            PKID = Guid.NewGuid().ToString(),
            ...
            string templateId = "")
        {
            ...
            plDataCostElements = UnitOfWork.Repository<ContructCostPLDataRepo>()
        ...
        private IEnumerable<T_MD_COST_PL_ELEMENT> GetSumDescendants(
            IEnumerable<T_MD_TEMPLATE_DETAIL_CONTRUCT_COST_PL> details,
            ...
            foreach (var centerCode in lookupCenter.Select(x => x.Key))
            {
                ...
                while (st.Count > 0)
                {
                    ...
                    foreach (var i in lookup[item.CODE])
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (detail != null)
                                {
                                    detail.PLData = costPLData.FirstOrDefault(x => x.COST_PL_ELEMENT_CODE == detail.ELEMENT_CODE && x.INTERNAL_ORDER_CODE == detail.CENTER_CODE);
                                }
                    ...
                    if (shouldReturn)
                    {
                        ...
                        {
                            var data = costPLData.Where(x => x.COST_PL_ELEMENT_CODE == item.CODE && x.INTERNAL_ORDER_CODE == item.CENTER_CODE);
                            if (data != null)
                            {
        ...
        private IList<T_BP_CONTRUCT_COST_PL_DATA> GetVersionData(
            string templateCode,
            string centerCode,
            int? year = null,
            int? version = null)
        {
            ...
            {
                return UnitOfWork.Repository<ContructCostPLDataRepo>().GetPLDataByOrgCode(template.ORG_CODE, year.Value, templateCode, version);
            }
            ...
            {
                return UnitOfWork.Repository<ContructCostPLDataRepo>().GetPLDataByCenterCode(template.ORG_CODE, new List<string> { centerCode }, year.Value, templateCode, version);
                // get 
            }
        }
        ...
        public IList<T_MD_COST_PL_ELEMENT> SummaryCenterOut(out IList<T_BP_CONTRUCT_COST_PL_DATA> plDataCostElements,
                                                              string centerCode,
                                                              int year,
                                                              int? version,
                                                              bool? isHasValue = null)
        ...
        /// <returns></returns>
        public IList<T_MD_COST_PL_ELEMENT> SummaryCenterVersion(out IList<T_BP_CONTRUCT_COST_PL_DATA> plDataCostElements,
                                                              string centerCode,
                                                              int year,
                                                              int? version,
        ...
        /// <returns></returns>
        private IList<T_BP_CONTRUCT_COST_PL_DATA> GetAllSumUpDetails(string centerCode, int year, int version)
        {
            var detailsSumUp = UnitOfWork.Repository<ContructCostPLSumUpDetailRepo>()
               .GetManyWithFetch(x => x.ORG_CODE == centerCode && x.TIME_YEAR == year && x.SUM_UP_VERSION == version);

            var lstChildren = GetListOfChildrenCenter(centerCode).Select(x => x.CODE);
            var lstDetails = new List<T_BP_CONTRUCT_COST_PL_DATA>();
            var lstResult = new List<T_BP_CONTRUCT_COST_PL_DATA>();
            var plDataRepo = UnitOfWork.Repository<ContructCostPLDataRepo>();
                ...
                else
                {
                    ...
                    foreach (var orgCode in lookupCenter.Select(x => x.Key))
                    {
                        ...
                        else
                        {
                            ...
                            lstResult.Add(new T_BP_CONTRUCT_COST_PL_DATA
                            {
                                ORG_CODE = orgCode,
        ...
        private IList<T_MD_COST_PL_ELEMENT> SummaryCenter(IList<T_BP_CONTRUCT_COST_PL_DATA> plDataCostElements, string centerCode, int year,
                                                          bool? isHasValue = null)
        {
        ...
        private IEnumerable<T_MD_COST_PL_ELEMENT> GetSumDescendants(
            IList<T_BP_CONTRUCT_COST_PL_DATA> costPLData,
        ...
        public override void GenerateExportExcel(
            ref MemoryStream outFileStream,
            string html,
            string path,
            int year,
            string centerCode,
        ...
        {
            var name = "DỮ LIỆU KẾ HOẠCH CHI PHÍ KINH DOANH XÂY DỰNG DỰ ÁN";
        ...
        {
            return UnitOfWork.Repository<TypeRepo>().GetFirstWithFetch(x => x.OBJECT_TYPE == TemplateObjectType.DevelopProject && x.ELEMENT_TYPE == ElementType.ChiPhi && x.BUDGET_TYPE == BudgetType.KinhDoanh);
        }
    }
