...
{
    public class ContructCostPLReviewService : BaseBPReviewService<T_BP_CONTRUCT_COST_PL_REVIEW, T_BP_CONTRUCT_COST_PL_REVIEW_RESULT, ContructCostPLReviewRepo>
    {
        /// <summary>
        /// List lịch sử thẩm định và tổng kiểm soát
        /// </summary>
        public IList<T_BP_CONTRUCT_COST_PL_HISTORY> ListHistory { get; set; }
        /// <summary>
        /// List tổng kiểm soát dữ liệu
        /// </summary>
        public IList<T_BP_CONTRUCT_COST_PL_REVIEW> ListControl { get; set; }

        public ContructCostPLReviewService()
        {
            ListHistory = new List<T_BP_CONTRUCT_COST_PL_HISTORY>();
            ListControl = new List<T_BP_CONTRUCT_COST_PL_REVIEW>();
        }
        /// <summary>
        /// Lấy dữ liệu lần thẩm định cuối cùng
        /// </summary>
        /// <returns></returns>
        internal IList<T_MD_COST_PL_ELEMENT> GetLastReview()
        {
        ...
        /// </summary>
        /// <returns></returns>
        internal IList<T_MD_COST_PL_ELEMENT> GetLastReviewSummary()
        {
            var header = GetNewestByExpression(x =>
            x.ORG_CODE == ObjDetail.ORG_CODE &&
            x.TIME_YEAR == ObjDetail.TIME_YEAR &&
        ...
        internal ReviewCenterViewModelBase<ContructCostPLElementReviewCenter> SummaryReviewDataCenter(ReviewDataCenterModel model)
        {
            var corp = GetCorp()?.CODE;

            var dataCost = SummaryCenterVersion(out IList<T_BP_CONTRUCT_COST_PL_DATA> detailCostElements, model, corp);
            dataCost = dataCost.OrderBy(x => x.C_ORDER)
                                            .GroupBy(x => x.CODE)
                                            .Select(x => x.First()).ToList();

            return new ReviewCenterViewModelBase<ContructCostPLElementReviewCenter>
            {
        ...
        private IList<ContructCostPLElementReviewCenter> PrepareListReviewSummary(IList<T_MD_COST_PL_ELEMENT> dataCost, ReviewDataCenterModel model, string corp)
        {
            if (!model.IS_COMPLETED && !model.IS_NOT_COMPLETED)
            {
                return new List<ContructCostPLElementReviewCenter>();
            }
            ...
            var comments = UnitOfWork.Repository<ContructCostPLReviewCommentRepo>()
            ...
            if (lstReviews == null || lstReviews.Count == 0)
            {
                ...
                else
                {
                    return (from d in dataCost
                            let result = objReview?.Results.FirstOrDefault(x => x.COST_PL_ELEMENT_CODE.Equals(d.CODE))
                            let status = result?.RESULT
                            let elementComments = comments.Where(x => x.COST_PL_ELEMENT_CODE == d.CODE)
                            let elementCommentsInOrg = elementComments.Where(x => x.ON_ORG_CODE == d.ORG_CODE)
                            select new ContructCostPLElementReviewCenter(
                                d,
                                null,
                                null,
                                null,
            ...
            var lstReviewResult = UnitOfWork.Repository<ContructCostPLReviewResultRepo>()
                .GetManyByExpression(x => lstReviews.Any(y => y.PKID.Equals(x.HEADER_ID)));

            var lookupReviewByElements = lstReviewResult.ToLookup(x => x.COST_PL_ELEMENT_CODE);
        ...
        internal T_BP_CONTRUCT_COST_PL GetHeader()
        {
            return new ContructCostPLService().GetBPHeader(string.Empty, ObjDetail.DATA_VERSION, ObjDetail.TIME_YEAR, GetCorp().CODE);
        }
        ...
        internal void SearchReview()
        {
            ...
            // Dữ liệu thẩm định
            ObjList = UnitOfWork.Repository<ContructCostPLReviewRepo>().GetManyWithFetch(x => x.TIME_YEAR == ObjDetail.TIME_YEAR
        ...
        internal IList<T_BP_CONTRUCT_COST_PL_HISTORY> GetHistory(int year, IEnumerable<string> lstActionsFilter, int? version = null)
        {
            var corp = GetCorp().CODE;
            if (version == null)
            {
                return UnitOfWork.Repository<ContructCostPLHistoryRepo>()
        ...
        /// <returns></returns>
        private T_BP_CONTRUCT_COST_PL_REVIEW GetControlReview(ReviewDataCenterModel model, string org)
        {
            T_BP_CONTRUCT_COST_PL_REVIEW objReview = null;
            if (model.IS_CONTROL)
            {
        ...
        /// <param name="org"></param>
        /// <returns></returns>
        private IList<T_BP_CONTRUCT_COST_PL_REVIEW> GetCouncils(ReviewDataCenterModel model, string org)
        {
            IList<T_BP_CONTRUCT_COST_PL_REVIEW> lstReviews = null;
        ...
        internal void UpdateReview(ContructCostPLReviewViewModel model)
        {
            // check budget period
            if (model.IsSummary)
            {
            ...
            if (resultCheckConstraint)
            {
                try
                {
                    ...
                    T_BP_CONTRUCT_COST_PL_REVIEW headerFound;
                    ...
                    if (headerFound == null)
                    {
                        ...
                        ObjDetail = new T_BP_CONTRUCT_COST_PL_REVIEW
                        {
                            PKID = headerId,
                    ...
                    else
                    {
                        ...
                        if (model.Status.HasValue)
                        {
                            ...
                            UnitOfWork.Repository<ContructCostPLReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId) && x.RESULT == model.Status.Value);
                        }
                        else
                        {
                            UnitOfWork.Repository<ContructCostPLReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId));
                        }
                    }

                    // create history
                    var history = new T_BP_CONTRUCT_COST_PL_REVIEW_HISTORY
                    {
                    ...
                    UnitOfWork.Repository<ContructCostPLReviewHistoryRepo>().Create(history);

                    // create history
                    var historyPL = new T_BP_CONTRUCT_COST_PL_HISTORY
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = model.OrgCode,
                        TIME_YEAR = model.Year,
                    ...
                    UnitOfWork.Repository<ContructCostPLHistoryRepo>().Create(historyPL);
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private bool CheckConstraintComments(ContructCostPLReviewViewModel model, out IDictionary<string, string> elementsNotValid)
        {
            ...
            var queryable = UnitOfWork.GetSession().Query<T_BP_CONTRUCT_COST_PL_REVIEW_COMMENT>();
            ...
            queryable = queryable.Where(x => elementsNotSuccess.Contains(x.COST_PL_ELEMENT_CODE));
            queryable = queryable.FetchMany(x => x.Comments);

            var lookupComments = queryable.ToList().ToLookup(x => x.COST_PL_ELEMENT_CODE);
            var allElements = GetAllMasterData<CostPLElementRepo, T_MD_COST_PL_ELEMENT>().Where(x => x.TIME_YEAR == model.Year);
        ...
        {
            var corp = GetCorp()?.CODE;
            return UnitOfWork.Repository<ContructCostPLVersionRepo>().GetManyByExpression(x => x.TIME_YEAR == year && x.ORG_CODE == corp)
                .Select(x => x.VERSION)
                .Distinct()
        ...
        internal IList<T_MD_COST_PL_ELEMENT> SummaryCenterVersion(out IList<T_BP_CONTRUCT_COST_PL_DATA> detailCostElements, int? version = null)
        {
            return new ContructCostPLService().SummaryCenterVersion(out detailCostElements, ObjDetail.ORG_CODE, ObjDetail.TIME_YEAR, version);
        }

        internal IList<T_MD_COST_PL_ELEMENT> SummaryCenterVersion(out IList<T_BP_CONTRUCT_COST_PL_DATA> detailCostElements, ReviewDataCenterModel model, string corp)
        {
            return new ContructCostPLService().SummaryCenterVersion(out detailCostElements, corp, model.YEAR, model.VERSION);
        }

        internal IList<ContructCostPLElementReview> PrepareListReview(IList<T_MD_COST_PL_ELEMENT> dataCost)
        {
            var comments = UnitOfWork.Repository<ContructCostPLReviewCommentRepo>()
                    .GetManyByExpression(x =>
                    x.ORG_CODE == ObjDetail.ORG_CODE &&
            ...
            var objReview = new T_BP_CONTRUCT_COST_PL_REVIEW();
            if (!IsReview)
            {
                objReview = GetFirstByExpression(
            ...
            return (from d in dataCost
                    ...
                    let elementComments = comments.Where(x => x.COST_PL_ELEMENT_CODE == d.CODE)
                    let elementCommentsInOrg = elementComments.Where(x => x.ON_ORG_CODE == d.ORG_CODE)
                    select new ContructCostPLElementReview(
                        d,
                        result?.RESULT,
                        elementComments.Sum(x => x.NUMBER_COMMENTS),
                        elementCommentsInOrg.Sum(x => x.NUMBER_COMMENTS)
