...
{
    public class OtherCostCFReviewService : BaseBPReviewService<T_BP_OTHER_COST_CF_REVIEW, T_BP_OTHER_COST_CF_REVIEW_RESULT, OtherCostCFReviewRepo>
    {
        /// <summary>
        /// List lịch sử thẩm định và tổng kiểm soát
        /// </summary>
        public IList<T_BP_OTHER_COST_CF_HISTORY> ListHistory { get; set; }
        /// <summary>
        /// List tổng kiểm soát dữ liệu
        /// </summary>
        public IList<T_BP_OTHER_COST_CF_REVIEW> ListControl { get; set; }

        public OtherCostCFReviewService()
        {
            ListHistory = new List<T_BP_OTHER_COST_CF_HISTORY>();
            ListControl = new List<T_BP_OTHER_COST_CF_REVIEW>();
        }
        /// <summary>
        ...
        internal ReviewCenterViewModelBase<OtherCostCFElementReviewCenter> SummaryReviewDataCenter(ReviewDataCenterModel model)
        {
            var corp = GetCorp()?.CODE;

            var dataCost = SummaryCenterVersion(out IList<T_BP_OTHER_COST_CF_DATA> detailCostElements, model, corp);
            dataCost = dataCost.OrderBy(x => x.C_ORDER)
                                            .GroupBy(x => x.CODE)
                                            .Select(x => x.First()).ToList();

            return new ReviewCenterViewModelBase<OtherCostCFElementReviewCenter>
            {
        ...
        private IList<OtherCostCFElementReviewCenter> PrepareListReviewSummary(IList<T_MD_COST_CF_ELEMENT> dataCost, ReviewDataCenterModel model, string corp)
        {
            if (!model.IS_COMPLETED && !model.IS_NOT_COMPLETED)
            {
                return new List<OtherCostCFElementReviewCenter>();
            }
            ...
            var comments = UnitOfWork.Repository<OtherCostCFReviewCommentRepo>()
            ...
            if (lstReviews == null || lstReviews.Count == 0)
            {
                ...
                else
                {
                    return (from d in dataCost
                            ...
                            select new OtherCostCFElementReviewCenter(
                                d,
                                null,
                                null,
                                null,
            ...
            var lstReviewResult = UnitOfWork.Repository<OtherCostCFReviewResultRepo>()
        ...
        internal T_BP_OTHER_COST_CF GetHeader()
        {
            return new OtherCostCFService().GetBPHeader(string.Empty, ObjDetail.DATA_VERSION, ObjDetail.TIME_YEAR, GetCorp().CODE);
        }
        ...
        internal void SearchReview()
        {
            ...
            // Dữ liệu thẩm định
            ObjList = UnitOfWork.Repository<OtherCostCFReviewRepo>().GetManyWithFetch(x => x.TIME_YEAR == ObjDetail.TIME_YEAR
        ...
        internal IList<T_BP_OTHER_COST_CF_HISTORY> GetHistory(int year, IEnumerable<string> lstActionsFilter, int? version = null)
        {
            var corp = GetCorp().CODE;
            if (version == null)
            {
                return UnitOfWork.Repository<OtherCostCFHistoryRepo>()
        ...
        /// <returns></returns>
        private T_BP_OTHER_COST_CF_REVIEW GetControlReview(ReviewDataCenterModel model, string org)
        {
            T_BP_OTHER_COST_CF_REVIEW objReview = null;
            if (model.IS_CONTROL)
            {
        ...
        /// <param name="org"></param>
        /// <returns></returns>
        private IList<T_BP_OTHER_COST_CF_REVIEW> GetCouncils(ReviewDataCenterModel model, string org)
        {
            IList<T_BP_OTHER_COST_CF_REVIEW> lstReviews = null;
        ...
        internal void UpdateReview(OtherCostCFReviewViewModel model)
        {
            // check budget period
            if (model.IsSummary)
            {
            ...
            if (resultCheckConstraint)
            {
                try
                {
                    ...
                    T_BP_OTHER_COST_CF_REVIEW headerFound;
                    ...
                    if (headerFound == null)
                    {
                        ...
                        ObjDetail = new T_BP_OTHER_COST_CF_REVIEW
                        {
                            PKID = headerId,
                    ...
                    else
                    {
                        ...
                        if (model.Status.HasValue)
                        {
                            ...
                            UnitOfWork.Repository<OtherCostCFReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId) && x.RESULT == model.Status.Value);
                        }
                        else
                        {
                            UnitOfWork.Repository<OtherCostCFReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId));
                        }
                    }

                    // create history
                    var history = new T_BP_OTHER_COST_CF_REVIEW_HISTORY
                    {
                    ...
                    UnitOfWork.Repository<OtherCostCFReviewHistoryRepo>().Create(history);

                    // create history
                    var historyPL = new T_BP_OTHER_COST_CF_HISTORY
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = model.OrgCode,
                        TIME_YEAR = model.Year,
                    ...
                    UnitOfWork.Repository<OtherCostCFHistoryRepo>().Create(historyPL);
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private bool CheckConstraintComments(OtherCostCFReviewViewModel model, out IDictionary<string, string> elementsNotValid)
        {
            ...
            var queryable = UnitOfWork.GetSession().Query<T_BP_OTHER_COST_CF_REVIEW_COMMENT>();
        ...
        {
            var corp = GetCorp()?.CODE;
            return UnitOfWork.Repository<OtherCostCFVersionRepo>().GetManyByExpression(x => x.TIME_YEAR == year && x.ORG_CODE == corp)
                .Select(x => x.VERSION)
                .Distinct()
        ...
        internal IList<T_MD_COST_CF_ELEMENT> SummaryCenterVersion(out IList<T_BP_OTHER_COST_CF_DATA> detailCostElements, int? version = null)
        {
            return new OtherCostCFService().SummaryCenterVersion(out detailCostElements, ObjDetail.ORG_CODE, ObjDetail.TIME_YEAR, version);
        }

        internal IList<T_MD_COST_CF_ELEMENT> SummaryCenterVersion(out IList<T_BP_OTHER_COST_CF_DATA> detailCostElements, ReviewDataCenterModel model, string corp)
        {
            return new OtherCostCFService().SummaryCenterVersion(out detailCostElements, corp, model.YEAR, model.VERSION);
        }

        internal IList<OtherCostCFElementReview> PrepareListReview(IList<T_MD_COST_CF_ELEMENT> dataCost)
        {
            var comments = UnitOfWork.Repository<OtherCostCFReviewCommentRepo>()
                    .GetManyByExpression(x =>
                    x.ORG_CODE == ObjDetail.ORG_CODE &&
            ...
            var objReview = new T_BP_OTHER_COST_CF_REVIEW();
            if (!IsReview)
            {
                objReview = GetFirstByExpression(
            ...
            return (from d in dataCost
                    ...
                    let elementComments = comments.Where(x => x.COST_CF_ELEMENT_CODE == d.CODE)
                    let elementCommentsInOrg = elementComments.Where(x => x.ON_ORG_CODE == d.ORG_CODE)
                    select new OtherCostCFElementReview(
                        d,
                        result?.RESULT,
                        elementComments.Sum(x => x.NUMBER_COMMENTS),
                        elementCommentsInOrg.Sum(x => x.NUMBER_COMMENTS)
