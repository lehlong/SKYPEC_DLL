...
{
    public class RevenuePLElementService : GenericService<T_MD_REVENUE_PL_ELEMENT, RevenuePLElementRepo>
    {
        ...
        public List<NodeCostElement> GetNodeSapCostElement(int year)
        {
            this.ObjDetail.TIME_YEAR = year;
            this.Search();
            var lstSapCostElm = UnitOfWork.Repository<SAPMdGLAccountRepo>().GetAll().OrderBy(x => x.CODE).ToList();
            var lstNode = new List<NodeCostElement>();
        ...
        public void Copy(int year, int yearCopy)
        {
            try
            {
                ...
                {
                    var newElement = new T_MD_REVENUE_PL_ELEMENT()
                    {
        ...
        public void UpdateTree(List<NodeCostCenter> lstNode, List<string> lstRemove, List<string> lstAdd, int year)
        {
            try
            {
                ...
                {
                    var findSap = UnitOfWork.Repository<SAPMdGLAccountRepo>().Get(item);
                    if (findSap != null)
                    {
                        var newElement = new T_MD_REVENUE_PL_ELEMENT()
                        {
